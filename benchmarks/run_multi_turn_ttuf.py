...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Measure cold and warm TTUF for multiple sequential requests"
    )
    parser.add_argument("--model", required=True)
    parser.add_argument(
        "--prompt-template",
        default="Write a short numbered list about GUI automation. Turn {turn}.",
    )
...
def _build_cfie_engine_args(args: argparse.Namespace):
    ...
    if args.cudagraph_capture_sizes is not None:
        engine_args.compilation_config.cudagraph_capture_sizes = sorted(
            set(int(size) for size in args.cudagraph_capture_sizes)
        )
    if args.cudagraph_decode_capture_sizes is not None:
        engine_args.compilation_config.cudagraph_decode_capture_sizes = sorted(
            set(int(size) for size in args.cudagraph_decode_capture_sizes)
        )
    if args.cudagraph_prefill_capture_sizes is not None:
        engine_args.compilation_config.cudagraph_prefill_capture_sizes = sorted(
...
def _run_turn(engine, args: argparse.Namespace, sampling_params, turn: int) -> dict:
    turn_args = argparse.Namespace(**vars(args))
    turn_args.prompt = args.prompt_template.format(turn=turn)
    request_id = f"turn-{turn}"

    render_t0 = time.perf_counter()
    prompt = _render_engine_prompt(engine, turn_args)
    ...
    steady_seconds = max(run_seconds - first_step_seconds, 0.0)
    return {
        "turn": turn,
        ...
        "steady_tokens_per_sec": (
            steady_tokens / steady_seconds if steady_seconds > 0 else 0.0
        ),
        "steps": step_count,
        "generated_text_prefix": "".join(text_parts)[:240],
    }
...
def main() -> None:
    ...
    engine_args = _build_cfie_engine_args(args)
    ...
    engine_init_seconds = time.perf_counter() - engine_t0

    turn_results: list[dict] = []
    try:
        for turn in range(1, args.turns + 1):
            turn_results.append(_run_turn(engine, args, sampling_params, turn))
    finally:
        try:
    ...
    warm_turns = turn_results[1:]
    warm_ttuf_avg = (
        sum(item["first_step_seconds"] for item in warm_turns) / len(warm_turns)
        if warm_turns
        else 0.0
    )
    result = {
        ...
        "turns": turn_results,
        "cold_first_step_seconds": turn_results[0]["first_step_seconds"],
