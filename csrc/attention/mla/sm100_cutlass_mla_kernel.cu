...
template <bool IsPaged128, bool NotManualSplitKV>
void dispatch_sm100_cutlass_mla_by_dtype(
    at::Tensor const& out, at::Tensor const& lse,
    at::Tensor const& q_nope, at::Tensor const& q_pe,
    at::Tensor const& kv_c_and_k_pe_cache, at::Tensor const& seq_lens,
    at::Tensor const& page_table, at::Tensor const& workspace,
    at::ScalarType in_dtype, double sm_scale, int64_t num_kv_splits,
    cudaStream_t stream) {
  if (in_dtype == at::ScalarType::Half) {
    runMla<cutlass::half_t, cutlass::half_t, IsPaged128,
           IsPersistent<NotManualSplitKV>>(
        out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
        workspace, sm_scale, num_kv_splits, stream);
  } else if (in_dtype == at::ScalarType::BFloat16) {
    runMla<cutlass::bfloat16_t, cutlass::bfloat16_t, IsPaged128,
           IsPersistent<NotManualSplitKV>>(
        out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
        workspace, sm_scale, num_kv_splits, stream);
  } else if (in_dtype == at::ScalarType::Float8_e4m3fn) {
    runMla<cutlass::float_e4m3_t, cutlass::bfloat16_t, IsPaged128,
           IsPersistent<NotManualSplitKV>>(
        out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
        workspace, sm_scale, num_kv_splits, stream);
  } else {
...
void sm100_cutlass_mla_decode(
  ...
  if (page_size == 128) {
    if (num_kv_splits <= 1) {
      dispatch_sm100_cutlass_mla_by_dtype<true, true>(
          out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
          workspace, in_dtype, sm_scale, num_kv_splits, stream);
    } else {
      dispatch_sm100_cutlass_mla_by_dtype<true, false>(
          out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
          workspace, in_dtype, sm_scale, num_kv_splits, stream);
    }
  } else {
    if (num_kv_splits <= 1) {
      dispatch_sm100_cutlass_mla_by_dtype<false, true>(
          out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
          workspace, in_dtype, sm_scale, num_kv_splits, stream);
    } else {
      dispatch_sm100_cutlass_mla_by_dtype<false, false>(
          out, lse, q_nope, q_pe, kv_c_and_k_pe_cache, seq_lens, page_table,
          workspace, in_dtype, sm_scale, num_kv_splits, stream);
    }
  }
