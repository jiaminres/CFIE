...
#ifdef USE_ROCM
    #define SELECTIVE_SCAN_SET_MAX_DYNAMIC_SMEM(KERNEL, SIZE) \
...
void selective_scan_fwd_launch_dispatch(SSMParamsBase &params, cudaStream_t stream) {
    ...
    using Ktraits = Selective_Scan_fwd_kernel_traits<
        kNThreads, kNItems, kNRows, kIsEvenLen, kIsVariableB, kIsVariableC,
        kHasZ, kVarlen, input_t, weight_t, state_t>;
    constexpr int kSmemSize =
        Ktraits::kSmemSize + kNRows * MAX_DSTATE * sizeof(typename Ktraits::scan_t);
    dim3 grid(params.batch, params.dim / kNRows);
    auto kernel = &selective_scan_fwd_kernel<Ktraits>;
    if (kSmemSize >= 48 * 1024) {
        SELECTIVE_SCAN_SET_MAX_DYNAMIC_SMEM(kernel, kSmemSize);
    }
...
void selective_scan_fwd_launch(SSMParamsBase &params, cudaStream_t stream) {
    ...
    if (is_even_len) {
        if (has_z) {
            if (is_varlen) {
                selective_scan_fwd_launch_dispatch<
                    kNThreads, kNItems, true, true, true, input_t, weight_t, state_t>(params, stream);
            } else {
                selective_scan_fwd_launch_dispatch<
                    kNThreads, kNItems, true, true, false, input_t, weight_t, state_t>(params, stream);
            }
        } else if (is_varlen) {
            selective_scan_fwd_launch_dispatch<
                kNThreads, kNItems, true, false, true, input_t, weight_t, state_t>(params, stream);
        } else {
            selective_scan_fwd_launch_dispatch<
                kNThreads, kNItems, true, false, false, input_t, weight_t, state_t>(params, stream);
        }
    } else if (has_z) {
        if (is_varlen) {
            selective_scan_fwd_launch_dispatch<
                kNThreads, kNItems, false, true, true, input_t, weight_t, state_t>(params, stream);
        } else {
            selective_scan_fwd_launch_dispatch<
                kNThreads, kNItems, false, true, false, input_t, weight_t, state_t>(params, stream);
        }
    } else if (is_varlen) {
        selective_scan_fwd_launch_dispatch<
            kNThreads, kNItems, false, false, true, input_t, weight_t, state_t>(params, stream);
    } else {
        selective_scan_fwd_launch_dispatch<
            kNThreads, kNItems, false, false, false, input_t, weight_t, state_t>(params, stream);
    }
}
...
#undef SELECTIVE_SCAN_SET_MAX_DYNAMIC_SMEM
