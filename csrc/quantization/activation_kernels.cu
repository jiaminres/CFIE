...
#ifndef USE_ROCM
using bf16x8_storage_t = int4;
#else
using bf16x8_storage_t = __int128_t;
#endif

constexpr __nv_bfloat16 make_bfloat16_from_bits(unsigned short bits) {
  return __nv_bfloat16(__nv_bfloat16_raw{bits});
}
...
constexpr __nv_bfloat16 get_fp8_max() {
  ...
  if constexpr (std::is_same_v<T, c10::Float8_e4m3fn>) {
    return make_bfloat16_from_bits(17376);
  } else {
    return make_bfloat16_from_bits(17264);
  }
}
...
constexpr __nv_bfloat16 get_fp8_min() {
  static_assert(std::is_same_v<T, c10::Float8_e4m3fn> ||
                std::is_same_v<T, c10::Float8_e4m3fnuz>);
  if constexpr (std::is_same_v<T, c10::Float8_e4m3fn>) {
    return make_bfloat16_from_bits(50144);
  } else {
    return make_bfloat16_from_bits(50032);
  }
}
...
__global__ void silu_mul_fp8_quant_deep_gemm_kernel(
  ...
  extern __shared__ __align__(16) bf16x8_storage_t smem_128[];
  ...
  static constexpr __nv_bfloat16 EPS = make_bfloat16_from_bits(11996);
  int tid = threadIdx.x;
  int warp_id = tid >> 5;
  ...
  bf16x8_storage_t* s_hidden_load =
      smem_128 + warp_id * ((2 * 128 / 8) * NUM_STAGES);
  bf16x8_storage_t* smem_load_ptr = s_hidden_load + lane_id;
  ...
  const bf16x8_storage_t* input_128_ptr =
      reinterpret_cast<const bf16x8_storage_t*>(_input) + gate_warp_offset +
      ((lane_id < 16) ? 0 : ((H * stride_i_h) / 8));
  bf16x8_storage_t* load_ptr =
      const_cast<bf16x8_storage_t*>(input_128_ptr + base_i);
  ...
  auto load_and_advance_y_pred = [&] {
    if (t_load < t_load_bound) {
      ...
      if (token_id >= next_expert_offset) {
        ...
        token_offset = 0;
        load_ptr = const_cast<bf16x8_storage_t*>(input_128_ptr + base_i);
      } else {
