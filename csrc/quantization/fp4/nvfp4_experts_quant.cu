...
constexpr auto FLOAT_DTYPE = at::ScalarType::Float;
constexpr auto INT_DTYPE = at::ScalarType::Int;
constexpr auto UINT8_DTYPE = at::ScalarType::Byte;
...
static void validate_fp4_experts_quant_inputs(
  ...
  TORCH_CHECK(input_global_scale.scalar_type() == FLOAT_DTYPE);
  TORCH_CHECK(input_offset_by_experts.scalar_type() == INT_DTYPE);
  TORCH_CHECK(output_scale_offset_by_experts.scalar_type() == INT_DTYPE);
  // output is uint8 (two nvfp4 values are packed into one uint8)
  // output_scale is int32 (four fp8 values are packed into one int32)
  TORCH_CHECK(output.scalar_type() == UINT8_DTYPE);
  TORCH_CHECK(output_scale.scalar_type() == INT_DTYPE);
