...
__device__ inline void mma(
  ...
  [[maybe_unused]] const uint32_t* a =
      reinterpret_cast<const uint32_t*>(&a_frag);
  [[maybe_unused]] const uint32_t* b =
      reinterpret_cast<const uint32_t*>(&frag_b);
  ...
  if constexpr (k_size == 16) {
    if constexpr (std::is_same<scalar_t, half>::value && !use_fp16_accum) {
      ...
      [[maybe_unused]] float* c = reinterpret_cast<float*>(&frag_c);
      asm volatile(
      ...
      asm volatile(
          ...
          : "r"(a[0]), "r"(a[1]), "r"(a[2]), "r"(a[3]), "r"(b[0]), "r"(b[1]),
            "f"(c[0]), "f"(c[1]), "f"(c[2]), "f"(c[3]));
                         ...
                         use_fp16_accum) {
#if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
      [[maybe_unused]] uint32_t* c = reinterpret_cast<uint32_t*>(&frag_c);
      asm volatile(
      ...
      [[maybe_unused]] int32_t* c = reinterpret_cast<int32_t*>(&frag_c);
      asm volatile(
...
__device__ inline void mma_trans(
  ...
  [[maybe_unused]] const uint32_t* b =
      reinterpret_cast<const uint32_t*>(&frag_b);
  [[maybe_unused]] const uint32_t* b2 =
      reinterpret_cast<const uint32_t*>(&frag_b2);
  [[maybe_unused]] float* c = reinterpret_cast<float*>(&frag_c);
  using scalar_t = typename MarlinScalarType<type_id>::scalar_t;
  if constexpr (!std::is_same<scalar_t, half>::value || k_size != 16) {
    static_assert(!use_fp16_accum);
