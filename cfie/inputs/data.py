...
class _PromptOptions(TypedDict):
    ...
    [`SingletonPrompt`][cfie.inputs.data.SingletonPrompt].
    """
...
- A text prompt (string or [`TextPrompt`][cfie.inputs.data.TextPrompt])
- A tokenized prompt (list of token IDs, or
  [`TokensPrompt`][cfie.inputs.data.TokensPrompt])
- An embeddings prompt ([`EmbedsPrompt`][cfie.inputs.data.EmbedsPrompt])
...
[`ExplicitEncoderDecoderPrompt`][cfie.inputs.data.ExplicitEncoderDecoderPrompt].
"""
...
This is the input format accepted by most [`LLM`][cfie.entrypoints.llm.LLM] APIs.
"""
...
class DataPrompt(_PromptOptions):
    ...
    [`PromptType`][cfie.inputs.data.PromptType] by IO processor plugins.
    """
...
def token_inputs(
    ...
    *,
    ...
    """Construct [`TokenInputs`][cfie.inputs.data.TokenInputs] from optional
    values."""
...
def embeds_inputs(
    ...
    *,
    ...
    """Construct [`EmbedsInputs`][cfie.inputs.data.EmbedsInputs] from optional
    values."""
...
[`InputPreprocessor`][cfie.inputs.preprocess.InputPreprocessor]
which can be passed to
[`InputProcessor`][cfie.v1.engine.input_processor.InputProcessor]
...
def build_enc_dec_inputs(
    ...
    if enc_inputs["type"] == "multimodal":
        from cfie.multimodal.inputs import mm_inputs
