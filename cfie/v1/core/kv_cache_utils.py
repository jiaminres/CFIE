...
from cfie.utils.hashing import sha256_cbor, xxhash_cbor
...
def _check_enough_kv_cache_memory(
    ...
    if needed_memory > available_memory:
        ...
        raise ValueError(
            ...
            f"See https://docs.cfie.ai/en/latest/configuration/conserving_memory/ "
            f"for more details."
        )


def max_memory_usage_bytes(
    cfie_config: CfieConfig, kv_cache_specs: Iterable[KVCacheSpec]
) -> int:
    """
    Get the maximum memory usage in bytes for the given KV cache specs.
    """
    return sum(spec.max_memory_usage_bytes(cfie_config) for spec in kv_cache_specs)
...
def estimate_max_model_len(
    ...
    original_max_model_len = cfie_config.model_config.max_model_len

    # Define a function to check if a given model length fits in memory
    def fits_in_memory(model_len: int) -> bool:
        # Temporarily modify the max_model_len for this calculation
        cfie_config.model_config.max_model_len = model_len
        # Calculate memory needed for the given model length
        memory_needed = max_memory_usage_bytes(cfie_config, kv_cache_spec.values())
    ...
    try:
        ...
        cfie_config.model_config.max_model_len = original_max_model_len
...
def check_enough_kv_cache_memory(
    ...
    if kv_cache_spec:
        _check_enough_kv_cache_memory(
            available_memory,
            lambda: max_memory_usage_bytes(cfie_config, kv_cache_spec.values()),
            cfie_config.model_config.max_model_len,
            lambda am: estimate_max_model_len(cfie_config, kv_cache_spec, am),
        )
...
def get_max_concurrency_for_kv_cache_config(
    cfie_config: CfieConfig, kv_cache_config: KVCacheConfig
) -> float:
    """
    ...
    max_memory_usage_per_request = num_layer_per_group * max_memory_usage_bytes(
        cfie_config, (group.kv_cache_spec for group in kv_cache_config.kv_cache_groups)
    )
    memory_per_block = (
...
def may_override_num_blocks(cfie_config: CfieConfig, num_blocks: int) -> int:
    """
    Override the number of kv cache blocks if `num_gpu_blocks_override` is set.
    """
    if cfie_config.cache_config.num_gpu_blocks_override is not None:
        num_gpu_blocks_override = cfie_config.cache_config.num_gpu_blocks_override
        logger.info(
...
def get_kv_cache_groups(
    cfie_config: CfieConfig, kv_cache_spec: dict[str, KVCacheSpec]
) -> list[KVCacheGroupSpec]:
    """
    ...
    """
    if cfie_config.scheduler_config.disable_hybrid_kv_cache_manager:
...
def _report_kv_cache_config(
    ...
    dcp_size = cfie_config.parallel_config.decode_context_parallel_size
    pcp_size = cfie_config.parallel_config.prefill_context_parallel_size
    ...
    logger.info_once(
        "Approximate maximum concurrency at configured max_model_len=%s: %.2fx",
        max_model_len_str,
        max_concurrency,
        scope="local",
    )
...
def _max_memory_usage_bytes_from_groups(
    ...
    if len(kv_cache_groups) == 1 and isinstance(
        ...
        per_layer_specs = kv_cache_groups[0].kv_cache_spec.kv_cache_specs
        return sum(
            spec.max_memory_usage_bytes(cfie_config)
...
def _estimate_max_model_len_from_groups(
    ...
    """
    original_max = cfie_config.model_config.max_model_len

    def fits(model_len: int) -> bool:
        cfie_config.model_config.max_model_len = model_len
        return (
            _max_memory_usage_bytes_from_groups(cfie_config, kv_cache_groups)
            <= available_memory
        )
    ...
    try:
        ...
        cfie_config.model_config.max_model_len = original_max
...
def _auto_fit_max_model_len(
    ...
    Args:
        cfie_config: The global CfieConfig (will be modified in-place)
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
        if not groups:
            continue
        worker_max = _estimate_max_model_len_from_groups(cfie_config, groups, avail_mem)
    ...
    if auto_fit_max >= original_max:
        ...
        cfie_config.model_config.max_model_len = auto_fit_max
        logger.info_once(
...
def get_kv_cache_configs(
    ...
    for groups, avail_mem in zip(projected_groups_per_worker, available_memory):
        ...
        _check_enough_kv_cache_memory(
            avail_mem,
            partial(_max_memory_usage_bytes_from_groups, cfie_config, groups),
            cfie_config.model_config.max_model_len,
    ...
    for projected_groups, kv_cache_spec_one_worker, available_memory_one_worker in zip(
        ...
        kv_cache_configs.append(
            get_kv_cache_config_from_groups(
                cfie_config, projected_groups, available_memory_one_worker
            )
        )
    ...
    for kv_cache_config in kv_cache_configs:
        ...
        if len(kv_cache_config.kv_cache_groups) > 0:
            _report_kv_cache_config(cfie_config, kv_cache_config)
