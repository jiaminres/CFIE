...
from cfie.config import CUDAGraphMode, CfieConfig, get_layers_from_cfie_config
from cfie.distributed.kv_transfer import has_kv_transfer_group
...
from cfie.v1.attention.backend import AttentionMetadataBuilder, CommonAttentionMetadata
from cfie.v1.cudagraph_dispatcher import CudagraphDispatcher
from cfie.v1.outputs import KVConnectorOutput
...
class ExtractHiddenStatesProposer:
    def __init__(self, cfie_config: CfieConfig, device):
        assert cfie_config.speculative_config is not None

        assert cfie_config.speculative_config.num_speculative_tokens == 1
        if cfie_config.speculative_config.disable_padded_drafter_batch:
            raise ValueError(
        ...
        self.max_num_tokens = (
            cfie_config.scheduler_config.max_num_batched_tokens + max_batch_size
        )

        self.hf_config = cfie_config.speculative_config.draft_model_config.hf_config
        ...
        self.hidden_size = cfie_config.model_config.get_hidden_size()
    ...
    def initialize_cudagraph_keys(self, cudagraph_mode: CUDAGraphMode) -> None:
        ...
        """
        assert self.cfie_config.speculative_config is not None
        if (
            not self.cfie_config.speculative_config.enforce_eager
    ...
    def _build_attn_metadata_builder(
        ...
        return attn_backend.get_builder_cls()(
            layer.get_kv_cache_spec(self.cfie_config),
            self.attn_layer_names,
            self.cfie_config,
            self.device,
        )
    ...
    def load_model(self, target_model: nn.Module) -> None:
        ...
        target_attn_layer_names = set(
            get_layers_from_cfie_config(self.cfie_config, AttentionLayerBase).keys()  # type: ignore[type-abstract]
        )

        assert self.cfie_config.speculative_config is not None
        draft_model_config = self.cfie_config.speculative_config.draft_model_config
        from cfie.compilation.backends import set_model_tag

        with set_model_tag("extract_hidden_states"):
            self.model = get_model(
                cfie_config=self.cfie_config, model_config=draft_model_config
            )
