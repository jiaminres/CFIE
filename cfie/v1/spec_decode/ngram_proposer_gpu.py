...
class NgramGPUKernel(nn.Module):
    ...
    def __init__(
        self, cfie_config: CfieConfig, prefix: str = "", device: torch.device = "cuda"
    ):
        super().__init__()

        assert cfie_config.speculative_config is not None
        assert cfie_config.speculative_config.prompt_lookup_min is not None
        assert cfie_config.speculative_config.prompt_lookup_max is not None

        self.min_n = cfie_config.speculative_config.prompt_lookup_min
        self.max_n = cfie_config.speculative_config.prompt_lookup_max
        self.k = cfie_config.speculative_config.num_speculative_tokens
        self.max_model_len = cfie_config.model_config.max_model_len
        self.max_num_seqs = cfie_config.scheduler_config.max_num_seqs
        self.device = device
...
class NgramProposerGPU:
    def __init__(self, cfie_config: CfieConfig, device: torch.device, runner=None):
        assert cfie_config.speculative_config is not None
        assert cfie_config.speculative_config.prompt_lookup_min is not None
        assert cfie_config.speculative_config.prompt_lookup_max is not None
        ...
        self.kernel = NgramGPUKernel(
            cfie_config=self.cfie_config, prefix="ngram_gpu_kernel", device=device
        )
