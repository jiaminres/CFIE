...
from cfie.model_executor.models import supports_multimodal
...
from cfie.v1.attention.backends.tree_attn import (
    TreeAttentionMetadata,
    TreeAttentionMetadataBuilder,
)
from cfie.v1.attention.backends.triton_attn import TritonAttentionMetadata
from cfie.v1.cudagraph_dispatcher import CudagraphDispatcher
from cfie.v1.kv_cache_interface import KVCacheConfig, UniformTypeKVCacheSpecs
from cfie.v1.sample.metadata import SamplingMetadata
from cfie.v1.sample.sampler import _SAMPLING_EPS
...
class SpecDecodeBaseProposer:
    def __init__(
        ...
        max_batch_size = cfie_config.scheduler_config.max_num_seqs
        ...
        self.compilation_config = self.cfie_config.compilation_config
        ...
        self.uses_xdrope_dim = self.cfie_config.model_config.uses_xdrope_dim
        ...
        if current_platform.is_rocm():
            from cfie.v1.attention.backends.mla.rocm_aiter_mla_sparse import (
                ROCMAiterMLASparseMetadata,
            )
            from cfie.v1.attention.backends.rocm_attn import RocmAttentionMetadata
            ...
            if find_spec(
                ...
                from cfie.v1.attention.backends.rocm_aiter_fa import (
            ...
            from cfie.v1.attention.backends.flex_attention import FlexAttentionMetadata
    ...
    def _set_positions(self, num_tokens: int, positions: torch.Tensor):
        if self.uses_mrope:
            ...
            if self.cfie_config.model_config.uses_mrope:
    ...
    def propose(
        ...
        if self.method == "eagle3":
            from cfie.model_executor.models.deepseek_eagle3 import (
                Eagle3DeepseekV2ForCausalLM,
            )
            from cfie.model_executor.models.llama_eagle3 import (
    ...
    def _get_model(self) -> nn.Module:
        ...
        load_cfie_config = self.cfie_config
        load_model_config = self.speculative_config.draft_model_config
        load_config = self.speculative_config.draft_load_config
        if self.method == "mtp":
            # ----------------- MTP 模型创建入口 -----------------
            # 对 mtp 来说，这里才是真正为 draft/MTP 模型准备“实际加载用”的独立 CfieConfig。
            # 这份配置不会复用 target 的 plan，而是会在 __post_init__ 中按 draft 视角重建 actual plan。
            load_cfie_config = create_vllm_config_for_draft_model(self.cfie_config)
            load_model_config = load_cfie_config.model_config
            load_config = load_cfie_config.load_config
        # 到这里为止，load_cfie_config 已经对应“这次真正要加载的 drafter”那份配置视角。
        draft_plan = get_moe_tiered_cache_plan(load_cfie_config)
        logger.info(
            "Loading speculative drafter: proposer=%s method=%s model_type=%s "
            "plan_model_type=%s gpu_slots/layer=%s prefill_burst_slots=%s "
            "cpu_slots/layer=%s",
            self.__class__.__name__,
            self.method,
            getattr(getattr(load_model_config, "hf_config", None), "model_type", ""),
            draft_plan.get("model_type") if isinstance(draft_plan, dict) else "",
            draft_plan.get("gpu_slots_per_layer") if isinstance(draft_plan, dict) else None,
            draft_plan.get("prefill_burst_slots") if isinstance(draft_plan, dict) else None,
            draft_plan.get("cpu_slots_per_layer") if isinstance(draft_plan, dict) else None,
        )
        with set_model_tag("eagle_head"):
            model = get_model(
                cfie_config=load_cfie_config,
    ...
    def load_model(self, target_model: nn.Module) -> None:
        # ----------------- speculative drafter 正式加载 -----------------
        # 先记录 target 模型原本有哪些 attention 层，后面用来识别 draft 额外引入的层。
        ...
        # _get_model() 内部会在 mtp 场景派生 draft CfieConfig，并真正创建 MTP 模型。
        self.model = self._get_model()

        # 找出 draft 相比 target 额外增加的 attention 层，供后续推理路径单独处理。
        all_attn_layers = get_layers_from_cfie_config(
            self.cfie_config,
            AttentionLayerBase,  # type: ignore[type-abstract]
        )
