...
from cfie.distributed.kv_transfer.kv_connector.utils import KVOutputAggregator
...
from cfie.v1.engine import ReconfigureDistributedRequest
from cfie.v1.kv_cache_interface import KVCacheConfig, KVCacheSpec
from cfie.v1.outputs import DraftTokenIds, ModelRunnerOutput
from cfie.v1.worker.worker_base import WorkerBase

if TYPE_CHECKING:
    from cfie.distributed.kv_transfer.kv_connector.base import KVConnectorBase
...
class Executor(ABC):
    ...
    执行器负责在单个设备上执行模型；也可以作为分布式执行器，
    在多个设备上协同执行模型。
    """

    uses_ray: bool = False  # 当前执行器是否使用 Ray 来编排 worker。
    supports_pp: bool = False  # 当前执行器是否支持 pipeline parallel。

    @staticmethod
    # 根据 `CfieConfig` 选择实际使用的执行器实现类。
    def get_class(cfie_config: CfieConfig) -> type["Executor"]:
        # 最终要返回的执行器实现类。
        executor_class: type[Executor]

        # 取出并行配置，后续按其中的 backend 决定具体实现。
        parallel_config = cfie_config.parallel_config

        # 分布式执行后端应当已经在 CfieConfig.__post_init__ 中补齐。
        distributed_executor_backend = parallel_config.distributed_executor_backend

        # 如果用户直接传入了一个执行器类，就先校验它是不是 Executor 子类。
        if isinstance(distributed_executor_backend, type):
            ...
            # Ray 后端对应 RayDistributedExecutor。
            from cfie.v1.executor.ray_executor import RayDistributedExecutor

            executor_class = RayDistributedExecutor
        elif distributed_executor_backend == "mp":
            # 多进程后端对应 MultiprocExecutor。
            from cfie.v1.executor.multiproc_executor import MultiprocExecutor

            executor_class = MultiprocExecutor
        elif distributed_executor_backend == "uni":
            # 单进程后端对应 UniProcExecutor。
            from cfie.v1.executor.uniproc_executor import UniProcExecutor

            executor_class = UniProcExecutor
        elif distributed_executor_backend == "external_launcher":
            # TODO: make v1 scheduling deterministic
            # to support external launcher
            # 外部启动器模式交给 ExecutorWithExternalLauncher 处理。
            executor_class = ExecutorWithExternalLauncher
        elif isinstance(distributed_executor_backend, str):
            # 其他字符串形式按全限定类名动态解析。
            executor_class = resolve_obj_by_qualname(distributed_executor_backend)
            # 动态解析出来的对象同样必须是 Executor 子类。
    ...
    # 初始化执行器公共状态，并在子类中创建实际 worker/进程拓扑。
    def __init__(
        self,
        ...
        self.device_config = cfie_config.device_config
    ...
    # 让所有 worker 按最终 KV cache 配置初始化缓存并完成 warmup。
    def initialize_from_config(self, kv_cache_configs: list[KVCacheConfig]) -> None:
        ...
        if compilation_times:
            self.cfie_config.compilation_config.compilation_time = max(
                compilation_times
            )
    ...
    # 触发 worker 做显存 profiling，并回传可用于 KV cache 的预算。
    def determine_available_memory(self) -> list[int]:  # in bytes
        return self.collective_rpc("determine_available_memory")

    # 收集每个 worker 暴露的 KV cache 规格，供 engine core 统一规划缓存。
...
from cfie.v1.executor.uniproc_executor import (  # noqa: E402
