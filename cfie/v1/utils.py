...
from cfie.usage.usage_lib import UsageContext, is_usage_stats_enabled, usage_message
from cfie.utils.network_utils import get_open_port, get_open_zmq_ipc_path, get_tcp_uri
from cfie.utils.system_utils import kill_process_tree
...
if TYPE_CHECKING:
    ...
    from cfie.v1.engine.utils import CoreEngineActorManager, CoreEngineProcManager
...
def report_usage_stats(
    cfie_config, usage_context: UsageContext = UsageContext.ENGINE_CONTEXT
) -> None:
    ...
    from cfie.model_executor.model_loader import get_architecture_class_name

    parallel_config = cfie_config.parallel_config

    # Prepare KV connector string if applicable
    kv_connector = None
    if cfie_config.kv_transfer_config is not None:
        kv_connector = cfie_config.kv_transfer_config.kv_connector

    usage_message.report_usage(
        get_architecture_class_name(cfie_config.model_config),
        usage_context,
        extra_kvs={
            # Common configuration
            "dtype": str(cfie_config.model_config.dtype),
            "block_size": cfie_config.cache_config.block_size,
            "gpu_memory_utilization": cfie_config.cache_config.gpu_memory_utilization,
            "kv_cache_memory_bytes": cfie_config.cache_config.kv_cache_memory_bytes,
            # Quantization
            "quantization": cfie_config.model_config.quantization,
            "kv_cache_dtype": str(cfie_config.cache_config.cache_dtype),
            # Feature flags
            "enable_lora": bool(cfie_config.lora_config),
            "enable_prefix_caching": cfie_config.cache_config.enable_prefix_caching,
            "enforce_eager": cfie_config.model_config.enforce_eager,
...
def tensor_data(tensor: torch.Tensor) -> memoryview:
    # 以 uint8 的 memoryview 形式返回 tensor 的原始数据，
    # 便于做序列化与哈希计算。
    """将 tensor 的原始数据提取为 uint8 类型的 memoryview，
    常用于序列化与哈希计算。
    ...
    # 返回值为基于 uint8 视图的 memoryview。
    Returns:
        tensor 数据对应的 uint8 memoryview。
    """

    # 先展平张量，再转为连续内存布局，
    # 然后按 uint8 重新解释底层字节数据，
    # 最后转成 numpy 并取其底层 buffer。
