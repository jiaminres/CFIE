...
from cfie.logits_process import LogitsProcessor as RequestLogitsProcessor
from cfie.sampling_params import SamplingParams
from cfie.utils.torch_utils import guard_cuda_initialization
from cfie.v1.sample.logits_processor.builtin import (
    LogitBiasLogitsProcessor,
    MinPLogitsProcessor,
    MinTokensLogitsProcessor,
    process_dict_updates,
)
from cfie.v1.sample.logits_processor.interface import (
    BatchUpdate,
    LogitsProcessor,
    MoveDirectionality,
)
from cfie.v1.sample.logits_processor.state import BatchUpdateBuilder, LogitsProcessors
...
LOGITSPROCS_GROUP = "cfie.logits_processors"
...
def build_logitsprocs(
    ...
    if cfie_config.speculative_config:
        ...
        return LogitsProcessors(
            [MinTokensLogitsProcessor(cfie_config, device, is_pin_memory)]
        )

    custom_logitsprocs_classes = _load_custom_logitsprocs(custom_logitsprocs)
    return LogitsProcessors(
        ctor(cfie_config, device, is_pin_memory)
...
class AdapterLogitsProcessor(LogitsProcessor):
    ...
    * Implement `self.new_req_logits_processor(params)`
    ...
    `self.__init__(cfie_config, device, is_pin_memory)` does not need to be
    overridden in general. However, to implement custom constructor behavior -
    especially any logic which operates on or stores `cfie_config`, `device`,
    or `is_pin_memory` - `self.__init__(cfie_config, device, is_pin_memory)`
    must be overridden and the override must call
    `super().__init__(cfie_config, device, is_pin_memory)`
    """

    def __init__(
        self, cfie_config: "CfieConfig", device: torch.device, is_pin_memory: bool
    ):
        """Subclass must invoke
        `super().__init__(cfie_config, device, is_pin_memory)`.

        Subclass constructor may find it useful to utilize the `cfie_config`,
