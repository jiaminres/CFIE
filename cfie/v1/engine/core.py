...
from cfie.envs import enable_envs_cache
from cfie.logger import init_logger
from cfie.logging_utils.dump_input import dump_engine_exception
from cfie.lora.request import LoRARequest
from cfie.multimodal import MULTIMODAL_REGISTRY
from cfie.tasks import POOLING_TASKS, SupportedTask
from cfie.tracing import instrument, maybe_init_worker_tracer
from cfie.transformers_utils.config import maybe_register_config_serialize_by_value
from cfie.utils.gc_utils import (
    freeze_gc_heap,
    maybe_attach_gc_debug_callback,
)
from cfie.utils.hashing import get_hash_fn_by_name
from cfie.utils.network_utils import make_zmq_socket
from cfie.utils.system_utils import decorate_logs, set_process_title
...
from cfie.v1.metrics.stats import SchedulerStats
from cfie.v1.outputs import ModelRunnerOutput
from cfie.v1.request import Request, RequestStatus
from cfie.v1.serial_utils import MsgpackDecoder, MsgpackEncoder
from cfie.v1.structured_output import StructuredOutputManager
from cfie.v1.utils import compute_iteration_details
from cfie.version import __version__ as VLLM_VERSION
...
class EngineCore:
    ...
    def __init__(
        ...
        # 调度层和执行层都可能依赖插件能力，因此这里先统一加载。
        from cfie.plugins import load_general_plugins

        # 先加载通用插件，确保调度层和执行层都能看到扩展能力。
        load_general_plugins()

        # 保存引擎全局配置。
        self.cfie_config = cfie_config
        # 仅在主 rank 打印初始化日志，避免重复刷屏。
        if not cfie_config.parallel_config.data_parallel_rank_local:
            logger.info(
        ...
        # 创建底层 model executor。
        self.model_executor = executor_class(cfie_config)
        # 若上层提供了失败回调，则注册到 executor。
        if executor_fail_callback is not None:
            self.model_executor.register_failure_callback(executor_fail_callback)

        # 先把可用于 KV cache 的显存标记为未知。
        self.available_gpu_memory_for_kv_cache = -1

        # Elastic EP 扩容场景下，要先完成扩容前置动作。
        if envs.VLLM_ELASTIC_EP_SCALE_UP_LAUNCH:
            self._eep_scale_up_before_kv_init()

        # Setup KV Caches and update CacheConfig after profiling.
        # 结合 profiling 结果初始化 KV cache。
        kv_cache_config = self._initialize_kv_caches(cfie_config)
        # 创建结构化输出管理器。
        self.structured_output_manager = StructuredOutputManager(cfie_config)

        # Setup scheduler.
        # 根据配置选择具体调度器实现。
        Scheduler = cfie_config.scheduler_config.get_scheduler_cls()

        if len(kv_cache_config.kv_cache_groups) == 0:  # noqa: SIM102
            # Encoder models without KV cache don't support
            # chunked prefill. But do SSM models?
            if cfie_config.scheduler_config.enable_chunked_prefill:
                logger.warning("Disabling chunked prefill for model without KVCache")
                cfie_config.scheduler_config.enable_chunked_prefill = False

        # 计算调度器视角的实际 block size。
        scheduler_block_size = (
            cfie_config.cache_config.block_size
            * cfie_config.parallel_config.decode_context_parallel_size
            * cfie_config.parallel_config.prefill_context_parallel_size
        )

        # 创建调度器并注入 KV cache 与结构化输出依赖。
        ...
        # 标记当前是否启用了 speculative decoding。
        self.use_spec_decode = cfie_config.speculative_config is not None
        # 若调度器启用了 KV connector，则把聚合器挂到 executor。
        ...
        # 取出 scheduler 上的 KV connector。
        kv_connector = self.scheduler.get_kv_connector()
        if kv_connector is not None:
            # Collect and store KV connector xfer metadata from workers
            # (after KV cache registration)
            # 从 worker 收集 KV connector 握手元数据。
        ...
        # 标记当前是否为 EC consumer。
        self.is_ec_consumer = (
            cfie_config.ec_transfer_config is None
            or cfie_config.ec_transfer_config.is_ec_consumer
        )
        # 标记当前模型是否为 pooling 类型。
        self.is_pooling_model = cfie_config.model_config.runner_type == "pooling"

        # 默认不启用 request block hasher。
        self.request_block_hasher: Callable[[Request], list[BlockHash]] | None = None
        if cfie_config.cache_config.enable_prefix_caching or kv_connector is not None:
            # 解析 prefix caching 使用的哈希函数。
            caching_hash_fn = get_hash_fn_by_name(
                cfie_config.cache_config.prefix_caching_hash_algo
            )
            # 初始化对 None 输入友好的哈希封装。
            init_none_hash(caching_hash_fn)

            # 基于调度 block size 构造请求块哈希器。
            self.request_block_hasher = get_request_block_hasher(
                scheduler_block_size, caching_hash_fn
            )

        # 根据是否启用批队列决定 step 入口函数。
        self.step_fn = (
            self.step if self.batch_queue is None else self.step_with_batch_queue
        )
        # 记录调度器是否开启异步调度。
        self.async_scheduling = cfie_config.scheduler_config.async_scheduling
    ...
    def _initialize_kv_caches(self, cfie_config: CfieConfig) -> KVCacheConfig:
        ...
        # 向 executor 查询模型所有层所需的 KV cache 规格。
        kv_cache_specs = self.model_executor.get_kv_cache_specs()

        # 判断当前模型是否真的需要 KV cache。
        ...
        if has_kv_cache:
            if envs.VLLM_ELASTIC_EP_SCALE_UP_LAUNCH:
                ...
                # 正常场景下先做显存 profiling，得到 KV cache 可用预算。
                available_gpu_memory = self.model_executor.determine_available_memory()
                # 缓存第一张卡的可用显存，供后续逻辑复用。
                self.available_gpu_memory_for_kv_cache = available_gpu_memory[0]
        else:
            # Attention free models don't need memory for kv cache
            # 无注意力模型不需要给 KV cache 预留显存。
            available_gpu_memory = [0] * len(kv_cache_specs)

        # executor 返回的可用显存数量必须与 KV cache 规格数量一致。
        assert len(kv_cache_specs) == len(available_gpu_memory)

        # Track max_model_len before KV cache config to detect auto-fit changes
        # 记录 KV cache 规划前的 max_model_len。
        max_model_len_before = cfie_config.model_config.max_model_len

        # 基于可用显存与规格生成每个 worker 的 KV cache 配置。
        kv_cache_configs = get_kv_cache_configs(
            cfie_config, kv_cache_specs, available_gpu_memory
        )

        # If auto-fit reduced max_model_len, sync the new value to workers.
        # This is needed because workers were spawned before memory profiling
        # and have the original (larger) max_model_len cached.
        # 检查 auto-fit 后是否压缩了 max_model_len。
        max_model_len_after = cfie_config.model_config.max_model_len
        if max_model_len_after != max_model_len_before:
            # 若发生变化，则广播给 worker 同步更新。
            self.collective_rpc("update_max_model_len", args=(max_model_len_after,))

        # 把多 worker KV cache 配置汇总成 scheduler 可用配置。
        scheduler_kv_cache_config = generate_scheduler_kv_cache_config(kv_cache_configs)
        # 把最终 GPU blocks 数回写到 CacheConfig。
        cfie_config.cache_config.num_gpu_blocks = scheduler_kv_cache_config.num_blocks
        # 取出汇总后的 KV cache group 集合。
        kv_cache_groups = scheduler_kv_cache_config.kv_cache_groups
        if kv_cache_groups:
            # 以所有 group 的最小 block_size 作为全局 block_size。
            cfie_config.cache_config.block_size = min(
                g.kv_cache_spec.block_size for g in kv_cache_groups
            )

        # 校验 block size 配置是否合法。
        cfie_config.validate_block_size()

        # Initialize kv cache and warmup the execution
        # 通知 executor 真正创建 KV cache 并做 warmup。
    ...
    def log_error_detail(self, scheduler_output: SchedulerOutput):
        ...
        try:
            ...
            dump_engine_exception(
                self.cfie_config, scheduler_output, self.scheduler.make_stats()
            )
            raise err

    @contextmanager
    def log_iteration_details(self, scheduler_output: SchedulerOutput):
        if not self.cfie_config.observability_config.enable_logging_iteration_details:
            yield
            return
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        # ----------------- 一轮标准执行：调度 -> 执行 -> 回写 -----------------
        # 只要 scheduler 里还有未完成请求，或还有待清理的 finished 请求，就继续推进。
        # 若调度器里已经没有请求，则这一轮不执行模型。
        ...
        # 预先生成语法约束 bitmask，供后续 sample 使用。
        ...
        with (
            ...
            if model_output is None:
                # 若 executor 只完成前向，则在这里补做采样。
                model_output = self.model_executor.sample_tokens(grammar_output)

        # 先处理执行期间积累的 abort，避免调度状态落后。
        self._process_aborts_queue()
        # 用模型输出更新调度器内部状态并生成对外输出。
        engine_core_outputs = self.scheduler.update_from_output(
            scheduler_output, model_output
        )

        # 返回本轮输出以及“是否真正执行了模型”的标记。
    ...
    def _eep_send_engine_core_notification(
        ...
        cfie_config: CfieConfig | None = None,
    ):
        raise NotImplementedError
...
class EngineCoreProc(EngineCore):
        ...
        *,
        ...
        with self._perform_handshakes(
            ...
            internal_dp_balancing = (
                self.has_coordinator
                and not cfie_config.parallel_config.data_parallel_external_lb
            )
            ...
            self._init_data_parallel(cfie_config)
    ...
    def _perform_handshakes(
        ...
        if client_handshake_address is None:
            ...
            local_handshake = self._perform_handshake(
                input_ctx, client_handshake_address, identity, True, False, cfie_config
            )
    ...
    def _perform_handshake(
        ...
        with make_zmq_socket(
            ...
            # Send ready message.
            num_gpu_blocks = cfie_config.cache_config.num_gpu_blocks
            ...
            if cfie_config.parallel_config.data_parallel_size > 1:
                ready_msg["parallel_config_hash"] = (
                    cfie_config.parallel_config.compute_hash()
                )
    ...
    def run_engine_core(*args, dp_rank: int = 0, local_dp_rank: int = 0, **kwargs):
        ...
        try:
            cfie_config: CfieConfig = kwargs["cfie_config"]
            parallel_config: ParallelConfig = cfie_config.parallel_config
            ...
            maybe_init_worker_tracer("cfie.engine_core", "engine_core", process_title)
            decorate_logs()

            if data_parallel and cfie_config.kv_transfer_config is not None:
                # modify the engine_id and append the local_dp_rank to it to ensure
                # that the kv_transfer_config is unique for each DP rank.
                cfie_config.kv_transfer_config.engine_id = (
                    f"{cfie_config.kv_transfer_config.engine_id}_dp{local_dp_rank}"
                )
                logger.debug(
            ...
            if data_parallel and cfie_config.model_config.is_moe:
    ...
    def _init_data_parallel(self, cfie_config: CfieConfig):
        pass
    ...
    def _handle_shutdown(self) -> bool:
        ...
        if self.shutdown_state == EngineShutdownState.REQUESTED:
            shutdown_timeout = self.cfie_config.shutdown_timeout
...
class DPEngineCoreProc(EngineCoreProc):
    ...
    def __init__(
        ...
        assert cfie_config.model_config.is_moe, (
        ...
        from cfie.distributed.elastic_ep.elastic_state import ElasticEPScalingState

        self.eep_scaling_state: ElasticEPScalingState | None = None

        # Initialize the engine.
        dp_rank = cfie_config.parallel_config.data_parallel_rank
        super().__init__(
            cfie_config,
            local_client,
            handshake_address,
    ...
    def reinitialize_distributed(
        ...
        new_parallel_config = deepcopy(self.cfie_config.parallel_config)
    ...
    def _eep_send_engine_core_notification(
        ...
        """
        if cfie_config is None:
            dp_rank = self.cfie_config.parallel_config.data_parallel_rank
        else:
            dp_rank = cfie_config.parallel_config.data_parallel_rank
    ...
    def _eep_scale_up_before_kv_init(self):
        ...
        self.eep_scaling_state = ElasticEPScalingState(
            ...
            engine_core=self,
            cfie_config=self.cfie_config,
            new_parallel_config=self.cfie_config.parallel_config,
            worker_type="new",
            scale_type="scale_up",
...
class EngineCoreActorMixin:
    ...
    def __init__(
        ...
        maybe_init_worker_tracer(
            instrumenting_module_name="cfie.engine_core",
            process_kind="engine_core",
            process_name=f"DPEngineCoreActor_DP{dp_rank}",
        )

        self.addresses = addresses
        cfie_config.parallel_config.data_parallel_index = dp_rank
        cfie_config.parallel_config.data_parallel_rank_local = local_dp_rank
        ...
        # of ray.
        self._set_visible_devices(cfie_config, local_dp_rank)

    def _set_visible_devices(self, cfie_config: CfieConfig, local_dp_rank: int):
        ...
        if current_platform.is_xpu():
            ...
            self._set_cuda_visible_devices(
                cfie_config, local_dp_rank, device_control_env_var
            )

    def _set_cuda_visible_devices(
        self, cfie_config: CfieConfig, local_dp_rank: int, device_control_env_var: str
    ):
...
class DPMoEEngineCoreActor(EngineCoreActorMixin, DPEngineCoreProc):
    ...
    def __init__(
        ...
        cfie_config.parallel_config.data_parallel_rank = dp_rank

        EngineCoreActorMixin.__init__(
            self, cfie_config, addresses, dp_rank, local_dp_rank
        )
        DPEngineCoreProc.__init__(
            self, cfie_config, local_client, "", executor_class, log_stats
        )
...
class EngineCoreActor(EngineCoreActorMixin, EngineCoreProc):
    ...
    def __init__(
        ...
        cfie_config.parallel_config.data_parallel_size = 1
        cfie_config.parallel_config.data_parallel_size_local = 1
        cfie_config.parallel_config.data_parallel_rank = 0
