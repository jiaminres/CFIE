...
from cfie.v1.attention.backend import AttentionBackend, CommonAttentionMetadata
...
from cfie.v1.worker.utils import AttentionGroup, bind_kv_cache


def get_kv_cache_spec(cfie_config: CfieConfig) -> dict[str, KVCacheSpec]:
    kv_cache_spec: dict[str, KVCacheSpec] = {}
    layer_type = cast(type[Any], AttentionLayerBase)
    attn_layers = get_layers_from_cfie_config(cfie_config, layer_type)
    for layer_name, attn_module in attn_layers.items():
        # Skip modules that don't need KV cache (eg encoder-only attention)
        if spec := attn_module.get_kv_cache_spec(cfie_config):
            kv_cache_spec[layer_name] = spec
    return kv_cache_spec


def init_attn_backend(
    kv_cache_config: KVCacheConfig, cfie_config: CfieConfig, device: torch.device
):
    ...
    for kv_cache_group_id, kv_cache_group_spec in enumerate(
        ...
        layer_type = cast(type[Any], AttentionLayerBase)
        attn_layers = get_layers_from_cfie_config(cfie_config, layer_type, layer_names)
