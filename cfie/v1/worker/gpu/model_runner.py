...
from cfie.utils.torch_utils import STR_DTYPE_TO_TORCH_DTYPE
...
from cfie.v1.worker.cp_utils import check_attention_cp_compatibility
from cfie.v1.worker.gpu.async_utils import AsyncOutput, AsyncPoolingOutput
...
from cfie.v1.worker.gpu.block_table import BlockTables
from cfie.v1.worker.gpu.buffer_utils import async_copy_to_gpu
from cfie.v1.worker.gpu.cp_utils import prepare_dcp_local_seq_lens
from cfie.v1.worker.gpu.cudagraph_utils import (
    BatchExecutionDescriptor,
    ModelCudaGraphManager,
    get_uniform_token_count,
)
from cfie.v1.worker.gpu.dp_utils import sync_cudagraph_and_dp_padding
from cfie.v1.worker.gpu.input_batch import (
    InputBatch,
    InputBuffers,
...
from cfie.v1.worker.gpu.kv_connector import (
    NO_OP_KV_CONNECTOR,
    KVConnector,
    get_kv_connector,
)
from cfie.v1.worker.gpu.lora_utils import LoraState
from cfie.v1.worker.gpu.mm.encoder_cache import EncoderCache
from cfie.v1.worker.gpu.model_states import init_model_state
from cfie.v1.worker.gpu.pool.pooling_runner import PoolingRunner
from cfie.v1.worker.gpu.pp_utils import pp_broadcast, pp_receive
from cfie.v1.worker.gpu.sample.output import SamplerOutput
from cfie.v1.worker.gpu.sample.prompt_logprob import PromptLogprobsWorker
from cfie.v1.worker.gpu.sample.sampler import Sampler
from cfie.v1.worker.gpu.spec_decode import init_speculator
from cfie.v1.worker.gpu.spec_decode.eagle.eagle3_utils import (
    set_eagle3_aux_hidden_state_layers,
)
from cfie.v1.worker.gpu.spec_decode.rejection_sampler import RejectionSampler
from cfie.v1.worker.gpu.spec_decode.utils import DraftTokensHandler
from cfie.v1.worker.gpu.states import RequestState
from cfie.v1.worker.gpu.structured_outputs import StructuredOutputsWorker
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, cfie_config: CfieConfig, device: torch.device):
        ...
        if self.speculative_config is not None:
            ...
            if self.is_last_pp_rank:
                self.speculator = init_speculator(self.cfie_config, self.device)
    ...
    def load_model(self, *args, **kwargs) -> None:
        ...
        with DeviceMemoryProfiler() as m:
            model_loader = get_model_loader(self.cfie_config.load_config)
            logger.info("Loading model from scratch...")

            self.model = model_loader.load_model(
                cfie_config=self.cfie_config,
                model_config=self.cfie_config.model_config,
            )
            if self.lora_config:
        ...
        self.model_state = init_model_state(
            self.cfie_config, self.model, self.encoder_cache, self.device
        )
    ...
    def get_kv_cache_spec(self):
        return get_kv_cache_spec(self.cfie_config)
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        self.attn_backends, self.attn_groups = init_attn_backend(
            self.kv_cache_config, self.cfie_config, self.device
        )
        ...
        self.kv_connector = get_kv_connector(self.cfie_config, kv_caches_dict)
