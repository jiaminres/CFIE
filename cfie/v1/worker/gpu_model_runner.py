...
from cfie.compilation.cuda_graph import CUDAGraphStat, CUDAGraphWrapper
from cfie.compilation.monitor import set_cudagraph_capturing_enabled
...
from cfie.distributed.ec_transfer import get_ec_transfer, has_ec_transfer
from cfie.distributed.eplb.eplb_state import EplbState
from cfie.distributed.kv_transfer import get_kv_transfer_group, has_kv_transfer_group
from cfie.distributed.kv_transfer.kv_connector.utils import copy_kv_blocks
...
from cfie.lora.layers import LoRAMapping, LoRAMappingType
...
from cfie.model_executor.model_loader import get_model_loader
...
from cfie.model_executor.models.interfaces_base import (
    VllmModelForPooling,
    is_pooling_model,
    is_text_generation_model,
)
from cfie.model_executor.offloader import (
    create_offloader,
    get_offloader,
    set_offloader,
)
...
from cfie.multimodal.utils import group_and_batch_mm_kwargs
from cfie.platforms import current_platform
from cfie.pooling_params import PoolingParams
from cfie.sampling_params import SamplingType
from cfie.sequence import IntermediateTensors
from cfie.tasks import GenerationTask, PoolingTask, SupportedTask
from cfie.tracing import instrument
from cfie.utils import length_from_prompt_token_ids_or_embeds
from cfie.utils.math_utils import cdiv, round_up
from cfie.utils.mem_utils import DeviceMemoryProfiler, format_gib
from cfie.utils.nvtx_pytorch_hooks import PytHooks
from cfie.utils.platform_utils import is_pin_memory_available, num_compute_units
...
from cfie.v1.attention.backends.gdn_attn import GDNAttentionMetadataBuilder
from cfie.v1.attention.backends.mamba2_attn import Mamba2AttentionMetadataBuilder
from cfie.v1.attention.backends.utils import (
    create_fast_prefill_custom_backend,
    get_dcp_local_seq_lens,
    reorder_batch_to_split_decodes_and_prefills,
)
from cfie.v1.core.sched.output import NewRequestData
from cfie.v1.cudagraph_dispatcher import CudagraphDispatcher
...
from cfie.v1.pool.metadata import PoolingMetadata, PoolingStates
from cfie.v1.sample.logits_processor import LogitsProcessors, build_logitsprocs
from cfie.v1.sample.logits_processor.interface import LogitsProcessor
from cfie.v1.sample.metadata import SamplingMetadata
from cfie.v1.sample.rejection_sampler import RejectionSampler
from cfie.v1.sample.sampler import Sampler
from cfie.v1.spec_decode.metadata import SpecDecodeMetadata
from cfie.v1.spec_decode.ngram_proposer_gpu import (
    NgramProposerGPU,
    copy_num_valid_draft_tokens,
    update_ngram_gpu_tensors_incremental,
    update_scheduler_for_invalid_drafts,
)
from cfie.v1.spec_decode.suffix_decoding import SuffixDecodingProposer
from cfie.v1.structured_output.utils import apply_grammar_bitmask
from cfie.v1.utils import CpuGpuBuffer, record_function_or_nullcontext
from cfie.v1.worker import mamba_utils
from cfie.v1.worker.cp_utils import (
    check_attention_cp_compatibility,
    get_total_cp_world_size,
)
from cfie.v1.worker.dp_utils import coordinate_batch_across_dp
from cfie.v1.worker.ec_connector_model_runner_mixin import ECConnectorModelRunnerMixin
from cfie.v1.worker.gpu.pool.late_interaction_runner import LateInteractionRunner
from cfie.v1.worker.gpu_input_batch import CachedRequestState, InputBatch
from cfie.v1.worker.gpu_ubatch_wrapper import UBatchWrapper
from cfie.v1.worker.kv_connector_model_runner_mixin import KVConnectorModelRunnerMixin
from cfie.v1.worker.lora_model_runner_mixin import LoRAModelRunnerMixin
from cfie.v1.worker.ubatch_utils import (
    UBatchSlices,
...
class GPUModelRunner(
    ...
    def __init__(
        ...
        self.input_batch = InputBatch(
            ...
            is_spec_decode=bool(self.cfie_config.speculative_config),
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        """
        用 scheduler_output 更新 worker 侧缓存状态与 persistent batch。

        更新后的状态会被 `_prepare_inputs()` 消费，用来构造本轮前向所需的 GPU 输入。
        """
        # ----------------- 先清理 finished / unscheduled 请求 -----------------
        # 先从 worker 本地缓存里移除已经结束的请求。
        ...
        # persistent batch 里也要删掉 finished 请求。
        # 若同一个 req_id 先 abort 再重提，这里把旧请求清掉，后面按新请求重新建态。
        for req_id in scheduler_output.finished_req_ids:
            self.input_batch.remove_request(req_id)

        # 新分配的 block 先清零，避免旧显存残留污染 attention / SSM 计算。
        if scheduler_output.new_block_ids_to_zero:
            self._zero_block_ids(scheduler_output.new_block_ids_to_zero)

        # scheduler 已判定可释放的 encoder 输出，在 worker 侧同步释放。
        for mm_hash in scheduler_output.free_encoder_mm_hashes:
            self.encoder_cache.pop(mm_hash, None)

        # 本轮未被调度到的请求也要先从 persistent batch 里移走。
        # 这些请求可能只是被抢占或暂时没排到，稍后还会再次进入 batch。
        scheduled_req_ids = scheduler_output.num_scheduled_tokens.keys()
        cached_req_ids = self.input_batch.req_id_to_index.keys()
        resumed_req_ids = scheduler_output.scheduled_cached_reqs.resumed_req_ids
        # resumed 请求会在后面的正常恢复路径里重新加入，因此这里先把旧 batch 痕迹清干净。
        unscheduled_req_ids = cached_req_ids - (scheduled_req_ids - resumed_req_ids)
        # persistent batch 的收益建立在“相邻批次请求大体重叠”这个前提上。
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            if req_id in self.requests:
                # 流式输入续写时，沿用已有 req_state 并更新其 prompt/output 视图。
            ...
            # M-RoPE 模型额外初始化多维 rope 位置缓存。
            if self.uses_mrope:
                self._init_mrope_positions(req_state)

            # XD-RoPE 模型额外初始化其 rope 位置缓存。
            if self.uses_xdrope_dim > 0:
                self._init_xdrope_positions(req_state)

            reqs_to_add.append(req_state)
            # ngram_gpu 需要知道哪些请求是本轮首次进入，以便走整张量拷贝。
            if is_ngram_gpu:
                ngram_gpu_new_reqs.append(req_state)

        # ----------------- 最后回写 running/resumed 请求的增量状态 -----------------
        is_last_rank = get_pp_group().is_last_rank
        req_data = scheduler_output.scheduled_cached_reqs
        scheduled_spec_tokens = scheduler_output.scheduled_spec_decode_tokens

        # 先记录 scheduler 原始下发的 spec 长度，后面做拒绝修正时还要用到。
        ...
        # 等待 valid sampled token 统计回到 CPU，再据此修正真实接受长度。
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            if req_state.prev_num_draft_len and self.use_async_scheduling:
                # async spec decode 下，上一轮草稿 token 是否已计入进度，
                # 取决于 prev_num_draft_len 是否非零；这里据此回退被拒绝的部分。
            ...
            if not is_last_rank:
                if not req_data.new_token_ids:
                    # async + PP 场景下，sampled token 通过 GPU 广播传播，这里无需显式携带。
                    new_token_ids: list[int] = []
                else:
                    # 非 async 的 PP 场景没有首尾 stage 直连，因此由 scheduler 回传 token ids。
                    new_token_ids = req_data.new_token_ids[i]
                    # 这里只补已经确认过的 sampled token，不包含尚未验证的 spec token。
                    num_new_tokens = (
                ...
                # 若同步 KV load 失败导致尾部 token 被丢弃，这里把本地输出视图对齐回去。
    ...
    def _prepare_inputs(
        ...
        """把本轮调度结果展开成模型前向所需的输入张量与采样元数据。"""
        # 读取这一轮总共需要处理的 token 数。
        ...
        # 先提交 block table 拷贝，尽量与后续 CPU 操作重叠。
        self.input_batch.block_table.commit_block_table(num_reqs)

        # 将每个请求的 token 数展开成逐 token 的请求索引。
        req_indices = np.repeat(self.arange_np[:num_reqs], num_scheduled_tokens)

        # 计算累计 token 边界和每个请求内的局部位置。
        cu_num_tokens, arange = self._get_cumsum_and_arange(num_scheduled_tokens)

        # 取出这轮 token 对应的位置缓存视图。
        positions_np = self.positions.np[:total_num_scheduled_tokens]
        # 把“已计算 token 数 + 当前局部偏移”写成真实 positions。
        np.add(
        ...
        # M-RoPE 模型额外计算其位置编码所需的位置张量。
        if self.uses_mrope:
            self._calc_mrope_positions(scheduler_output)

        # XD-RoPE 模型额外计算其位置编码所需的位置张量。
        if self.uses_xdrope_dim > 0:
            self._calc_xdrope_positions(scheduler_output)

        # 把二维的 [请求, 位置] 映射成 token buffer 上的一维索引。
        token_indices = (
            positions_np + req_indices * self.input_batch.token_ids_cpu.shape[1]
        )
        # 把 numpy 索引包装成 torch tensor，供 index_select 使用。
        token_indices_tensor = torch.from_numpy(token_indices)

        # 这里用 torch.index_select，而不是 np.take，以减少大张量 gather 的开销。
        # 根据索引把 token ids 收集到本轮输入缓冲区。
        torch.index_select(
        ...
        if self.enable_prompt_embeds:
            # 使用 prompt embeds 时同步收集 is_token_ids 标记。
        ...
        # InputBatch 不会预先分配一整块超大的 prompt_embeds CPU 缓冲，
        # 因此这里把每个请求的 prompt embeds 拷入本轮实际需要的位置。
        if self.input_batch.req_prompt_embeds:
            ...
            for req_idx in range(num_reqs):
                ...
                # 起始位置越过可用 embeds 时，不再尝试读取。
                if start_pos >= req_embeds.shape[0]:
                    output_idx += num_sched
                    continue

                # 只拷贝本轮实际可用的那一段 embeds。
                end_pos = start_pos + num_sched
                actual_end = min(end_pos, req_embeds.shape[0])
                actual_num_sched = actual_end - start_pos

                if actual_num_sched > 0:
                    # 将本轮真正可用的 prompt embeds 拷入预分配缓冲区。
                    self.inputs_embeds.cpu[
                        output_idx : output_idx + actual_num_sched
                    ].copy_(req_embeds[start_pos:actual_end])

                # 无论是否拷贝成功，都推进本请求占用的输出区间。
                output_idx += num_sched

        # 计算这轮 token 对应的 slot mapping。
        self.input_batch.block_table.compute_slot_mapping(req_indices, positions_np)
        # 提交 slot mapping 到设备侧缓冲区。
        self.input_batch.block_table.commit_slot_mapping(total_num_scheduled_tokens)

        # ----------------- 生成 attention metadata -----------------
        self.query_start_loc.np[0] = 0
        self.query_start_loc.np[1 : num_reqs + 1] = cu_num_tokens
        # FlashAttention 等 kernel 要求 query_start_loc 单调不减，尾部因此补平。
        self.query_start_loc.np[num_reqs + 1 :].fill(cu_num_tokens[-1])
        # 将 query_start_loc 拷到 GPU。
        self.query_start_loc.copy_to_gpu()
        # 取出当前 batch 实际使用的 query_start_loc 视图。
        ...
        # 记录哪些请求本轮不应采样，返回前会据此清理多余 sampled token。
        ...
        if self.lora_config:
            assert (
                ...
                <= self.cfie_config.scheduler_config.max_num_batched_tokens
            )
    ...
    def sync_and_slice_intermediate_tensors(
        ...
        tp = self.cfie_config.parallel_config.tensor_parallel_size
        is_rs = is_residual_scattered_for_sp(self.cfie_config, num_tokens)

        # When sequence parallelism is enabled, the "residual" tensor is sharded
        # across tensor parallel ranks, so each rank only needs its own slice.
        if sync_self:
            assert intermediate_tensors is not None
            incoming_keys = set(intermediate_tensors.tensors)
            for k, v in intermediate_tensors.items():
                if not isinstance(v, torch.Tensor):
                    self.intermediate_tensors[k] = v
                    continue
            ...
            stale_metadata_keys = [
                ...
                if not isinstance(v, torch.Tensor) and k not in incoming_keys
            ]
            for key in stale_metadata_keys:
                self.intermediate_tensors.tensors.pop(key, None)

        return IntermediateTensors(
            {
                k: v[: num_tokens // tp]
                if isinstance(v, torch.Tensor) and k == "residual" and is_rs
                else v[:num_tokens]
    ...
    def _pad_for_sequence_parallelism(self, num_scheduled_tokens: int) -> int:
        ...
        tp_size = self.cfie_config.parallel_config.tensor_parallel_size
    ...
    def _sample(
        ...
        # 读取当前 batch 已整理好的采样元数据。
        sampling_metadata = self.input_batch.sampling_metadata
        # Update output token ids with tokens sampled in last step
        # if async scheduling and required by current sampling params.
        # 异步调度场景先把上一轮采样结果同步回 output token ids。
        self.input_batch.update_async_output_token_ids()
        # 非 speculative decoding 直接走普通 sampler。
        ...
        if self.use_async_scheduling and self._draft_token_req_ids is not None:
            # 异步 speculative 场景下补回真实 draft token ids。
            draft_token_ids_cpu, _ = self._get_draft_token_ids_cpu()
            self.input_batch.update_async_spec_token_ids(draft_token_ids_cpu)

        # speculative decoding 通过 rejection sampler 产出最终 token。
    ...
    def _determine_batch_execution_and_padding(
        ...
        if self.compilation_config.pass_config.enable_sp:
            assert (
                ...
                % self.cfie_config.parallel_config.tensor_parallel_size
                == 0
            ), (
        ...
        if self.cfie_config.parallel_config.data_parallel_size > 1:
        ...
        if self.cfie_config.observability_config.cudagraph_metrics:
    ...
    def _register_layerwise_nvtx_hooks(self) -> None:
        ...
        if (
            self.cfie_config.observability_config.enable_layerwise_nvtx_tracing
            ...
            if (
                self.cfie_config.compilation_config.mode
    ...
    def execute_model(
        ...
        # ----------------- GPUModelRunner 主执行入口 -----------------
        # execute_model 与 sample_tokens 之间共享状态，未消费前禁止重入。
        ...
        # 若已启用 routed experts 捕获，则先清空上一轮捕获缓冲。
        if self.routed_experts_initialized:
        ...
        # ngram_gpu 会在本地修改 scheduler_output 的可变字段，因此先复制一份本地副本。
        # 这里用 replace + 浅拷贝，比 deepcopy 便宜得多。
        if (
            self.speculative_config is not None
            and self.speculative_config.use_ngram_gpu()
        ):
            # 复制可变字段，避免 engine core 那边的 scheduler_output 被联动修改。
        ...
        # 若存在抢占且启用了 KV transfer，则先通知 transfer group。
        ...
        with (
            ...
            # 先把 scheduler_output 落到 worker 侧的 persistent batch / req_state。
            ...
            if has_ec_transfer() and not get_ec_transfer().is_consumer:
                with self.maybe_get_ec_connector_output(
                    ...
                    # 仅做编码侧处理时，执行多模态 encoder 后直接返回空输出。
                    self._execute_mm_encoder(scheduler_output)
            ...
            if not num_scheduled_tokens:
                ...
                if not has_kv_transfer_group():
                    # 无 token 且无 KV transfer 时直接返回空输出。
                    return EMPTY_MODEL_RUNNER_OUTPUT
                return self.kv_connector_no_forward(scheduler_output, self.cfie_config)
            ...
            # 根据 InputBatch 当前状态与本轮 token 计划，准备 input_ids/positions/slot mapping。
    ...
    def load_model(self, load_dummy_weights: bool = False) -> None:
        ...
        try:
            with DeviceMemoryProfiler() as m:
                ...
                if load_dummy_weights:
                    # dummy run 场景下改为 dummy 权重加载格式。
                    self.load_config.load_format = "dummy"
                # 选择当前 load_format 对应的模型加载器。
                model_loader = get_model_loader(self.load_config)
                # 真正加载主模型权重并构造模型对象。
                self.model = model_loader.load_model(
                    cfie_config=self.cfie_config, model_config=self.model_config
                )
                if self.lora_config:
                    # 若启用了 LoRA，则继续包装 LoRA 能力。
                    self.model = self.load_lora_model(
                        self.model, self.cfie_config, self.device
                    )
                if hasattr(self, "drafter"):
                    # speculative decoding 场景下继续加载 drafter。
                    # 这一步发生在主模型加载完成之后，因此 draft/MTP 可以拿到 target 的最终运行配置。
                    ...
                    if (
                        ...
                        spec_config = self.cfie_config.speculative_config
                        assert spec_config is not None
                        ...
                        if self.eplb_state is None:
                            # 若主模型未创建 EPLB 状态，则在 drafter 分支补建。
                ...
                if self.use_aux_hidden_state_outputs:
                    ...
                    # 把 auxiliary hidden state 层配置写入模型。
            ...
            msg = (
                ...
                "See https://docs.cfie.ai/en/latest/configuration/conserving_memory/ "
                "for more tips."
            )
        ...
        if (
            ...
            # stock torch compile 模式下直接编译并返回。
            backend = self.cfie_config.compilation_config.init_backend(self.cfie_config)
            compilation_counter.stock_torch_compile_count += 1
            self.model.compile(fullgraph=True, backend=backend)
            return
        # for other compilation modes, cudagraph behavior is controlled by
        # CudagraphWrapper and CudagraphDispatcher of cfie.
        ...
        if (
            ...
            # 非 ubatching 场景用完整 CUDAGraphWrapper 包装模型。
            self.model = CUDAGraphWrapper(
                self.model, self.cfie_config, runtime_mode=CUDAGraphMode.FULL
            )
        elif self.parallel_config.use_ubatching:
            if cudagraph_mode.has_full_cudagraphs():
                # ubatching + full cudagraph 场景使用 FULL 模式包装。
                self.model = UBatchWrapper(
                    self.model, self.cfie_config, CUDAGraphMode.FULL, self.device
                )
            else:
                # ubatching 但无 full cudagraph 时退化为 NONE 模式包装。
                self.model = UBatchWrapper(
                    self.model, self.cfie_config, CUDAGraphMode.NONE, self.device
                )

        # 模型构建完后通知 offloader 做收尾初始化。
    ...
    def reload_weights(
        ...
        load_device = (
            self.cfie_config.load_config.device or self.cfie_config.device_config.device
        )
    ...
    def maybe_randomize_inputs(
        ...
        dp_size = self.cfie_config.parallel_config.data_parallel_size
    ...
    def _dummy_run(
        ...
        """
        mm_config = self.cfie_config.model_config.multimodal_config
        ...
        ubatch_slices, ubatch_slices_padded = maybe_create_ubatch_slices(
            ...
            num_tokens_padded,
            num_reqs_padded,
            self.cfie_config.parallel_config.num_ubatches,
        )
        logger.debug(
            "ubatch_slices: %s, ubatch_slices_padded: %s",
            ubatch_slices,
    ...
    def _dummy_pooler_run(
        ...
        if not supported_pooling_tasks:
            raise RuntimeError(
                ...
                "https://docs.cfie.ai/en/latest/models/pooling_models.html "
                "to learn more."
            )
    ...
    def _init_minimal_kv_cache_for_profiling(self) -> None:
        from cfie.v1.core.kv_cache_utils import (
            get_kv_cache_config_from_groups,
            get_kv_cache_groups,
        )

        kv_cache_spec = self.get_kv_cache_spec()
        kv_cache_groups = get_kv_cache_groups(self.cfie_config, kv_cache_spec)
        ...
        minimal_config = get_kv_cache_config_from_groups(
            self.cfie_config, kv_cache_groups, available_memory=0
        )
        ...
        for batch_desc in batch_descriptors:
            ...
            allow_microbatching = (
                ...
                and check_ubatch_thresholds(
                    config=self.cfie_config.parallel_config,
    ...
    def initialize_attn_backend(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        def get_attn_backends_for_group(
            ...
            layers = get_layers_from_cfie_config(
                self.cfie_config, layer_type, kv_cache_group_spec.layer_names
            )
            attn_backends = {}
        ...
        check_attention_cp_compatibility(self.cfie_config)
        ...
        for attn_backend_set, kv_cache_group in zip(
            ...
            for attn_backend in attn_backend_set:
                ...
                cg_support = builder_cls.get_cudagraph_support(
                    self.cfie_config, kv_cache_group.kv_cache_spec
                )
    ...
    def _update_hybrid_attention_mamba_layout(
        ...
        for group in self._kv_cache_spec_attn_group_iterator():
            ...
            # 用 probe shape 判断该 backend 的“语义布局”是否以 KV 维开头，
            # 避免 num_blocks=2 时仅靠实际 shape 发生二义性。
            kernel_block_size = kernel_block_sizes[group.kv_cache_group_id]
            ...
            if not (len(probe_shape) >= 2 and probe_shape[0] == 2 and probe_shape[1] == 3):
                continue
    ...
    def initialize_kv_cache_tensors(
        ...
        if self.use_uniform_kv_cache(self.attn_groups, cache_dtype):
            ...
            # 记录对应 attention backend。
            self.cross_layers_attn_backend = attn_backend
        else:
            # Fallback to the general case
            # Initialize the memory buffer for KV cache
            # 通用路径先分配原始 KV cache buffer。
            kv_cache_raw_tensors = self._allocate_kv_cache_tensors(kv_cache_config)

            # Change the memory buffer to the desired shape
            # 再把原始 buffer reshape 成各层实际使用的视图。
            kv_caches = self._reshape_kv_cache_tensors(
                kv_cache_config, kv_cache_raw_tensors, kernel_block_sizes
            )

        # Set up cross-layer KV cache sharing
        # 为 cross-layer KV sharing 的层复用目标层缓存。
        ...
        # 把准备好的 KV cache 绑定进静态 forward 上下文。
        bind_kv_cache(
            kv_caches,
            self.compilation_config.static_forward_context,
            self.kv_caches,
    ...
    def maybe_add_kv_sharing_layers_to_kv_cache_groups(
        ...
        if self.cache_config.kv_sharing_fast_prefill:
            ...
            attn_layers = get_layers_from_cfie_config(self.cfie_config, Attention)
    ...
    def initialize_kv_cache(self, kv_cache_config: KVCacheConfig) -> None:
        ...
        # 保存本 runner 使用的 KV cache 配置。
        self.kv_cache_config = kv_cache_config
        # 重置 mamba copy buffer 状态。
        self._mamba_copy_bufs = None
        # 把 encoder-only attention 层补进 KV cache 配置。
        self.may_add_encoder_only_layers_to_kv_cache_config()
        # 把 KV sharing 层补进对应的 KV cache group。
        self.maybe_add_kv_sharing_layers_to_kv_cache_groups(kv_cache_config)
        # 根据 KV cache 配置初始化 attention backend。
        ...
        # 缓存各个 KV cache group 的 kernel block size。
        self._kernel_block_sizes = kernel_block_sizes

        # create metadata builders
        # 为 attention backend 创建 metadata builder。
        self.initialize_metadata_builders(kv_cache_config, kernel_block_sizes)

        # Reinitialize need to after initialize_attn_backend
        # 若 block size 发生变化，则重建 input batch。
        ...
        if (
            ...
            # group
            # 校验 extract_hidden_states 模式的 drafter 与主模型 KV group 对齐。
            self.drafter.validate_same_kv_cache_group(kv_cache_config)

        if has_kv_transfer_group():
            # 若启用了 KV transfer，则把 KV cache 注册给 transfer group。
            kv_transfer_group = get_kv_transfer_group()
            if self.cross_layers_kv_cache is not None:
                assert self.cross_layers_attn_backend is not None
                # 跨层共享场景注册统一的 cross-layer KV cache。
    ...
    def _bind_routed_experts_capturer(self, capturer: RoutedExpertsCapturer) -> None:
        ...
        from cfie.model_executor.layers.fused_moe.router.base_router import (
            BaseRouter,
        )
    ...
    def may_add_encoder_only_layers_to_kv_cache_config(self) -> None:
        ...
        """
        block_size = self.cfie_config.cache_config.block_size
    ...
    def get_kv_cache_spec(self) -> dict[str, KVCacheSpec]:
        ...
        layer_type = cast(type[Any], AttentionLayerBase)
        attn_layers = get_layers_from_cfie_config(self.cfie_config, layer_type)
        for layer_name, attn_module in attn_layers.items():
