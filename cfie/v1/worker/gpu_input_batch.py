...
from cfie.sampling_params import SamplingParams, SamplingType
from cfie.utils import length_from_prompt_token_ids_or_embeds
from cfie.utils.collection_utils import swap_dict_values
...
from cfie.v1.utils import copy_slice
from cfie.v1.worker.block_table import MultiGroupBlockTable
...
class CachedRequestState:
    ...
    # async scheduling + spec decode 同时开启时，用它记录上一轮草稿 token 长度。
    prev_num_draft_len: int = 0

    # pooling 模型额外携带 pooler 相关参数与状态。
...
class InputBatch:
    def __init__(
        ...
        pin_memory: bool,
        vocab_size: int,
        block_sizes: list[int],  # 每个 KV cache group 的 block_size。
        ...
        # req_id_to_index / _req_ids 共同描述 persistent batch 中请求槽位的双向映射。
        self._req_ids: list[str | None] = []
        self.req_id_to_index: dict[str, int] = {}

        # token_ids_cpu_tensor 保存每个请求完整的 token 视图。
        # 这块缓冲不直接搬到 GPU，因此不需要 pin memory。
        ...
        # prompt embeds 按请求单独保存，避免一次性为整个 max_model_len 预分配大缓冲。
        ...
        # prompt 长度和已计算长度是 scheduler -> worker 对齐进度的关键状态。
        ...
        # block_table 维护每个请求当前位置映射到哪块 KV cache slot。
