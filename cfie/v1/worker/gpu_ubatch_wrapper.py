...
from cfie.distributed.device_communicators.pynccl_allocator import set_graph_pool_id
...
from cfie.utils.import_utils import has_deep_gemm
from cfie.utils.platform_utils import num_compute_units
from cfie.v1.worker.ubatching import UBatchContext, make_ubatch_contexts
...
class UBatchWrapper:
    def __init__(
        ...
        self.ready_barrier = threading.Barrier(
            self.cfie_config.parallel_config.num_ubatches + 1
        )
        ...
        if runtime_mode is not CUDAGraphMode.NONE:
            self.cudagraph_wrapper = CUDAGraphWrapper(
                runnable, cfie_config, runtime_mode=runtime_mode
            )

        self.sm_control = self._create_sm_control_context(cfie_config)
        self.device = device
    ...
    @staticmethod
    def _create_sm_control_context(cfie_config: CfieConfig):
        comm_sms: int = envs.VLLM_DBO_COMM_SMS

        set_comm_sms = lambda sms: None
        if cfie_config.parallel_config.enable_expert_parallel:
