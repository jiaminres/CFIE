...
class CPUModelRunner(GPUModelRunner):
    def __init__(self, cfie_config: CfieConfig, device: torch.device):
        with _torch_cuda_wrapper():
            super().__init__(cfie_config, device)
    ...
    def load_model(self, load_dummy_weights: bool = False) -> None:
        ...
        self.model = get_model(cfie_config=self.cfie_config)

        if self.lora_config:
            self.model = self.load_lora_model(self.model, self.cfie_config, self.device)
    ...
    def warming_up_model(self) -> None:
        ...
        with _set_global_compilation_settings(self.cfie_config):
            self._dummy_run(
                min(
...
@contextmanager
def _set_global_compilation_settings(config: CfieConfig):
