...
def sanity_check_mm_encoder_outputs(
    ...
    [`cfie.model_executor.models.SupportsMultiModal.embed_multimodal`][].
    """
...
def get_static_memory_budget(
    ...
    if init_snapshot.free_memory < static_memory_budget:
        raise ValueError(
            ...
            f"is less than the required static GPU budget "
            f"({cache_config.gpu_memory_utilization}, "
            f"{format_gib(static_memory_budget)} GiB). Decrease GPU memory "
...
def is_residual_scattered_for_sp(
    cfie_config: CfieConfig, num_input_tokens: int
) -> bool:
    ...
    """
    if not cfie_config.compilation_config.pass_config.enable_sp:
        return False

    tp = cfie_config.parallel_config.tensor_parallel_size
    ...
    if (
        not cfie_config.compilation_config.splitting_ops
        or cfie_config.compilation_config.use_inductor_graph_partition
    ):
        return True
    compile_sizes = cfie_config.compilation_config.compile_sizes
