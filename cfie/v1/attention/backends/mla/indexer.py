...
from cfie.v1.worker.cp_utils import get_total_cp_world_size
...
def get_max_prefill_buffer_size(cfie_config: CfieConfig):
...
class DeepseekV32IndexerMetadataBuilder(AttentionMetadataBuilder):
    ...
    def __init__(self, *args, **kwargs):
        ...
        scheduler_config = self.cfie_config.scheduler_config
        # NOTE(Chen):an estimated max size of flattened_kv. Need to double check.
        self.max_prefill_buffer_size = get_max_prefill_buffer_size(self.cfie_config)
        self.num_speculative_tokens = (
            self.cfie_config.speculative_config.num_speculative_tokens
            if self.cfie_config.speculative_config
            else 0
        )
