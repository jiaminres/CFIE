...
class FlashMLABackend(MLACommonBackend):
    ...
    def supports_combination(
        ...
        if use_sparse:
            from cfie.v1.attention.ops.flashmla import is_flashmla_sparse_supported

            return is_flashmla_sparse_supported()[1]
        else:
            from cfie.v1.attention.ops.flashmla import is_flashmla_dense_supported
...
class FlashMLAMetadataBuilder(MLACommonMetadataBuilder[FlashMLAMetadata]):
    ...
    def __init__(
        ...
        super().__init__(
            kv_cache_spec, layer_names, cfie_config, device, FlashMLAMetadata
        )

        self.num_q_heads = cfie_config.model_config.get_num_attention_heads(
            cfie_config.parallel_config
        )

        self.cg_buf_tile_scheduler_metadata = None
        self.cg_buf_num_splits = None
        self.is_fp8_kvcache = cfie_config.cache_config.cache_dtype.startswith("fp8")
        ...
        if self.compilation_config.cudagraph_mode.has_full_cudagraphs():
            ...
            self.cg_buf_num_splits = torch.empty(
                (cfie_config.scheduler_config.max_num_seqs + 1),
                device=self.device,
                dtype=torch.int32,
            )
