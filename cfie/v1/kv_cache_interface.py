...
from cfie.utils.torch_utils import get_dtype_size
...
class KVCacheSpec:
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        """
...
class FullAttentionSpec(AttentionSpec):
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        ...
        dcp_world_size = cfie_config.parallel_config.decode_context_parallel_size
        pcp_world_size = cfie_config.parallel_config.prefill_context_parallel_size
...
class MLAAttentionSpec(FullAttentionSpec):
    ...
    def real_page_size_bytes(self) -> int:
        if self.cache_dtype_str == "fp8_ds_mla":
            # See `cfie/v1/attention/backends/mla/flashmla_sparse.py`
            #  for details.
...
class ChunkedLocalAttentionSpec(AttentionSpec):
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        max_model_len = cfie_config.model_config.max_model_len
        max_num_batched_tokens = cfie_config.scheduler_config.max_num_batched_tokens
...
class SlidingWindowSpec(AttentionSpec):
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        assert cfie_config.parallel_config.decode_context_parallel_size == 1, (
...
class MambaSpec(KVCacheSpec):
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        if cfie_config.cache_config.mamba_cache_mode == "all":
            max_model_len = cfie_config.model_config.max_model_len
            return cdiv(max_model_len, self.block_size) * self.page_size_bytes
        elif cfie_config.cache_config.mamba_cache_mode == "align":
...
class CrossAttentionSpec(AttentionSpec):
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        ...
        max_encoder_len = cfie_config.scheduler_config.max_num_encoder_input_tokens
...
class UniformTypeKVCacheSpecs(KVCacheSpec):
    ...
    def max_memory_usage_bytes(self, cfie_config: CfieConfig) -> int:
        max_num_pages = max(
            cdiv(spec.max_memory_usage_bytes(cfie_config), spec.page_size_bytes)
