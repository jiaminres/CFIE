...
class InvalidComponent(Exception):
    ...
    applicable to the given CfieConfig.
    """
...
class Parser(Protocol):
    def parse(self, args: ParsedArgs, cfie_config: CfieConfig) -> ParsedArgs:
        """
        Parse the cfie config and update the current ParsedArgs and pass it on.
        If the parser isn't applicable to the cfie_config, it will do nothing.
        """
        ...
...
class ParserChain:
    ...
    def parse(self, cfie_config: CfieConfig) -> ParsedArgs:
        args = ParsedArgs()
        for parser in self.parsers:
            args = parser.parse(args, cfie_config)
        return args
...
class ComponentMetrics(BaseModel, ABC):
    ...
    - parser to parse CfieConfig into fields
    ...
    def get_parser(cls) -> ParserChain:
        ...
        a ValidationError when from_cfie_config() is called.
    ...
    @classmethod
    def from_cfie_config(cls, cfie_config: CfieConfig) -> Self:
        """
        Instantiate this class from CfieConfig.
        Raises ValidationError if parsing fails.
        """

        parser = cls.get_parser()
        parsed_args = parser.parse(cfie_config)
        try:
...
class BaseConfigParser(Parser):
    ...
    def parse(self, args: ParsedArgs, cfie_config: CfieConfig) -> ParsedArgs:
        ...
        model_dtype = cfie_config.model_config.dtype
        ...
        args.dp_size = cfie_config.parallel_config.data_parallel_size
        args.tp_size = cfie_config.parallel_config.tensor_parallel_size
        args.pp_size = cfie_config.parallel_config.pipeline_parallel_size
        args.enable_ep = cfie_config.parallel_config.enable_expert_parallel
...
class BaseAttentionConfigParser(Parser):
    ...
    def parse(self, args: ParsedArgs, cfie_config: CfieConfig) -> ParsedArgs:
        ...
        model_dtype = cfie_config.model_config.dtype
        cache_dtype = cfie_config.cache_config.cache_dtype
...
class BaseFfnConfigParser(Parser):
    ...
    def parse(self, args: ParsedArgs, cfie_config: CfieConfig) -> ParsedArgs:
        cfg = cfie_config.model_config.hf_config
        ...
        args.num_experts = cfie_config.model_config.get_num_experts()
...
class ModelMetrics:
    def __init__(self, cfie_config: CfieConfig) -> None:
        """
        Parse cfie_config to instantiate metrics for each component.
        ...
        for metric_cls in ComponentMetrics.registered_metrics():
            try:
                metric = metric_cls.from_cfie_config(cfie_config)
...
class PerfMetricsLogging:
    def __init__(self, cfie_config: CfieConfig):
        self.cfie_config = cfie_config
        self.pp_size = cfie_config.parallel_config.pipeline_parallel_size
...
class PerfMetricsProm:
    ...
    calculated using a PromQL query:
      ...
      rate(cfie:estimated_flops_per_gpu_total[1m]) / 1e12

    Average memory bandwidth in GB/s can be calculated using:

      (rate(cfie:estimated_read_bytes_per_gpu_total[1m]) +
       rate(cfie:estimated_write_bytes_per_gpu_total[1m])) / 1e9
    """
    ...
    def __init__(
        ...
        counter_flops = self._counter_cls(
            name="cfie:estimated_flops_per_gpu_total",
            documentation=(
        ...
        counter_read_bytes = self._counter_cls(
            name="cfie:estimated_read_bytes_per_gpu_total",
            documentation=(
        ...
        counter_write_bytes = self._counter_cls(
            name="cfie:estimated_write_bytes_per_gpu_total",
            documentation=(
