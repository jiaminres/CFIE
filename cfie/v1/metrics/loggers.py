...
from cfie.compilation.cuda_graph import CUDAGraphLogging
from cfie.config import SupportsMetricsInfo, CfieConfig
from cfie.distributed.kv_transfer.kv_connector.v1.metrics import (
    KVConnectorLogging,
    KVConnectorPrometheus,
)
from cfie.logger import init_logger
from cfie.plugins import STAT_LOGGER_PLUGINS_GROUP, load_plugins_by_group
from cfie.v1.engine import FinishReason
from cfie.v1.metrics.perf import PerfMetricsLogging, PerfMetricsProm
from cfie.v1.metrics.prometheus import unregister_cfie_metrics
from cfie.v1.metrics.stats import (
    CachingMetrics,
    IterationStats,
...
from cfie.v1.spec_decode.metrics import SpecDecodingLogging, SpecDecodingProm

logger = init_logger(__name__)

PerEngineStatLoggerFactory = Callable[[CfieConfig, int], "StatLoggerBase"]
...
class StatLoggerBase(ABC):
    ...
    @abstractmethod
    def __init__(self, cfie_config: CfieConfig, engine_index: int = 0): ...
...
class AggregateStatLoggerBase(StatLoggerBase):
    ...
    @abstractmethod
    def __init__(self, cfie_config: CfieConfig, engine_indexes: list[int]): ...
...
class LoggingStatLogger(StatLoggerBase):
    def __init__(self, cfie_config: CfieConfig, engine_index: int = 0):
        ...
        kv_transfer_config = self.cfie_config.kv_transfer_config
        ...
        if self.cfie_config.observability_config.cudagraph_metrics:
            self.cudagraph_logging = CUDAGraphLogging(
                self.cfie_config.compilation_config.cudagraph_mode,
                self.cfie_config.compilation_config.cudagraph_capture_sizes,
            )
        ...
        if self._enable_perf_stats():
            self.perf_metrics_logging = PerfMetricsLogging(cfie_config)
    ...
    def _enable_perf_stats(self) -> bool:
        return self.cfie_config.observability_config.enable_mfu_metrics
    ...
    def log_engine_initialized(self):
        if self.cfie_config.cache_config.num_gpu_blocks:
            logger.debug(
                "Engine %03d: cfie cache_config_info with initialization "
                "after num_gpu_blocks is: %d",
                self.engine_index,
                self.cfie_config.cache_config.num_gpu_blocks,
            )
...
class AggregatedLoggingStatLogger(LoggingStatLogger, AggregateStatLoggerBase):
    def __init__(
        ...
        LoggingStatLogger.__init__(self, cfie_config, engine_index=-1)
    ...
    def log_engine_initialized(self):
        if self.cfie_config.cache_config.num_gpu_blocks:
            logger.info(
                "%d Engines: cfie cache_config_info with initialization "
                "after num_gpu_blocks is: %d",
...
class PrometheusStatLogger(AggregateStatLoggerBase):
    ...
    def __init__(
        self, cfie_config: CfieConfig, engine_indexes: list[int] | None = None
    ):
        ...
        unregister_cfie_metrics()
        self.cfie_config = cfie_config
        # Use this flag to hide metrics that were deprecated in
        # a previous release and which will be removed future
        self.show_hidden_metrics = cfie_config.observability_config.show_hidden_metrics
        self.kv_cache_metrics_enabled = (
            cfie_config.observability_config.kv_cache_metrics
        )

        labelnames = ["model_name", "engine"]
        model_name = cfie_config.model_config.served_model_name
        max_model_len = cfie_config.model_config.max_model_len
        ...
        self.spec_decoding_prom = self._spec_decoding_cls(
            cfie_config.speculative_config, labelnames, per_engine_labelvalues
        )
        self.kv_connector_prom = self._kv_connector_cls(
            cfie_config, labelnames, per_engine_labelvalues
        )
        ...
        gauge_scheduler_running = self._gauge_cls(
            name="cfie:num_requests_running",
        ...
        gauge_scheduler_waiting = self._gauge_cls(
            name="cfie:num_requests_waiting",
        ...
        counter_prefix_cache_queries = self._counter_cls(
            name="cfie:prefix_cache_queries",
            documentation=(
        ...
        counter_connector_prefix_cache_queries = self._counter_cls(
            name="cfie:external_prefix_cache_queries",
            documentation=(
        ...
        counter_connector_prefix_cache_hits = self._counter_cls(
            name="cfie:external_prefix_cache_hits",
            documentation=(
        ...
        counter_prompt_tokens_by_source = self._counter_cls(
            name="cfie:prompt_tokens_by_source",
        ...
        counter_prompt_tokens_cached = self._counter_cls(
            name="cfie:prompt_tokens_cached",
        ...
        counter_prompt_tokens_recomputed = self._counter_cls(
            name="cfie:prompt_tokens_recomputed",
        ...
        histogram_num_prompt_tokens_request = self._histogram_cls(
            name="cfie:request_prompt_tokens",
            documentation="Number of prefill tokens processed.",
        ...
        histogram_num_generation_tokens_request = self._histogram_cls(
            name="cfie:request_generation_tokens",
            documentation="Number of generation tokens processed.",
            buckets=build_1_2_5_buckets(max_model_len),
            labelnames=labelnames,
        )
        ...
        # See: https://github.com/cfie-project/cfie/pull/18053
        histogram_iteration_tokens = self._histogram_cls(
            name="cfie:iteration_tokens_total",
        ...
        histogram_max_num_generation_tokens_request = self._histogram_cls(
            name="cfie:request_max_num_generation_tokens",
        ...
        histogram_max_tokens_request = self._histogram_cls(
            name="cfie:request_params_max_tokens",
        ...
        histogram_time_to_first_token = self._histogram_cls(
            name="cfie:time_to_first_token_seconds",
        ...
        histogram_inter_token_latency = self._histogram_cls(
            name="cfie:inter_token_latency_seconds",
        ...
        histogram_request_time_per_output_token = self._histogram_cls(
            name="cfie:request_time_per_output_token_seconds",
        ...
        histogram_e2e_time_request = self._histogram_cls(
            name="cfie:e2e_request_latency_seconds",
        ...
        histogram_queue_time_request = self._histogram_cls(
            name="cfie:request_queue_time_seconds",
        ...
        histogram_inference_time_request = self._histogram_cls(
            name="cfie:request_inference_time_seconds",
        ...
        histogram_prefill_time_request = self._histogram_cls(
            name="cfie:request_prefill_time_seconds",
        ...
        histogram_decode_time_request = self._histogram_cls(
            name="cfie:request_decode_time_seconds",
        ...
        histogram_prefill_kv_computed_request = self._histogram_cls(
            name="cfie:request_prefill_kv_computed_tokens",
            documentation=(
        ...
        if self.kv_cache_metrics_enabled:
            ...
            histogram_kv_block_lifetime = self._histogram_cls(
                name="cfie:kv_block_lifetime_seconds",
                documentation=(
            ...
            histogram_kv_block_idle_before_evict = self._histogram_cls(
                name="cfie:kv_block_idle_before_evict_seconds",
                documentation=(
            ...
            histogram_kv_block_reuse_gap = self._histogram_cls(
                name="cfie:kv_block_reuse_gap_seconds",
                documentation=(
        ...
        if cfie_config.lora_config is not None:
            if len(self.engine_indexes) > 1:
                logger.warning(
                    "cfie:lora_requests_info prometheus metrics may be "
                    "incorrect/misleading with data parallel deployments."
                )
            self.labelname_max_lora = "max_lora"
            self.labelname_waiting_lora_adapters = "waiting_lora_adapters"
            self.labelname_running_lora_adapters = "running_lora_adapters"
            self.max_lora = cfie_config.lora_config.max_loras
            self.gauge_lora_info = self._gauge_cls(
                name="cfie:lora_requests_info",
    ...
    def log_engine_initialized(self):
        self.log_metrics_info("cache_config", self.cfie_config.cache_config)
    ...
    def __init__(
        ...
        if not custom_prometheus_logger:
            self.stat_loggers.append(
                PrometheusStatLogger(cfie_config, self.engine_indexes)
            )
