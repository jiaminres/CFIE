...
class Vector(Metric):
    ...
    specific metric, cfie:spec_decode_num_accepted_tokens_per_pos.
    """
...
def get_metrics_snapshot() -> list[Metric]:
    ...
    for metric in REGISTRY.collect():
        if not metric.name.startswith("cfie:"):
            continue
        if metric.type == "gauge":
            ...
            if metric.name == "cfie:spec_decode_num_accepted_tokens_per_pos":
                #
                # Ugly cfie:num_accepted_tokens_per_pos special case.
                #
