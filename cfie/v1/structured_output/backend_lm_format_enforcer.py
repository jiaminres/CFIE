...
if TYPE_CHECKING:
    ...
    import lmformatenforcer.integrations.cfie as lmfe_cfie
else:
    lmformatenforcer = LazyLoader("lmformatenforcer", globals(), "lmformatenforcer")
    lmfe_cfie = LazyLoader(
        "lmformatenforcer.integrations.cfie",
        globals(),
        "lmformatenforcer.integrations.cfie",
    )
...
@lru_cache
def _cached_build_cfie_token_enforcer_tokenizer_data(
    tokenizer: PreTrainedTokenizerBase, vocab_size: int
) -> "lmfe_cfie.TokenEnforcerTokenizerData":
    return lmfe_cfie.build_cfie_token_enforcer_tokenizer_data(
...
class LMFormatEnforcerBackend(StructuredOutputBackend):
    def __post_init__(self):
        self.tokenizer_data = _cached_build_cfie_token_enforcer_tokenizer_data(
