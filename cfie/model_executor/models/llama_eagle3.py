...
from cfie.model_executor.layers.linear import QKVParallelLinear, ReplicatedLinear
...
from cfie.model_executor.models.llama import LlamaDecoderLayer, LlamaForCausalLM
from cfie.multimodal.inputs import NestedTensors
...
class LlamaDecoderLayer(LlamaDecoderLayer):
    def __init__(
        ...
        super().__init__(cfie_config, prefix=prefix, config=config)

        config = config or cfie_config.model_config.hf_config
        quant_config = self.get_quant_config(cfie_config)
    ...
    def get_quant_config(self, cfie_config: CfieConfig) -> QuantizationConfig | None:
        """Use drafter's quantization config instead of verifier's."""
        return get_draft_quant_config(cfie_config)
...
class LlamaModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.config = cfie_config.speculative_config.draft_model_config.hf_config
        self.vocab_size = self.config.vocab_size

        # Get drafter's quantization config
        self.quant_config = get_draft_quant_config(cfie_config)
        ...
        current_vllm_config = get_current_cfie_config()
        ...
        if self.use_aux_hidden_state:
            ...
            self.fc = ReplicatedLinear(
                ...
                bias=False,
                params_dtype=cfie_config.model_config.dtype,
...
class Eagle3LlamaForCausalLM(LlamaForCausalLM):
    def __init__(self, *, cfie_config: CfieConfig, prefix: str = ""):
        ...
        target_layer_num = cfie_config.model_config.get_num_layers(
        ...
        self.model = LlamaModel(
            cfie_config=cfie_config, prefix="model", start_layer_id=target_layer_num
        )
        ...
        self.use_parallel_drafting = cfie_config.speculative_config.parallel_drafting
