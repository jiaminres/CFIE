...
from cfie.config import MultiModalConfig, CfieConfig, get_current_cfie_config
...
def _get_vit_attn_backend(
    ...
    *,
    ...
    """
    获取当前可用的 Vision Transformer attention backend。
    """

    # 把 backend 选择逻辑交给当前平台（如 CUDA / CPU / 其他设备）去决定
    # 如果 attn_backend_override 不为 None，则优先按 override 指定的 backend 走
    return current_platform.get_vit_attn_backend(
        head_size,                         # 每个 attention head 的维度
        dtype,                             # 当前计算 dtype
        backend=attn_backend_override,     # 用户/配置显式指定的 backend（若有）
    )
...
def get_vit_attn_backend(
    ...
    """
    获取 Vision Transformer 使用的 attention backend。
    """

    try:
        # 取当前全局配置对象
        cfie_config: CfieConfig = get_current_cfie_config()

        # 取模型配置
        model_config = cfie_config.model_config

        # 如果当前模型存在多模态配置，则取出 multimodal_config
        # 否则为 None
        multimodal_config: MultiModalConfig | None = (
            model_config.multimodal_config if model_config is not None else None
        )
    except (AssertionError, AttributeError):
        # 如果当前环境下还没有全局配置，或者相关字段不存在，
        # 则认为没有显式的多模态 attention backend 配置
        multimodal_config = None

    # 如果多模态配置里显式指定了 mm_encoder_attn_backend，
    # 就把它取出来作为 override；否则为 None
