...
from cfie.config import ModelConfig, SpeechToTextConfig
from cfie.inputs import TokensPrompt
from cfie.inputs.data import PromptType
from cfie.logger import init_logger
from cfie.model_executor.layers.mamba.mamba_utils import MambaStateCopyFunc
from cfie.model_executor.layers.quantization import QuantizationConfig
from cfie.tasks import ScoreType
from cfie.utils.collection_utils import common_prefix
from cfie.utils.func_utils import supports_kw

from .interfaces_base import CfieModel

if TYPE_CHECKING:
    from cfie.config import CfieConfig
    from cfie.model_executor.models.utils import WeightsMapper
    from cfie.multimodal.inputs import MultiModalFeatureSpec
    from cfie.multimodal.registry import _ProcessorFactories
...
def _require_is_multimodal(is_multimodal: Tensor | None) -> Tensor:
    ...
    [cfie.model_executor.models.interfaces.SupportsMultiModal.embed_input_ids][]
    ...
    if is_multimodal is None:
        raise ValueError(
            ...
            "https://github.com/cfie-project/cfie/pull/16229."
        )
...
_language_model_by_module = dict[nn.Module, CfieModel]()
...
class SupportsMultiModal(Protocol):
    ...
    def get_language_model(self) -> CfieModel:
        """
    ...
    def _mark_language_model(
        ...
        """
        在这个上下文里，凡是被赋值到当前模型上的子模块，
        都会被标记为 language model（语言模型）组件。

        当启用 `--mm-encoder-only` 模式时，
        这些语言模型组件会被自动跳过，不进行真实初始化/使用。

        如果传入了 `targets`，
        则不只收集直接子模块，还会把所有属于 targets 类型的后代模块也纳入。
        """
        # 延迟导入，避免循环依赖
        from .utils import StageMissingLayer, collect_children, no_init_weights

        # 取出当前模型的多模态配置
        mm_config = cfie_config.model_config.multimodal_config

        # 收集在这个上下文里新挂到 self 上的语言模型相关子模块名字
        # 如果 targets 不为空，则改为收集所有匹配类型的后代模块
        with collect_children(self, targets=targets) as children_names:  # noqa: SIM117

            # 如果开启了 mm_encoder_only，
            # 说明当前只跑多模态编码器（如视觉塔），不跑语言模型
            # 这时用 no_init_weights 把语言模型模块替换成占位层 StageMissingLayer，
            # 避免真实分配权重和初始化
            with (
                no_init_weights(
                    self,
                ...
                else nullcontext()  # 否则什么都不做，正常构造真实语言模型
            ):
                # 把控制权交给外层 with 代码块
                # 外面通常会在这里执行：
    ...
    def _mark_tower_model(
        ...
        """
        在这个上下文里，凡是被赋值到当前模型上的子模块，
        都会被标记为某个 tower model（塔模型）组件。

        这些 tower 组件在 `--limit-mm-per-prompt` 对应模态全部设为 0 时，
        可以被自动跳过，不实际初始化/使用。

        如果传了 `targets`，
        那么除了直接子模块外，还会额外把所有属于 targets 类型的后代模块也纳入收集范围。
        """
        # 延迟导入，避免循环依赖
        from .utils import StageMissingLayer, collect_children, no_init_weights

        # 如果 modalities 传进来是单个字符串，例如 "image"
        # 就统一转成集合 {"image"}
        if isinstance(modalities, str):
            modalities = {modalities}

        # 如果模态正好是 image + video，
        # 统一把这个 tower 阶段命名为 vision_tower
        ...
        # collect_children(self, targets=targets)：
        # 进入上下文后，会记录“在这个 with 块中新增/赋值到 self 上的子模块名称”
        # 最后把这些名字收集到 children_names 中
        with collect_children(self, targets=targets) as children_names:  # noqa: SIM117

            # 如果当前这些模态的 limit_per_prompt 全都是 0，
            # 例如 {"image": 16, "video": 2}
            # 表示一个请求里最多 16 张图、2 个视频。
            # 说明这类模态被完全禁用了
            # 这时进入 no_init_weights 上下文：
            # 会把原本要初始化的模块替换成 StageMissingLayer 占位层，
            # 从而跳过真实 tower 的构造/权重初始化
            with (
                no_init_weights(
                    self,
                ...
                else nullcontext()  # 否则什么都不做，正常执行 yield 内的模块构造
            ):
                # 把控制权交还给外层 with 块
                # 外层通常会在这里写：
        ...
        # 退出上下文后，把刚刚收集到的 tower 子模块名字记录下来
        # 后续框架可以用它知道：
        # “哪些模块属于这个 tower，可以在对应模态关闭时跳过”
...
class SupportsPP(Protocol):
    ...
    def forward(
        ...
        """
        Accept [`IntermediateTensors`][cfie.sequence.IntermediateTensors] when
        PP rank > 0.

        Return [`IntermediateTensors`][cfie.sequence.IntermediateTensors] only
...
class SupportsQuant:
    ...
    hf_to_cfie_mapper: ClassVar[WeightsMapper | None] = None
    packed_modules_mapping: ClassVar[dict[str, list[str]] | None] = None
    quant_config: QuantizationConfig | None = None

    @property
    def hf_to_vllm_mapper(self) -> WeightsMapper | None:
        # Keep upstream mapper naming available while CFIE is still migrating
        # model implementations from `hf_to_vllm_mapper` to
        # `hf_to_cfie_mapper`.
        return self.hf_to_cfie_mapper
    ...
    def _find_quant_config(*args, **kwargs) -> QuantizationConfig | None:
        ...
        from cfie.config import CfieConfig  # avoid circular import

        args_values = list(args) + list(kwargs.values())
        for arg in args_values:
            if isinstance(arg, CfieConfig):
    ...
    def _maybe_apply_model_mapping(self):
        ...
        if self.quant_config is None:
            return
        if (hf_to_cfie_mapper := self.hf_to_cfie_mapper) is not None:
            self.quant_config.apply_cfie_mapper(hf_to_cfie_mapper)
