...
from cfie.compilation.decorators import (
    should_torch_compile_mm_encoder,
    support_torch_compile,
)
from cfie.config import CfieConfig
from cfie.distributed import parallel_state
from cfie.distributed import utils as dist_utils
from cfie.forward_context import set_forward_context
from cfie.logger import init_logger
from cfie.model_executor.layers.activation import get_act_and_mul_fn
from cfie.model_executor.layers.attention import MMEncoderAttention
...
from cfie.model_executor.layers.rotary_embedding.common import (
    ApplyRotaryEmb,
)
...
from cfie.multimodal.parse import MultiModalDataItems
from cfie.multimodal.processing import PromptReplacement, PromptUpdate
from cfie.sequence import IntermediateTensors
from cfie.utils.platform_utils import is_pin_memory_available
from cfie.utils.tensor_schema import TensorSchema, TensorShape
...
class Qwen2_5_VisionAttention(nn.Module):
    def __init__(
        self,
        embed_dim: int,   # 输入/输出 token 特征维度
        num_heads: int,   # 总 attention 头数
        projection_size: int,   # Q/K/V 投影后的总维度，通常等于 embed_dim
        quant_config: QuantizationConfig | None = None,  # 量化配置
        prefix: str = "",   # 参数名前缀
    ) -> None:
        super().__init__()

        # 是否对 ViT 使用 data parallel 而不是 tensor parallel
        use_data_parallel = is_vit_use_data_parallel()

        # 当前 tensor parallel 大小
        # 如果走 data parallel，则这里视为不做 TP，因此 tp_size=1
        self.tp_size = (
            1
        ...
        #   [..., embed_dim]
        # 输出:
        #   [..., 3 * num_heads_per_partition * head_dim]
        #
        # 这里 total_num_heads=num_heads, total_num_kv_heads=num_heads
        # 说明这是标准 MHA，不是 GQA/MQA
        ...
        # RoPE 应用模块
        # 这里只对 q 和 k 做旋转，不对 v 做旋转
        self.apply_rotary_emb = ApplyRotaryEmb(enforce_enable=True)

    def forward(
        self,
        x: torch.Tensor,   # 形状: [seq_len, batch_size, embed_dim]
        cu_seqlens: torch.Tensor,   # 形状通常: [batch_size + 1]
        rotary_pos_emb_cos: torch.Tensor,   # 形状常见: [seq_len_rotary, rotary_dim/2]
        rotary_pos_emb_sin: torch.Tensor,   # 形状常见: [seq_len_rotary, rotary_dim/2]
        max_seqlen: torch.Tensor,  # 标量，某些 FlashAttention 后端需要
        sequence_lengths: torch.Tensor,  # 形状常见: [batch_size]，某些 FlashInfer 后端需要
    ) -> torch.Tensor:
        # 经过 qkv 线性层
        #
        # 输入:
        ...
        #
        # 输出:
        #   x: [seq_len, batch_size, 3 * num_heads_per_partition * head_dim]
        x, _ = self.qkv(x)
        ...
        #
        # 原始:
        #   [seq_len, batch_size, 3 * num_heads_per_partition * head_dim]
        #
        # 重排后:
        #   qkv: [batch_size, seq_len, 3, num_heads_per_partition, head_dim]
        ...
        # 如果提供了 rotary cos/sin，则对 q 和 k 应用 RoPE
        if rotary_pos_emb_cos is not None and rotary_pos_emb_sin is not None:
            # 取前两块作为 q 和 k
            # qk: [batch_size, seq_len, 2, num_heads_per_partition, head_dim]
            # v:  [batch_size, seq_len, num_heads_per_partition, head_dim]
            qk, v = qkv[:, :, :2], qkv[:, :, 2]

            # 为了复用 ApplyRotaryEmb 的接口，
            # 把 q 和 k 合并到 batch 维前面
            #
            # 原始:
            #   [batch_size, seq_len, 2, num_heads_per_partition, head_dim]
            #
            # 重排后:
            #   [2 * batch_size, seq_len, num_heads_per_partition, head_dim]
        ...
        #
        # 输入:
        #   q/k/v: [batch_size, seq_len, num_heads_per_partition, head_dim]
        #
        # 输出:
        #   context_layer: [batch_size, seq_len, num_heads_per_partition, head_dim]
        ...
        #
        # 重排后:
        #   [seq_len, batch_size, num_heads_per_partition * head_dim]
        # 也就是:
        #   [seq_len, batch_size, projection_size]
...
class Qwen2_5_VisionTransformer(nn.Module):
    def __init__(
        ...
        # see https://github.com/cfie-project/cfie/issues/27044
...
class Qwen2_5_VLForConditionalGeneration(
    ...
    def __init__(self, *, cfie_config: CfieConfig, prefix: str = ""):
        super().__init__()
        config: Qwen2_5_VLConfig = cfie_config.model_config.hf_config
        ...
        with self._mark_language_model(cfie_config):
            self.language_model = init_cfie_registered_model(
    ...
    def _process_image_input(
        ...
        if image_input["type"] == "image_embeds":
            ...
            with set_forward_context(None, self.cfie_config):
