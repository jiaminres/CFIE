...
import cfie.model_executor.offloader.prefetch_ops  # noqa: F401
...
class PrefetchOffloader(BaseOffloader):
    ...
    def _hook_module_forward(self, index: int, module: nn.Module):
        ...
        def forward(*args, **kwargs):
            ...
            torch.ops.cfie.wait_prefetch(input_tensor, index)
            ...
            if isinstance(output, tuple):
                torch.ops.cfie.start_prefetch(output[0], next_index)
            else:
                torch.ops.cfie.start_prefetch(output, next_index)
