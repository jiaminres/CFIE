...
from cfie.distributed import get_tensor_model_parallel_rank, get_world_group
from cfie.logger import init_logger
from cfie.model_executor.layers.quantization import (
    QuantizationConfig,
    get_quantization_config,
)
from cfie.platforms import current_platform
from cfie.tracing import instrument
from cfie.utils.import_utils import PlaceholderModule
...
from cfie.model_executor.layers.quantization.torchao import torchao_version_at_least
...
def get_quant_config(
    ...
    # -------------------- 先根据量化方法名找到对应的 QuantizationConfig 类 --------------------
    quant_cls = get_quantization_config(model_config.quantization)

    # GGUF 不依赖额外的量化配置文件，直接构造默认配置对象即可。
    if model_config.quantization == "gguf":
        return quant_cls()

    # -------------------- 优先尝试从 Hugging Face 配置对象里直接读取量化配置 --------------------
    # 常规 Hugging Face 模型会把量化配置放在 hf_config.quantization_config 中。
    hf_quant_config = getattr(model_config.hf_config, "quantization_config", None)
    # 有些多模态/视觉模型会把文本子模型配置挂在 text_config 上。
    hf_text_config = getattr(model_config.hf_config, "text_config", None)
    # 如果顶层没找到，就继续尝试从 text_config 里读取 quantization_config。
    if hf_quant_config is None and hf_text_config is not None:
        hf_quant_config = getattr(hf_text_config, "quantization_config", None)
    # compressed-tensors 这一路有时不用 quantization_config，而是放在 compression_config。
    if hf_quant_config is None:
        hf_quant_config = getattr(model_config.hf_config, "compression_config", None)

    # -------------------- 为 compressed-tensors 补充注意力头信息，便于 TP 感知加载 --------------------
    # compressed-tensors 在加载 attention head scale 时，需要知道全局 head 数和 kv head 数。
    if (
        hf_quant_config is not None
        and hf_quant_config.get("quant_method") == "compressed-tensors"
        and "config_groups" in hf_quant_config
    ):
        # 优先从 text_config 里取文本模型的 head 信息；没有时再回退到顶层配置。
        ...
        # 把总 head 数写回量化配置，后续量化加载逻辑会直接读取这些字段。
        hf_quant_config["total_num_heads"] = n_heads
        # 如果模型没有单独声明 kv head 数，就默认与 attention head 数相同。
        hf_quant_config["total_num_kv_heads"] = (
            n_kv_heads if n_kv_heads is not None else n_heads
        )

    # -------------------- 如果 HF 配置里已经带了足够的量化信息，直接据此构造配置 --------------------
    if hf_quant_config is not None:
        # modelopt_mixed 比较特殊：
        # 新 checkpoint 可能直接把 quantized_layers 写进 config.json，
        # 老 checkpoint 则可能只放在单独的 hf_quant_config.json 里。
        # 如果这里缺少 quantized_layers，就继续走后面的“按文件读取”路径。
        if (
    ...
    # -------------------- 处理 bitsandbytes 这种运行时注入量化配置的特例 --------------------
    # bitsandbytes 可以接受空配置，由量化类自己补默认值。
    if model_config.quantization == "bitsandbytes":
        return quant_cls.from_config({})
    # 先尝试从 ModelScope 下载 JSON 配置；下载不到时继续使用原始 model 路径。
    model_name_or_path = (
    ...
    # 判断当前路径是不是本地目录，后面据此决定是否需要从 HF 拉配置文件。
    is_local = os.path.isdir(model_name_or_path)
    if not is_local:
        # -------------------- 远端模型场景：先把所需 JSON 配置文件下载到本地缓存 --------------------
        # 用文件锁避免多进程并发下载同一份模型配置。
        ...
        # 本地目录场景直接把该目录当作配置搜索根目录。
        hf_folder = model_name_or_path

    # -------------------- 枚举该量化方法接受的配置文件名 --------------------
    # 不同量化后端会约定不同的配置文件名，这里统一向量化类查询。
    possible_config_filenames = quant_cls.get_config_filenames()

    # 如果该量化实现根本不需要单独的配置文件，就退回默认构造。
    if not possible_config_filenames:
        return quant_cls()

    # 收集目录下的所有 JSON 文件，稍后再筛选出命中的量化配置文件。
    config_files = glob.glob(os.path.join(hf_folder, "*.json"))

    # 只保留文件名后缀命中该量化类约定列表的 JSON 文件。
    quant_config_files = [
        f for f in config_files if any(f.endswith(x) for x in possible_config_filenames)
    ]
    # 一个都没找到，说明模型目录里缺少这一路量化所需的配置文件。
    if len(quant_config_files) == 0:
        raise ValueError(f"Cannot find the config file for {model_config.quantization}")
    # 命中过多个文件，说明目录状态不明确，直接报错避免误用错误配置。
    ...
    # -------------------- 读取最终命中的量化配置文件，并按量化方法做末端分支 --------------------
    # 走到这里时，量化配置文件已经唯一确定。
    quant_config_file = quant_config_files[0]
    # 把配置文件读取成 Python 字典，后续统一交给量化类解析。
    with open(quant_config_file) as f:
        config = json.load(f)

        # bitsandbytes 需要补一个 adapter_name_or_path，指向原始模型路径。
        if model_config.quantization == "bitsandbytes":
            config["adapter_name_or_path"] = model_config.model
        # modelopt / modelopt_mixed 还要进一步校验 producer，避免错误读入其他格式配置。
        elif model_config.quantization in ("modelopt", "modelopt_mixed"):
            if config.get("producer", {}).get("name") == "modelopt":
                # producer 校验通过，说明这是合法的 modelopt 配置。
                return quant_cls.from_config(config)
            else:
                # producer 不匹配时，直接拒绝使用，避免后续加载过程出现更隐蔽的问题。
                raise ValueError(
                    f"Unsupported quantization config"
                    f" found for {model_config.quantization} in {f}."
                )

    # 默认分支：把读取到的配置字典交给量化类标准解析入口。
    return quant_cls.from_config(config)
