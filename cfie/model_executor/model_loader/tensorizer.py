...
from cfie.config import ModelConfig, ParallelConfig, CfieConfig, set_current_cfie_config
from cfie.logger import init_logger
from cfie.model_executor.layers.vocab_parallel_embedding import VocabParallelEmbedding
...
class TensorizerConfig(MutableMapping):
    ...
    cfie_tensorized: bool | None = None
    ...
    Attributes:
        ...
        cfie_tensorized: If True, indicates that the serialized model is a 
        ...
        encryption_keyfile: File path to a binary file containing a  
            ...
            examples/others/tensorize_cfie_model.py. 
...
def init_tensorizer_model(
    tensorizer_config: TensorizerConfig, cfie_config: CfieConfig
) -> nn.Module:
    ...
    with meta_tensor_mode(), set_current_cfie_config(cfie_config, check_compile=True):
        return tensorizer_config.model_class(cfie_config=cfie_config)
...
def deserialize_tensorizer_model(
    ...
    del model.cfie_tensorized_marker
...
def tensorizer_weights_iterator(
    ...
    logger.warning(
        ...
        "load times. See the "
        "examples/others/tensorize_cfie_model.py example script "
...
def is_cfie_tensorized(tensorizer_config: "TensorizerConfig") -> bool:
    """
    ...
    if tensorizer_config.cfie_tensorized:
        logger.warning(
            "Please note that newly serialized vLLM models are automatically "
            "inferred as vLLM models, so setting cfie_tensorized=True is "
            "only necessary for models serialized prior to this change."
        )
        return True
    return ".cfie_tensorized_marker" in deserializer
...
def serialize_cfie_model(
    model: nn.Module,
    tensorizer_config: TensorizerConfig,
    model_config: "ModelConfig",
) -> nn.Module:
    model.register_parameter(
        "cfie_tensorized_marker",
...
def tensorize_cfie_model(
    ...
    engine = LLMEngine.from_cfie_config(engine_config)
...
def tensorize_lora_adapter(lora_path: str, tensorizer_config: TensorizerConfig):
    ...
    from cfie.lora.utils import get_adapter_absolute_path
