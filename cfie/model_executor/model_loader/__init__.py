...
from cfie.model_executor.model_loader.base_loader import BaseModelLoader
from cfie.model_executor.model_loader.bitsandbytes_loader import BitsAndBytesModelLoader
from cfie.model_executor.model_loader.default_loader import DefaultModelLoader
from cfie.model_executor.model_loader.dummy_loader import DummyModelLoader
from cfie.model_executor.model_loader.gguf_loader import GGUFModelLoader
from cfie.model_executor.model_loader.runai_streamer_loader import (
    RunaiModelStreamerLoader,
)
from cfie.model_executor.model_loader.sharded_state_loader import ShardedStateLoader
from cfie.model_executor.model_loader.tensorizer_loader import TensorizerLoader
...
# 注册某种 `load_format` 对应的模型加载器实现。
def register_model_loader(load_format: str):
    """Register a customized cfie model loader.

    When a load format is not supported by cfie, you can register a customized
    ...
    Examples:
        >>> from cfie.config.load import LoadConfig
        >>> from cfie.model_executor.model_loader import (
        ...     get_model_loader,
...
# 根据当前 `load_config.load_format` 选出具体的模型加载器。
...
def get_model(
    *,
    ...
    loader = get_model_loader(load_config or cfie_config.load_config)
    if model_config is None:
        model_config = cfie_config.model_config
    return loader.load_model(
        cfie_config=cfie_config, model_config=model_config, prefix=prefix
    )
