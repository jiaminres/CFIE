...
class TensorizerLoader(BaseModelLoader):
    ...
    def _load_model_serialized_cpu(
        ...
        examples/others/tensorize_cfie_model.py) This should still
        ...
        with set_default_torch_dtype(model_config.dtype):
            with torch.device(device_config.device):
                model = initialize_model(cfie_config=cfie_config, prefix=prefix)
    ...
    def load_weights(self, model: nn.Module, model_config: ModelConfig) -> None:
        ...
        if is_cfie_tensorized(self.tensorizer_config):
            ...
            with set_default_torch_dtype(model_config.dtype):
                with torch.device(device_config.device):
                    model = init_tensorizer_model(
                        tensorizer_config=tensorizer_config, cfie_config=cfie_config
                    )
            self.load_weights(model, model_config)
            return model
        return self._load_model_serialized_cpu(cfie_config=cfie_config, prefix=prefix)
