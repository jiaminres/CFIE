...
from cfie.transformers_utils.repo_utils import list_filtered_repo_files
...
class DefaultModelLoader(BaseModelLoader):
    ...
    def load_weights(self, model: nn.Module, model_config: ModelConfig) -> None:
        # 只有 torchao 量化需要在真正加载前调整 safetensors 的加载策略。
        ...
        # 收集模型中理论上应该被初始化的参数名集合。
        # 这个集合主要给后面的严格检查使用。
        weights_to_load = {name for name, _ in model.named_parameters()}

        # 真正遍历 checkpoint 权重并调用模型对象自己的 load_weights()。
        # 对当前 Qwen3.5 MoE 路径，会进入 Qwen3_5MoeForConditionalGeneration.load_weights()。
        ...
        # 如果是非量化模型，则进一步检查是否存在“完全没从 checkpoint 初始化”的参数。
