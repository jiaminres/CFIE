...
from cfie import _oink_ops, envs
...
def _should_use_internal_torch_compile() -> bool:
    try:
        from cfie.config import CompilationMode, get_current_cfie_config_or_none
    except Exception:
        return True
    ...
    if cfie_config.model_config.enforce_eager:
        return False
...
class RMSNorm(CustomOp):
    ...
    def forward_cuda(
        ...
        if (
            ...
            and x.dim() >= 2
            and self.has_weight
            and not cfie_is_batch_invariant()
...
class GemmaRMSNorm(CustomOp):
    ...
    def forward_cuda(
        ...
        if not _should_use_internal_torch_compile():
            return self.forward_native(x, residual)
...
class RMSNormGated(CustomOp):
    ...
    def forward_cuda(
        ...
        from cfie.model_executor.layers.fla.ops.layernorm_guard import rmsnorm_fn
