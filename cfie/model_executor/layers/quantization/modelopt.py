...
from cfie.model_executor.kernels.linear import init_fp8_linear_kernel
...
from cfie.model_executor.layers.fused_moe.oracle.mxfp8 import (
    MxFp8MoeBackend,
...
from cfie.model_executor.layers.quantization.utils.mxfp8_utils import (
    MXFP8_BLOCK_SIZE,
    MXFP8_SCALE_DTYPE,
    MXFP8_VALUE_DTYPE,
    Mxfp8LinearBackend,
...
from cfie.model_executor.layers.quantization.utils.nvfp4_utils import (
    apply_nvfp4_linear,
...
from cfie.utils.flashinfer import flashinfer_trtllm_fp8_block_scale_moe
...
class ModelOptQuantConfigBase(QuantizationConfig):
    ...
    def apply_cfie_mapper(self, hf_to_cfie_mapper: "WeightsMapper"):
        if len(self.exclude_modules) > 0:
            # This is a workaround for the weights remapping issue:
            # https://github.com/cfie-project/cfie/issues/28072
            ...
            self.exclude_modules = hf_to_cfie_mapper.apply_list(new_exclude_modules)
...
class ModelOptMixedPrecisionConfig(ModelOptQuantConfigBase):
    ...
    def apply_cfie_mapper(self, hf_to_cfie_mapper: "WeightsMapper"):
        super().apply_cfie_mapper(hf_to_cfie_mapper)
        if self.quantized_layers:
            self.quantized_layers = hf_to_cfie_mapper.apply_dict(self.quantized_layers)
