...
from cfie.model_executor.layers.linear import LinearMethodBase
...
class GPTQConfig(QuantizationConfig):
    ...
    def get_quant_method(
        ...
        if isinstance(layer, FusedMoE):
            # Honor dynamic negative-match rules (for example `-:.*mtp.*`) so
            # draft-only MoE branches can stay in full precision even when the
            # main model uses GPTQ.
