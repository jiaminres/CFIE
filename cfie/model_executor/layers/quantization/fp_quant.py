...
from cfie.model_executor.layers.quantization.qutlass_utils import to_blocked
...
def quantized_forward(
    ...
    if forward_dtype == "mxfp4":
        x_flat_q, x_flat_scales = torch.ops.cfie.fused_quantize_mx(
            x_flat, forward_hadamard_matrix, forward_method
        )
        y = torch.ops.cfie.matmul_mxf4_bf16(
            x_flat_q,
            qweight,
            x_flat_scales,
            weight_scales,
        ...
        x_flat_q, x_flat_scales = torch.ops.cfie.fused_quantize_nv(
            x_flat, forward_hadamard_matrix, act_global_scale
        )
        y = torch.ops.cfie.matmul_nvf4_bf16(
            x_flat_q,
            qweight,
            x_flat_scales,
            weight_scales,
