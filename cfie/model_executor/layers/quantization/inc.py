...
class INCConfig(QuantizationConfig):
    ...
    def from_config(cls, config: dict[str, Any]) -> "INCConfig":
        return cls(
            ...
            backend=cls.get_from_keys_or(config, ["backend", "cfie_backend"], "auto"),
        )
    ...
    def apply_cfie_mapper(self, hf_to_cfie_mapper: "WeightsMapper"):
        if self.block_name_to_quantize is not None:
            self.block_name_to_quantize = hf_to_cfie_mapper.apply_list(
                self.block_name_to_quantize
            )
        if self.extra_config is not None:
            self.extra_config = hf_to_cfie_mapper.apply_dict(self.extra_config)
    ...
    def apply_awq_quant_layer(self, layer, prefix: str, backend: str = "auto"):
        ...
        if backend == "auto" or "marlin" in backend:
            ...
            use_marlin = False
        if use_marlin:
            from cfie.model_executor.layers.quantization.awq_marlin import (
                AWQMarlinConfig,
            ...
            from cfie.model_executor.layers.quantization.awq import (
                AWQConfig,
                AWQLinearMethod,
            )
    ...
    def apply_gptq_quant_layer(self, layer, prefix: str, backend: str = "auto"):
        ...
        if backend == "auto" or "marlin" in backend:
            ...
            use_marlin = False
        if use_marlin:
            from cfie.model_executor.layers.quantization.gptq_marlin import (
                GPTQMarlinConfig,
            ...
            from cfie.model_executor.layers.quantization.gptq import (
                GPTQConfig,
                GPTQLinearMethod,
            )
        ...
        if isinstance(layer, FusedMoE):
            if use_marlin:
                ...
                from cfie.model_executor.layers.quantization.moe_wna16 import (
                    MoeWNA16Config,
                )
