...
from cfie.model_executor.layers.quantization.base_config import (  # noqa: E501
    QuantizationConfig,
    QuantizeMethodBase,
)
from cfie.model_executor.layers.quantization.kv_cache import BaseKVCacheMethod
from cfie.model_executor.layers.quantization.quark.quark_moe import (  # noqa: E501
    QuarkMoEMethod,
)
from cfie.model_executor.layers.quantization.quark.schemes import (
    QuarkOCP_MX,
    QuarkScheme,
    QuarkW4A8_MXFP4_FP8,
    QuarkW8A8Fp8,
...
class QuarkConfig(QuantizationConfig):
    ...
    def apply_cfie_mapper(  # noqa: B027
        self, hf_to_cfie_mapper: "WeightsMapper"
    ):
        """
        Interface for models to update module names referenced in
        quantization configs in order to reflect the cfie model structure

        :param hf_to_cfie_mapper: maps from hf model structure (the assumed
            structure of the qconfig) to cfie model structure
        """
        quant_config_with_hf_to_cfie_mapper = {}

        for k, v in self.quant_config.items():
            if isinstance(v, list):
                quant_config_with_hf_to_cfie_mapper[k] = hf_to_cfie_mapper.apply_list(v)
            elif isinstance(v, dict):
                quant_config_with_hf_to_cfie_mapper[k] = hf_to_cfie_mapper.apply_dict(v)
            else:
                if isinstance(v, str):
                    mapped_v_list = hf_to_cfie_mapper.apply_list([v])
                    if mapped_v_list:
                        quant_config_with_hf_to_cfie_mapper[k] = mapped_v_list[0]
                else:
                    quant_config_with_hf_to_cfie_mapper[k] = v

        self.quant_config = quant_config_with_hf_to_cfie_mapper
