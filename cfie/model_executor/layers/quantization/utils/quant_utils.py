...
def pack_cols(
    ...
    # -------------------------------------------------------------
    # 1) 检查输入 q_w 的逻辑 shape
    # -------------------------------------------------------------
    # 这里要求输入必须是“已经解包后的普通二维矩阵”：
    #   shape = [size_k, size_n]
    #
    # 例如：
    # - size_k 可以是 grouped_k
    # - size_n 可以是 N_local
    #
    # 也就是说，每个元素当前都单独占一个 int32 槽位，
    # 还没有重新 packed 回低比特布局。
    assert q_w.shape == (size_k, size_n)

    # -------------------------------------------------------------
    # 2) 计算 pack_factor：一个 uint32 里能装多少个 low-bit 值
    # -------------------------------------------------------------
    # 例如：
    # - num_bits = 4  -> pack_factor = 8
    # - num_bits = 8  -> pack_factor = 4
    pack_factor = get_pack_factor(num_bits)

    # 因为要沿列方向打包，所以列数 size_n 必须能被 pack_factor 整除
    assert size_n % pack_factor == 0

    # 保存原 device，后面会先搬到 CPU/Numpy，再搬回原设备
    orig_device = q_w.device

    # -------------------------------------------------------------
    # 3) 转到 CPU + numpy，准备做位运算
    # -------------------------------------------------------------
    # astype(uint32) 是因为后面会用移位和按位或来拼 packed 值
    q_w = q_w.cpu().numpy().astype(numpy.uint32)
    ...
    # 含义：
    # - 行数 size_k 不变
    # - 列数缩成原来的 1/pack_factor
    #
    # 因为 pack 是沿“列方向”发生的：
    # 每 pack_factor 个原始列值，会被塞进一个 uint32
    ...
    # 含义是：
    # - 对于第 i 个子槽位
    # - 取原矩阵中第 i, i+pack_factor, i+2*pack_factor, ... 列
    # - 把这些值左移到当前 uint32 中对应的 bit 位置
    # - 再 OR 进结果 q_res
    #
    # 举例：
    # 若 num_bits = 4, pack_factor = 8
    #
    # 那么 q_res[:, j] 这个 uint32 里会装：
    #   bits [3:0]   <- q_w[:, 8*j + 0]
    #   bits [7:4]   <- q_w[:, 8*j + 1]
    #   bits [11:8]  <- q_w[:, 8*j + 2]
    #   ...
    #   bits [31:28] <- q_w[:, 8*j + 7]
    #
    # 所以 pack_cols 的本质就是：
    #   “把每行每 pack_factor 个列元素，重新塞进一个 uint32”
    for i in range(pack_factor):
        q_res |= q_w[:, i::pack_factor] << num_bits * i

    # -------------------------------------------------------------
    # 6) 转回 torch tensor，并搬回原 device
    # -------------------------------------------------------------
    # astype(int32)：
    # 后续在 PyTorch/CUDA 侧通常用 int32 tensor 来承载 packed 数据
    q_res = torch.from_numpy(q_res.astype(numpy.int32)).to(orig_device)

    # 保证 contiguous，方便后续 C++/CUDA kernel 读取
...
def unpack_cols(
    ...
    #
    # 含义：
    # 一个 32bit 整数里，可以 packed 多少个量化值
    pack_factor = get_pack_factor(num_bits)

    # 逻辑列数 size_n 必须能被 pack_factor 整除
    assert size_n % pack_factor == 0

    # -------------------------------------------------------------
    # 2) 检查 packed 输入张量 shape 是否符合预期
    # -------------------------------------------------------------
    # unpack_cols 约定：
    #   packed_q_w 的 shape 必须是
    #   (size_k, size_n // pack_factor)
    #
    # 也就是说：
    # - 行数还是逻辑上的 size_k
    # - 列数被压缩成了原来的 1/pack_factor
    #
    # 例如：
    #   原始逻辑矩阵 shape = [grouped_k, N_local]
    #   若 4bit，则 pack_factor=8
    #   packed 后 shape = [grouped_k, N_local/8]
    ...
    # 保存原始 device，后面解包过程中会先搬到 CPU 再搬回去
    orig_device = packed_q_w.device

    # -------------------------------------------------------------
    # 3) 转到 CPU + numpy 处理
    # -------------------------------------------------------------
    # 这里不是运行时主 kernel，而是“加载后预处理”阶段，
    # 所以用 numpy 逐步解包是可以接受的。
    #
    # astype(uint32)：
    # 因为后面要做按位与、右移等位运算
    packed_q_w_cpu = packed_q_w.cpu().numpy().astype(numpy.uint32)

    # 创建解包后的结果矩阵，shape 恢复成 [size_k, size_n]
    q_res = numpy.zeros((size_k, size_n), dtype=numpy.uint32)

    # 低 num_bits 位的掩码
    # 例如：
    # - 4bit -> mask = 0b1111 = 15
    # - 8bit -> mask = 0b11111111 = 255
    mask = (1 << num_bits) - 1

    # -------------------------------------------------------------
    # 4) 逐步把每个 uint32 里的 packed 值解出来
    # -------------------------------------------------------------
    # 核心思想：
    # packed_q_w_cpu 的每个元素里，沿“列方向” packed 了 pack_factor 个值。
    #
    # 例如 4bit 时，一个 uint32 里装 8 个值：
    #   [v0, v1, v2, v3, v4, v5, v6, v7]
    #
    # 这里每轮循环：
    # - vals = packed_q_w_cpu & mask
    #   取出当前最低 num_bits 的那个值
    ...
    # 注意：
    # i::pack_factor 表示：
    #   第 i, i+pack_factor, i+2*pack_factor, ... 列
    #
    # 所以一个 packed 列会被拆回 pack_factor 个真实列。
    for i in range(pack_factor):
        # 取出当前最低 num_bits 位
        vals = packed_q_w_cpu & mask

        # 把已经取出的这一段从 packed_q_w_cpu 中右移掉
        packed_q_w_cpu >>= num_bits
    ...
    # -------------------------------------------------------------
    # 5) 转回 torch tensor，并搬回原 device
    # -------------------------------------------------------------
    # astype(int32)：
    # 后续代码通常把这些解包后的 low-bit 值当作 int32 张量继续处理
    q_res = torch.from_numpy(q_res.astype(numpy.int32)).to(orig_device)

    # 保证 contiguous，方便后续 kernel / C++ 扩展使用
    q_res = q_res.contiguous()
