...
from cfie.model_executor.layers.attention.attention import (
    _init_kv_cache_quant,
    get_attention_context,
    set_default_quant_scales,
    should_load_quant_weights,
)
from cfie.model_executor.layers.attention.kv_transfer_utils import (
...
from cfie.utils.flashinfer import has_flashinfer, has_nvidia_artifactory
from cfie.utils.math_utils import cdiv, round_down
...
from cfie.v1.attention.ops.common import cp_lse_ag_out_rs
from cfie.v1.attention.ops.dcp_alltoall import dcp_a2a_lse_reduce
from cfie.v1.attention.ops.merge_attn_states import merge_attn_states
from cfie.v1.attention.selector import get_attn_backend
...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        self.kv_cache = [
            torch.tensor([])
            for _ in range(
                get_current_cfie_config().parallel_config.pipeline_parallel_size
            )
        ]
        ...
        self.dcp_a2a = (
            ...
            and cfie_config.parallel_config.decode_context_parallel_size > 1
            and cfie_config.parallel_config.dcp_comm_backend == "a2a"
        )
        ...
        self._cfie_config = get_current_cfie_config()
    ...
    def forward(
        ...
        if self.calculate_kv_scales:
            torch.ops.cfie.maybe_calc_kv_scales(q, kv_c_normed, k_pe, self.layer_name)
        ...
        if self.use_direct_call:
            ...
            kv_cache_dummy_dep = torch.ops.cfie.unified_mla_kv_cache_update(
                kv_c_normed,
                k_pe,
                self.layer_name,
                self.kv_cache_dtype,
            ...
            if self.attn_backend.accept_output_buffer:
                output = torch.empty(output_shape, dtype=q.dtype, device=q.device)
                torch.ops.cfie.unified_mla_attention_with_output(
                    q,
                    kv_c_normed,
                    k_pe,
                    output,
                ...
                return torch.ops.cfie.unified_mla_attention(
                    q,
                    kv_c_normed,
                    k_pe,
                    self.layer_name,
    ...
    def process_weights_after_loading(self, act_dtype: torch.dtype):
        ...
        # If kv_b_proj_weight is unquantized, quantize it to mxfp4 if supported
        if self.is_aiter_triton_fp4_bmm_enabled:
            from cfie.model_executor.layers.quantization.quark.utils import (
    ...
    def get_kv_cache_spec(self, cfie_config: CfieConfig) -> KVCacheSpec:
        kv_cache_dtype = kv_cache_dtype_str_to_dtype(
            self.kv_cache_dtype, cfie_config.model_config
        )
        return MLAAttentionSpec(
            block_size=cfie_config.cache_config.block_size,
            num_kv_heads=1,
            head_size=self.head_size,
            dtype=kv_cache_dtype,
            cache_dtype_str=cfie_config.cache_config.cache_dtype,
        )
...
try:
    from cfie.cfie_flash_attn import (  # type: ignore[attr-defined]
    ...
    # On ROCm, cfie_flash_attn is not available, try upstream flash_attn instead.
    # On CUDA, cfie_flash_attn should always be available (built with vLLM),
...
def use_flashinfer_prefill() -> bool:
    ...
    if not (
        not cfie_config.attention_config.disable_flashinfer_prefill
        and has_flashinfer()
        and not cfie_config.attention_config.use_cudnn_prefill
        and current_platform.is_device_capability_family(100)
    ):
        return False

    return is_deepseek_r1_mla_compatible(cfie_config)
...
def use_cudnn_prefill() -> bool:
    ...
    return (
        has_flashinfer()
        and cfie_config.attention_config.use_cudnn_prefill
        and current_platform.is_device_capability_family(100)
...
def use_trtllm_ragged_deepseek_prefill() -> bool:
    ...
    if not (
        has_flashinfer()
        and cfie_config.attention_config.use_trtllm_ragged_deepseek_prefill
        and current_platform.is_device_capability_family(100)
    ):
        return False
...
class MLACommonMetadataBuilder(AttentionMetadataBuilder[M]):
    ...
    @staticmethod
    def determine_chunked_prefill_workspace_size(cfie_config: CfieConfig) -> int:
    ...
    def determine_prefill_query_data_type(
        ...
        """
        use_fp8 = (
            cfie_config.cache_config.cache_dtype.startswith("fp8")
            and cfie_config.attention_config.use_prefill_query_quantization
        ...
        if use_fp8:
            ...
            return fp8_dtype
        elif cfie_config.attention_config.use_prefill_query_quantization:
            logger.info_once(
    ...
    def __init__(
        ...
        self.q_data_type = self.determine_prefill_query_data_type(
            cfie_config, self.model_config.dtype
        )
        ...
        self.chunked_prefill_workspace_size = (
            self.determine_chunked_prefill_workspace_size(cfie_config)
        )
        ...
        if self._use_fi_prefill:
            ...
            self._global_hyperparameters = infer_global_hyperparameters(
                get_per_layer_parameters(cfie_config, layer_names, MLACommonImpl)  # type: ignore[type-abstract]
            )
...
class MLACommonImpl(MLAAttentionImpl[M], Generic[M]):
    ...
    def __init__(
        ...
        if use_trtllm_ragged_deepseek_prefill():
            ...
            # flash_attn and the one from cfie_flash_attn. The former is used on
            # RoCM and the latter has an additional parameter to control
            # FA2 vs FA3
            self.flash_attn_varlen_func = flash_attn_varlen_func
            self.cfie_flash_attn_version = get_flash_attn_version(
                head_size=self.qk_head_dim
            )
            if self.cfie_flash_attn_version is not None:
                self.flash_attn_varlen_func = functools.partial(
                    flash_attn_varlen_func, fa_version=self.cfie_flash_attn_version
                )
            ...
            self._pad_v = self.cfie_flash_attn_version is None or not (
                (
                    self.cfie_flash_attn_version == 3
                    and device_capability is not None
                    and device_capability[0] == 9
                )
                or self.cfie_flash_attn_version == 4
            )

        self.dcp_world_size: int = -1

        self.cp_kv_cache_interleave_size: int = (
            get_current_cfie_config().parallel_config.cp_kv_cache_interleave_size
        )
    ...
    def _concat_k_nope_k_pe(
        ...
        if self._use_flashinfer_concat_mla_k:
            torch.ops.cfie.flashinfer_concat_mla_k(k, k_nope, k_pe)
        else:
