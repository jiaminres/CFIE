...
def create_static_sink_attention_backend(
    ...
    class StaticSinkAttentionBuilder(underlying_builder):  # type: ignore
        def __init__(
            ...
            self.num_sink_blocks = self.sink_len // cfie_config.cache_config.block_size
            self.max_num_blocks = cdiv(
                model_config.max_model_len, cfie_config.cache_config.block_size
            )
...
class StaticSinkAttention(Attention, CustomOp):
    ...
    def forward_native(
        ...
        if not self.sink_populated:
            ...
            torch.ops.cfie.maybe_populate_sink(self_kv_cache, self.layer_name)
