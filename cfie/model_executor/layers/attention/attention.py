...
from cfie.config import CacheConfig, get_current_cfie_config
...
from cfie.model_executor.layers.quantization.base_config import QuantizeMethodBase
...
if TYPE_CHECKING:
    from cfie.model_executor.layers.attention import MLAAttention
...
class Attention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        self.kv_cache_torch_dtype = kv_cache_dtype_str_to_dtype(
            kv_cache_dtype, cfie_config.model_config
        )
        ...
        self.kv_cache = [
            torch.tensor([])
            for _ in range(cfie_config.parallel_config.pipeline_parallel_size)
        ]
    ...
    def forward(
        ...
        context using
        `cfie.forward_context.get_forward_context().attn_metadata`.
        """
        if self.calculate_kv_scales:
            torch.ops.cfie.maybe_calc_kv_scales(query, key, value, self.layer_name)
        ...
        if self.use_output:
            ...
            if self.use_direct_call:
                ...
                if (
                    ...
                    kv_cache_dummy_dep = torch.ops.cfie.unified_kv_cache_update(
                        key, value, self.layer_name
                    )
                torch.ops.cfie.unified_attention_with_output(
                    query,
                    key,
                    value,
                    output,
            ...
            if self.use_direct_call:
                ...
                return torch.ops.cfie.unified_attention(
    ...
    def get_kv_cache_spec(self, cfie_config: CfieConfig) -> KVCacheSpec:
        ...
        if self.sliding_window is not None:
            assert not cfie_config.model_config.use_mla, (
