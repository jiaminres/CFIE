...
class SiluAndMul(CustomOp):
    ...
    形状约定：
        x: (num_tokens, 2 * d) 或 (batch_size, seq_len, 2 * d)
        return: (num_tokens, d) 或 (batch_size, seq_len, d)
    """
    ...
    def __init__(self, *, compile_native: bool = True):
        # -------------------- 先初始化 CustomOp 基类 --------------------
        # `compile_native` 控制是否允许基类为 native 路径做编译相关处理。
    ...
    def forward_native(x: torch.Tensor) -> torch.Tensor:
        """与 forward() 等价的 PyTorch 原生实现。"""
        # -------------------- 先根据最后一维对输入做等分 --------------------
        # SwiGLU 约定最后一维前半段是 gate，后半段是 up/value。
        d = x.shape[-1] // 2
        # 对前半段做 SiLU，再与后半段逐元素相乘，输出维度减半。
        return F.silu(x[..., :d]) * x[..., d:]

    def forward_cuda(self, x: torch.Tensor) -> torch.Tensor:
        # -------------------- 先计算输出张量形状 --------------------
        # 输入最后一维是 2*d，输出最后一维只保留乘法结果 d。
        d = x.shape[-1] // 2
        # 保留除最后一维外的所有 batch/token 维度，只把最后一维改成 d。
        output_shape = x.shape[:-1] + (d,)
        # 在与输入相同 dtype / device 上预先分配输出张量，交给 fused kernel 原地写入。
        out = torch.empty(output_shape, dtype=x.dtype, device=x.device)
        # 调用底层 CUDA 自定义算子，把结果直接写入 out。
        self.op(out, x)
        # 返回 fused kernel 产出的最终结果。
        return out

    def forward_xpu(self, x: torch.Tensor) -> torch.Tensor:
        # -------------------- XPU 路径当前复用 CUDA 风格实现 --------------------
        # 两者都走 `self.op(out, x)` 这一套 fused kernel 调用接口。
