...
from cfie._custom_ops import cpu_fused_moe, cpu_prepack_moe_weight
...
from cfie.model_executor.layers.quantization.utils.layer_utils import replace_parameter
...
def _swigluoai_forward_native(
    ...
    which would trigger get_current_cfie_config() before config is set.
    """
    gate, up = x[..., ::2], x[..., 1::2]
...
# classes, which would call get_current_cfie_config() before config is set.
...
class CPUFusedMOE:
    ...
    def forward_torch(
        ...
        layer_id = id(layer)
        torch.ops.cfie.cpu_fused_moe_torch(
            layer_id,
            output,
            input,
            topk_weights,
