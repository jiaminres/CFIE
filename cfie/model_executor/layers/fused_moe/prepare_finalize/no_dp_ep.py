...
def _quantize_input(
    ...
    # 某些后端会把“输入量化 + experts 计算”合并进一个 kernel，
    # 这时把输入量化延后到 MoE kernel 内部处理。
...
class MoEPrepareAndFinalizeNoDPEPModular(mk.FusedMoEPrepareAndFinalizeModular):
    # 不启用 DP/EP 时的 modular prepare/finalize 实现。
    @property
    ...
    def prepare(
        ...
        if apply_router_weight_on_input:
            ...
            # TODO: 当前只支持 topK=1，后续需要扩展到 topK>1。
            assert topk == 1, (
...
class MoEPrepareAndFinalizeNoDPEPMonolithic(mk.FusedMoEPrepareAndFinalizeMonolithic):
    # 不启用 DP/EP 时的 monolithic prepare/finalize 实现。
    @property
    def activation_format(self) -> mk.FusedMoEActivationFormat:
        return mk.FusedMoEActivationFormat.Standard
...
def make_moe_prepare_and_finalize_no_dp_ep(
    ...
    # 按执行模式返回 no-DP/EP 的 prepare/finalize 实现。
    return (
