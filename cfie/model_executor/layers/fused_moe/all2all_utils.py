...
from cfie.model_executor.layers.fused_moe.flashinfer_a2a_prepare_finalize import (
...
from cfie.utils.import_utils import has_deep_ep, has_mori, has_nixl_ep
...
def maybe_roundup_layer_hidden_size(
    ...
    """
    按当前 MoE 并行配置检查是否需要把层的 hidden size 向上对齐。
    ...
    返回：
    - 若当前 all2all backend 对 hidden size 有对齐要求，则返回对齐后的值
    - 否则返回原始 hidden size
    """
    # DeepEP high-throughput 路径可能要求 hidden size 满足特定对齐约束。
    if moe_parallel_config.use_deepep_ht_kernels:
        hidden_size = DeepEPHTPrepareAndFinalize.maybe_roundup_layer_hidden_size(
            hidden_size, act_dtype
        )

    # DeepEP low-latency 路径有自己的一套 hidden size 对齐规则。
    if moe_parallel_config.use_deepep_ll_kernels:
        hidden_size = DeepEPLLPrepareAndFinalize.maybe_roundup_layer_hidden_size(
            hidden_size
        )

    # Nixl EP 路径同样可能要求对 hidden size 做额外补齐。
