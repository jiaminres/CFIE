...
[`MultiModalDummyOptionsBuiltins`][cfie.config.multimodal.MultiModalDummyOptionsBuiltins].
"""
...
class MultiModalConfig:
    ...
    """可选地覆盖多模态编码器在使用视觉 Transformer 时的 attention 后端。
    可接受 `cfie.v1.attention.backends.registry.AttentionBackendEnum`
    中的任意枚举值（例如 `FLASH_ATTN`）。"""
