...
from cfie.config import LoadConfig
...
from cfie.transformers_utils.config import get_hf_text_config
from cfie.utils.hashing import safe_hash
from cfie.utils.import_utils import LazyLoader, has_arctic_inference
...
class SpeculativeConfig:
    ...
    def hf_config_override(hf_config: PretrainedConfig) -> PretrainedConfig:
        ...
        if hf_config.model_type in ("qwen3_5", "qwen3_5_moe"):
            ...
            # 当前 122B-A10B checkpoint 的原始配置是：
            # - model_type = "qwen3_5_moe"
            # - architectures = ["Qwen3_5MoeForConditionalGeneration"]
            # 当用户传入 --spec-method mtp 时，这里会把 draft HF 配置改写为：
            # - model_type = "qwen3_5_mtp"
            # - architectures = ["Qwen3_5MoeMTP"]
            # - n_predict = mtp_num_hidden_layers = 1
    ...
    def __post_init__(self):
        ...
        if self.model is None and self.num_speculative_tokens is not None:
            if self.method == "mtp":
                ...
                # 当前启动命令没有单独提供 speculative model，
                # 只给了 --spec-method mtp 和 --num-speculative-tokens 1；
                # 因此这里会复用 target model 路径来构造 draft/MTP 模型。
        ...
        if self.method in ("ngram", "ngram_gpu"):
            ...
            from cfie.transformers_utils.configs.extract_hidden_states import (
            ...
            if self.model is not None:
                ...
                if self.method in ("eagle", "eagle3"):
                    from cfie.transformers_utils.configs import SpeculatorsConfig
                    from cfie.transformers_utils.configs.eagle import EAGLEConfig
                ...
                if n_predict is not None:
                    # 当前 Qwen3.5 MTP 的 n_predict=1，
                    # 与当前命令的 num_speculative_tokens=1 完全一致，
                    # 因此这里会直接通过，不需要做多层 MTP 复用。
