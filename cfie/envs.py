...
if TYPE_CHECKING:
    ...
    VLLM_CACHE_ROOT: str = os.path.expanduser("~/.cache/cfie")
    VLLM_CONFIG_ROOT: str = os.path.expanduser("~/.config/cfie")
    VLLM_USAGE_STATS_SERVER: str = "https://stats.cfie.ai"
    ...
    VLLM_ENABLE_V1_MULTIPROCESSING: bool = os.name != "nt"
    ...
    try:
        ...
        if parsed.scheme:
            raise ValueError(
                ...
                "check the warning in: https://docs.cfie.ai/en/stable/serving/env_vars.html"
            ) from None
...
environment_variables: dict[str, Callable[[], Any]] = {
    ...
    # If set, cfie will use precompiled binaries (*.so)
    ...
    # If set, cfie will print verbose logs during installation
    "VERBOSE": lambda: bool(int(os.getenv("VERBOSE", "0"))),
    # Root directory for vLLM configuration files
    # Defaults to `~/.config/cfie` unless `XDG_CONFIG_HOME` is set
    # Note that this not only affects how cfie finds its configuration files
    # during runtime, but also affects how cfie installs its configuration
    # files during **installation**.
    "VLLM_CONFIG_ROOT": lambda: os.path.expanduser(
        os.getenv(
            "VLLM_CONFIG_ROOT",
            os.path.join(get_default_config_root(), "cfie"),
        )
    ),
    # ================== Runtime Env Vars ==================
    # Root directory for vLLM cache files
    # Defaults to `~/.cache/cfie` unless `XDG_CACHE_HOME` is set
    "VLLM_CACHE_ROOT": lambda: os.path.expanduser(
        os.getenv(
            "VLLM_CACHE_ROOT",
            os.path.join(get_default_cache_root(), "cfie"),
        )
    ),
    ...
    # when `VLLM_NCCL_SO_PATH` is not set, cfie will try to find the nccl
    ...
    # cfie cold compile times by O(1s)
    ...
    # Force cfie to always load AOT compiled models from disk. Failure
    ...
    "VLLM_USAGE_STATS_SERVER": lambda: os.environ.get(
        "VLLM_USAGE_STATS_SERVER", "https://stats.cfie.ai"
    ),
    ...
    # If set to 0, cfie will not configure logging
    # If set to 1, cfie will configure logging using the default configuration
    ...
    # If set, cfie will log stats at this interval in seconds
    # If not set, cfie will log stats every 10 seconds.
    ...
    "VLLM_ASSETS_CACHE": lambda: os.path.expanduser(
        os.getenv(
            "VLLM_ASSETS_CACHE",
            os.path.join(get_default_cache_root(), "cfie", "assets"),
        )
    ),
    ...
    "VLLM_XLA_CACHE_PATH": lambda: os.path.expanduser(
        os.getenv(
            "VLLM_XLA_CACHE_PATH",
            os.path.join(get_default_cache_root(), "cfie", "xla_cache"),
        )
    ),
    ...
    # See https://github.com/cfie-project/cfie/blob/a9b15c606fea67a072416ea0ea115261a2756058/cfie/distributed/device_communicators/custom_all_reduce_utils.py#L101-L108 for details. # noqa
    ...
    "VLLM_ENABLE_V1_MULTIPROCESSING": lambda: bool(
        int(os.getenv("VLLM_ENABLE_V1_MULTIPROCESSING", "0" if os.name == "nt" else "1"))
    ),
    "VLLM_LOG_BATCHSIZE_INTERVAL": lambda: float(
        os.getenv("VLLM_LOG_BATCHSIZE_INTERVAL", "-1")
    ),
    "VLLM_DISABLE_COMPILE_CACHE": disable_compile_cache,
    # If set, cfie will run in development mode, which will enable
    ...
    # defined in ``cfie.ray.ray_env`` (VLLM_, etc.).  Example: "MYLIB_,OTHER_"
    ...
    # defined in ``cfie.ray.ray_env`` (PYTHONHASHSEED).
    ...
    # https://github.com/cfie-project/cfie/issues/35772
    ...
    # cfie should use flashinfer fused allreduce. The variable should be a
    ...
    # KV Cache layout used throughout cfie.
    ...
    # Allows cfie to find tuned config under customized folder
    ...
    #     Safe for multiple cfie serve processes accessing the same torch compile cache.
...
def compile_factors() -> dict[str, object]:
    ...
    from cfie.config.utils import normalize_value
