...
from cfie.config import CUDAGraphMode, ParallelConfig, CfieConfig
...
from cfie.v1.worker.ubatch_utils import UBatchSlices
...
class ForwardContext:
    ...
    # 保存“不可编译层”的名字到层对象的映射
    # 常用于自定义 op 在运行时根据 layer_name 找回对应层实例
    no_compile_layers: dict[str, Any]

    # 当前 forward 所使用的 attention 元数据
    #
    # 两种可能：
    # 1. Dict[str, AttentionMetadata]
    #    普通 v1 路径
    #    key 是每个 attention 层的 layer_name
    #    value 是该层对应的 attention metadata
    #
    # 2. List[Dict[str, AttentionMetadata]]
    #    DBO 场景
    #    长度通常为 2，分别对应两个 microbatch
    #
    # 这个字段会在每次 forward 时动态设置
    attn_metadata: dict[str, AttentionMetadata] | list[dict[str, AttentionMetadata]]

    # 当前 forward 对应的 slot mapping
    #
    # 两种可能：
    # 1. Dict[str, torch.Tensor]
    # 2. List[Dict[str, torch.Tensor]]
    #
    # 通常用于把逻辑上的 token / sequence 映射到 KV cache 或状态缓存里的槽位
    slot_mapping: dict[str, torch.Tensor] | list[dict[str, torch.Tensor]]

    """
    v1 时类型为 Dict[str, AttentionMetadata]，
    表示从每个 attention 层名字映射到其 attention metadata。

    DBO 时类型为 List[Dict[str, AttentionMetadata]]，
    列表长度通常为 2，每个元素对应一个 microbatch。

    该字段在每次 forward 时动态设置。
    """

    # 当前 forward 使用的 virtual engine 编号
    # 主要用于区分不同虚拟执行流 / 不同 kv_cache 分组
    # TODO: 当所有 virtual_engine 共用同一份 kv cache 后可移除
    virtual_engine: int

    # Data Parallel 相关的元数据
    # 每次 forward 动态设置
    dp_metadata: DPMetadata | None = None

    # 当前运行时采用哪种 cudagraph 模式：
    # - FULL
    # - PIECEWISE
    # - NONE
    # 默认 NONE，表示不使用 cudagraph
    cudagraph_runtime_mode: CUDAGraphMode = CUDAGraphMode.NONE

    # 当前 batch 的描述信息
    batch_descriptor: BatchDescriptor | None = None

    # 当前 batch 被切分成的微批（ubatch）切片信息
    ubatch_slices: UBatchSlices | None = None

    # 若为 True，则跳过 compiled model 调用
    # 例如直接调用 .forward()，而不走 torch.compile 后的路径
    skip_compiled: bool = False

    # 为了避免 torch.compile 冷启动时把字符串常量硬编码进图中，
    # 这里专门保存本次 forward 过程中所有 MoE 层的名字列表
    #
    # 背景：
    # 当前 vllm.moe_forward / vllm.moe_forward_shared 自定义算子
    # 还会把层名字字符串作为运行时信息用到。
    # 如果直接把字符串写进图，会影响 compile 行为。
    #
    # 解决方式：
    # - all_moe_layers 保存本次 forward 需要的所有 MoE 层名字
    # - moe_layer_index 记录当前已经取到了第几个
    #
    # 这样自定义算子运行时就能按顺序从这里取名字，而不是把字符串写死进图
    #
    # 注意：
    # 这依赖于各个 MoE custom op 的执行顺序保持不变
    ...
    # 额外可扩展的 kwargs 容器
    # 用于在不改 dataclass 字段定义的情况下，附加更多运行时上下文
    additional_kwargs: dict[str, Any] = field(default_factory=dict)

    def __post_init__(self):
        # dataclass 初始化完成后，检查 cudagraph runtime mode 是否合法
...
def create_forward_context(
    ...
    if cfie_config.compilation_config.fast_moe_cold_start:
        all_moe_layers = cfie_config.compilation_config.static_all_moe_layers
    else:
        all_moe_layers = None

    return ForwardContext(
        no_compile_layers=cfie_config.compilation_config.static_forward_context,
...
def set_forward_context(
    ...
    if (
            ...
            and cfie_config.parallel_config.is_moe_model is not False
        ...
        dp_metadata = DPMetadata.make(
            cfie_config.parallel_config, num_tokens or 0, num_tokens_across_dp
        )
