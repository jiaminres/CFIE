...
from cfie.utils.platform_utils import cuda_get_device_properties
...
class UsageContext(str, Enum):
    ...
    # 通过 Python `LLM` 类直接创建和使用引擎的场景。
    LLM_CLASS = "LLM_CLASS"
    # 通用 API server 场景。
    API_SERVER = "API_SERVER"
    # OpenAI 兼容 API server 场景。
    OPENAI_API_SERVER = "OPENAI_API_SERVER"
    # OpenAI batch runner 批处理执行场景。
    OPENAI_BATCH_RUNNER = "OPENAI_BATCH_RUNNER"
    # 引擎内部直接创建 `LLMEngine` 的场景。
...
class UsageMessage:
    ...
    def __init__(self) -> None:
        ...
        self.cfie_version: str | None = None
    ...
    def _report_usage_once(
        ...
        self.cfie_version = VLLM_VERSION
