...
from cfie.model_executor.models import SupportsMultiModal
from cfie.multimodal import MULTIMODAL_REGISTRY, MultiModalDataDict, MultiModalUUIDDict
...
from cfie.multimodal.media import MEDIA_CONNECTOR_REGISTRY, MediaConnector
...
def __getattr__(name: str):
    if name == "resolve_hf_chat_template":
        from cfie.renderers.hf import resolve_chat_template

        warnings.warn(
            "`cfie.entrypoints.chat_utils.resolve_hf_chat_template` has been moved to "
            "`cfie.renderers.hf.resolve_chat_template`. "
...
def validate_chat_template(chat_template: Path | str | None):
    ...
    if chat_template is None:
        ...
        if (
            ...
            from cfie.transformers_utils.chat_templates.registry import (
                CHAT_TEMPLATES_DIR,
            )
