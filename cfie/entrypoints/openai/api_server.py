...
try:
    ...
    uvloop = None  # Windows 不支持 uvloop，回退到 asyncio
...
from cfie.config import VllmConfig
from cfie.engine.arg_utils import AsyncEngineArgs
from cfie.engine.protocol import EngineClient
from cfie.entrypoints.chat_utils import load_chat_template
from cfie.entrypoints.launcher import serve_http
from cfie.entrypoints.logger import RequestLogger
from cfie.entrypoints.openai.cli_args import make_arg_parser, validate_parsed_serve_args
from cfie.entrypoints.openai.models.protocol import BaseModelPath
from cfie.entrypoints.openai.models.serving import OpenAIServingModels
from cfie.entrypoints.openai.server_utils import (
...
from cfie.entrypoints.sagemaker.api_router import sagemaker_standards_bootstrap
from cfie.entrypoints.serve.elastic_ep.middleware import (
    ScalingMiddleware,
)
from cfie.entrypoints.serve.tokenize.serving import OpenAIServingTokenization
from cfie.entrypoints.utils import (
    cli_env_setup,
...
from cfie.tool_parsers import ToolParserManager
...
from cfie.utils.network_utils import is_valid_ipv6_address
from cfie.utils.system_utils import decorate_logs, set_ulimit
from cfie.v1.engine.exceptions import EngineDeadError, EngineGenerateError
from cfie.version import __version__ as VLLM_VERSION

prometheus_multiproc_dir: tempfile.TemporaryDirectory

# Cannot use __name__ (https://github.com/vllm-project/vllm/pull/4765)
logger = init_logger("cfie.entrypoints.openai.api_server")
...
async def build_async_engine_client(
    ...
    *,
    ...
    if os.getenv("VLLM_WORKER_MULTIPROC_METHOD") == "forkserver":
        ...
        multiprocessing.set_forkserver_preload(["cfie.v1.engine.async_llm"])
...
async def build_async_engine_client_from_engine_args(
    ...
    *,
    ...
    from cfie.v1.engine.async_llm import AsyncLLM
...
def build_app(
    ...
    from cfie.entrypoints.serve import register_vllm_serve_api_routers

    register_vllm_serve_api_routers(app)

    from cfie.entrypoints.openai.models.api_router import (
        attach_router as register_models_api_router,
    )

    register_models_api_router(app)

    from cfie.entrypoints.sagemaker.api_router import (
    ...
    if "generate" in supported_tasks:
        from cfie.entrypoints.openai.generate.api_router import (
            register_generate_api_routers,
        )

        register_generate_api_routers(app)

        from cfie.entrypoints.serve.disagg.api_router import (
            attach_router as attach_disagg_router,
        )

        attach_disagg_router(app)

        from cfie.entrypoints.serve.rlhf.api_router import (
            attach_router as attach_rlhf_router,
        )

        attach_rlhf_router(app)

        from cfie.entrypoints.serve.elastic_ep.api_router import (
    ...
    if "generate" in supported_tasks or "render" in supported_tasks:
        from cfie.entrypoints.serve.render.api_router import (
    ...
    if "transcription" in supported_tasks:
        from cfie.entrypoints.openai.speech_to_text.api_router import (
    ...
    if "realtime" in supported_tasks:
        from cfie.entrypoints.openai.realtime.api_router import (
    ...
    if any(task in POOLING_TASKS for task in supported_tasks):
        from cfie.entrypoints.pooling import register_pooling_api_routers
    ...
    if tokens := [key for key in (args.api_key or [envs.VLLM_API_KEY]) if key]:
        from cfie.entrypoints.openai.server_utils import AuthenticationMiddleware

        app.add_middleware(AuthenticationMiddleware, tokens=tokens)

    if args.enable_request_id_headers:
        from cfie.entrypoints.openai.server_utils import XRequestIdMiddleware
    ...
    if "realtime" in supported_tasks:
        ...
        from cfie.entrypoints.openai.realtime.metrics import (
...
async def init_app_state(
    ...
    if "generate" in supported_tasks:
        from cfie.entrypoints.openai.generate.api_router import init_generate_state
    ...
    if "realtime" in supported_tasks:
        from cfie.entrypoints.openai.realtime.api_router import init_realtime_state

        init_realtime_state(engine_client, state, args, request_logger, supported_tasks)

    if any(task in POOLING_TASKS for task in supported_tasks):
        from cfie.entrypoints.pooling import init_pooling_state
...
async def init_render_app_state(
    ...
    """
    from cfie.entrypoints.chat_utils import load_chat_template
    from cfie.entrypoints.openai.models.serving import OpenAIModelRegistry
    from cfie.entrypoints.serve.render.serving import OpenAIServingRender
...
def create_server_socket(addr: tuple[str, int]) -> socket.socket:
    ...
    # SO_REUSEPORT 在 Windows 上不可用，仅在支持的平台启用
...
if __name__ == "__main__":
    # Windows 上 multiprocessing spawn 模式需要在主模块入口调用 freeze_support。
    # 引擎 core 进程会重新 import 此模块，若缺少该调用会导致 RuntimeError。
