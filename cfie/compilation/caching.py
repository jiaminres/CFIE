...
from cfie.compilation.compiler_interface import get_inductor_factors
from cfie.config import CfieConfig, get_current_cfie_config
from cfie.config.utils import hash_factors
...
class CfieSerializableFunction(SerializableCallable):  # type: ignore[misc]
    """
    A wrapper around a compiled function by cfie. It will forward the tensor
    ...
    def __init__(
        ...
        self.shape_env = None
        self.cfie_backend = cfie_backend
    ...
    def serialize_compile_artifacts(
        cls, compiled_fn: "CfieSerializableFunction"
    ) -> bytes:
        import sympy
        ...
        if compiled_fn.cfie_backend:
            (
                standalone_compile_artifacts,
                sym_shape_indices_map,
                returns_tuple_map,
            ) = compiled_fn.cfie_backend.collect_standalone_compile_artifacts()
    ...
    @classmethod
    def deserialize_compile_artifacts(cls, data: bytes) -> "CfieSerializableFunction":
        ...
        # Fall back to standard CfieBackend.
        ...
        def optimized_call(*example_inputs: Any) -> Any:
            cfie_backend: CfieBackend = CfieBackend(
                cfie_config, state["prefix"], is_encoder
            )
            with tracing(TracingContext(fake_mode)), functorch_ctx:
                fn.optimized_call = cfie_backend(
                    state["graph_module"], compile_inputs
                ).optimized_call
                fn.cfie_backend = cfie_backend
    ...
    def finalize_loading(self, cfie_config: CfieConfig) -> None:
        ...
        cfie_backend = CfieBackend(cfie_config, self.prefix, self.is_encoder)
        with tracing(TracingContext(self._fake_mode)), functorch_ctx:
            result = cfie_backend(self.graph_module, list(self.example_inputs))
    ...
    @property
    def co_name(self) -> Literal["CfieSerializableFunction"]:
        """
        Used for depyf debugging.
        """
        return "CfieSerializableFunction"
...
def reconstruct_serializable_fn_from_mega_artifact(
...
) -> "CfieSerializableFunction":
    """Construct a CfieSerializableFunction from cached inductor artifacts.
    ...
    5. Returns the final CfieSerializableFunction
    ...
    Args:
        ...
        cfie_config: The vLLM configuration.
    ...
    Returns:
        A CfieSerializableFunction that can be called directly.
    """
    from cfie.compilation.backends import (
        CfieBackend,
        make_copy_and_call,
        wrap_with_cudagraph_if_needed,
    )
    from cfie.compilation.piecewise_backend import PiecewiseBackend
    ...
    if compilation_config.cudagraph_copy_inputs:
        ...
        input_buffers = [
            torch.empty_like(
                state["example_inputs"][idx], device=cfie_config.device_config.device
            )
...
def aot_compile_hash_factors(cfie_config: CfieConfig) -> list[str]:
    factors = []
    ...
    # 1. factors come from the cfie_config (it mainly summarizes how the
    #    model is created)
    config_hash = cfie_config.compute_hash()
