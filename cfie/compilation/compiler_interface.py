...
class CompilerInterface:
    ...
    def compute_hash(self, cfie_config: CfieConfig) -> str:
        ...
        See [`CfieConfig.compute_hash`][cfie.config.CfieConfig.compute_hash]
...
def is_compile_cache_enabled(
    cfie_additional_inductor_config: dict[str, Any],
) -> bool:
    cfie_inductor_config_disable_cache = cfie_additional_inductor_config.get(
    ...
    return (
        ...
        and not cfie_inductor_config_disable_cache
    )
...
def _patch_standalone_compile_atomic_save() -> None:
    ...
    if getattr(cls.save, "_cfie_patched", False):
        return
    ...
    _save._cfie_patched = True  # type: ignore[attr-defined]
        ...
        if is_compile_cache_enabled(compiler_config):
            if hash_str is None:
                raise RuntimeError(
                    ...
                    "remove ~/.cache/cfie/torch_compile_cache and try again "
