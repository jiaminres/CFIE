...
from cfie.distributed import get_tp_group
...
class GEMMReduceScatterPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(mul: torch.Tensor, mm_weight: torch.Tensor) -> torch.Tensor:
            ...
            reduce_scatter = torch.ops.cfie.reduce_scatter.default(
                mm,
                dim=0,
...
class AllGatherGEMMPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            all_gather = torch.ops.cfie.all_gather.default(
                x,
                dim=0,
                world_size=self.tp_size,
                group_name=self.tp.unique_name,
...
class ScaledMMReduceScatterPattern(BasePattern):
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        ...
        def replacement(
            ...
            scatter_dim = 0
            gemm_rs = torch.ops.cfie.patched_fused_scaled_matmul_reduce_scatter(
                input,
                mat2,
                scale_a,
                scale_b,
...
class AsyncTPPass(CfiePatternMatcherPass):
    @enable_fake_mode
