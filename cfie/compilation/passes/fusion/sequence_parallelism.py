...
from cfie.distributed.parallel_state import get_tensor_model_parallel_world_size
...
class _SequenceParallelPatternHelper:
    ...
    def _reduce_scatter(self, x: torch.Tensor) -> torch.Tensor:
        return torch.ops.cfie.reduce_scatter.default(
            x, dim=0, world_size=self.tp_size, group_name=self.tp_group.unique_name
        )

    def _all_gather(self, x: torch.Tensor) -> torch.Tensor:
        return torch.ops.cfie.all_gather.default(
            x, dim=0, world_size=self.tp_size, group_name=self.tp_group.unique_name
        )
...
class SequenceParallelismPass(CfiePatternMatcherPass):
    """
