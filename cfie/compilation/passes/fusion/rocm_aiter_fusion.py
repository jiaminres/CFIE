...
import cfie.model_executor.layers.quantization.utils.fp8_utils  # noqa: F401
...
from ..cfie_inductor_pass import CfieInductorPass, CfiePatternMatcherPass
...
class RocmAiterRMSNormQuantFusionPass(CfiePatternMatcherPass):
    """
    This pass fuses aiter rms_norm & cfie/aiter quant custom ops
    into a fused rms_norm_quant op.
    It also supports fused_add_rms_norm.
    """

    @enable_fake_mode
    def __init__(self, config: CfieConfig) -> None:
        ...
        for epsilon in [1e-5, 1e-6]:
            ...
            for match_aiter_quant in [True, False]:
                # Fuse aiter rms_norm + (aiter / cfie built-in)
                # dynamic per-token fp8 quant
                AiterRMSNormDynamicQuantPattern(
                    epsilon, FP8_DTYPE, match_aiter_quant=match_aiter_quant
                ).register(self.patterns)

                # Fuse aiter fused_add_rms_norm + (aiter / cfie built-in)
                # dynamic per-token fp8 quant
...
class RocmAiterSiluMulFp8GroupQuantFusionPass(CfiePatternMatcherPass):
    """
    ...
    def uuid(self) -> str:
        ...
        return CfieInductorPass.hash_source(self, *fusion_patterns)
...
class AddAiterRMSNormPadPattern:
    ...
    def register(self, pm_pass: PatternMatcherPass) -> None:
        def pattern(
            ...
            result_rms, residual_out = self.rmsnorm_matcher(input, weight, residual)
            router_logits = torch.ops.cfie.rocm_unquantized_gemm(
...
class RocmAiterTritonAddRMSNormPadFusionPass(CfiePatternMatcherPass):
    ...
    def uuid(self) -> str:
        return CfieInductorPass.hash_source(self, AddAiterRMSNormPadPattern)
