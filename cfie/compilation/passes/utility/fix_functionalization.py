...
class FixFunctionalizationPass(CfieInductorPass):
    """
    ...
    def __call__(self, graph: torch.fx.Graph) -> None:
        ...
        # Will enable this when switch to cfie-xpu-kernels.
        ...
        if hasattr(torch.ops.cfie, "rocm_aiter_triton_rotary_embedding"):
            rope_targets.append(
                torch.ops.cfie.rocm_aiter_triton_rotary_embedding.default
            )
        ...
        for node in graph.nodes:
            ...
            # rms_norm replacements avoid the most copies for LLaMa.
                ...
                hasattr(torch.ops.cfie, "flashinfer_trtllm_fused_allreduce_norm")
                and at_target
                == torch.ops.cfie.flashinfer_trtllm_fused_allreduce_norm.default
            ):
                mutated_args = {
                    1: "allreduce_in",
                    2: "residual",
            ...
            # Defunctionalize fused_qk_norm_rope to remove higher-order wrapper.
                ...
                hasattr(torch.ops.cfie, "fused_rope_and_unified_kv_cache_update")
                and at_target
                == torch.ops.cfie.fused_rope_and_unified_kv_cache_update.default
            ):
                mutated_args = {
                    1: "query",
                    2: "key",
            ...
            # only used for test_functionalization::TestFunctionWithMutatedArgsAndReturn
                ...
                hasattr(torch.ops.cfie, "function_with_mutated_args_and_return")
                and at_target
                == torch.ops.cfie.function_with_mutated_args_and_return.default
            ):
                mutated_args = {1: "x"}
                self.defunctionalize(graph, node, mutated_args=mutated_args)
            else:
