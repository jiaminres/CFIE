...
from cfie.config import CompilationConfig, CUDAGraphMode, CfieConfig
from cfie.config.compilation import DynamicShapesType
from cfie.config.utils import Range, hash_factors
from cfie.logger import init_logger
from cfie.logging_utils import lazy
from cfie.platforms import current_platform
from cfie.tracing import instrument, instrument_manual
...
class CompilerManager:
    ...
    def compute_hash(self, cfie_config: CfieConfig) -> str:
        return self.compiler.compute_hash(cfie_config)
    ...
    def initialize_cache(
        ...
        self.cache_file_path = os.path.join(cache_dir, "cfie_compile_cache.py")
...
def wrap_with_cudagraph_if_needed(
    ...
    This function is shared between CfieBackend and
...
class CfieBackend:
    ...
    def configure_post_pass(self) -> None:
        self.pass_manager.configure(self.cfie_config)
    ...
    def _log_compilation_config(self):
        ...
        trace_structured(
            "artifact",
            metadata_fn=lambda: {
                "name": "cfie_compilation_config",
                "encoding": "json",
            },
            payload_fn=lambda: json.dumps(
                {
                    "model": self.cfie_config.model_config.model,
                    "prefix": self.prefix,
                    "mode": str(cc.mode),
                    "backend": cc.backend,
    ...
    def __call__(self, graph: fx.GraphModule, example_inputs: Sequence[Any]) -> Any:
        ...
        compiler_hash = self.compiler_manager.compute_hash(cfie_config)
        ...
        self.compilation_config.cache_dir = cache_dir
        rank = cfie_config.parallel_config.rank
        dp_rank = cfie_config.parallel_config.data_parallel_index
        ...
        # TODO(patchy): ngram gpu kernel will cause cfie torch compile cache errors.
        is_ngram_gpu_enabled = (
            cfie_config.speculative_config is not None
            and cfie_config.speculative_config.use_ngram_gpu()
        )
        ...
        # called once
        assert not self._called, "CfieBackend can only be called once"
        ...
        trace_structured(
            "graph_dump",
            metadata_fn=lambda: {"name": "cfie_piecewise_split_graph"},
        ...
        PiecewiseCompileInterpreter(
            self.split_gm, submod_names_to_compile, self.cfie_config, self
        ).run(*fake_args)
