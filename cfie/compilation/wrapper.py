...
from cfie.config import CompilationMode, CUDAGraphMode, get_current_cfie_config
from cfie.config.compilation import DynamicShapesType
from cfie.logger import init_logger
from cfie.utils.nvtx_pytorch_hooks import layerwise_nvtx_marker_context
...
class TorchCompileWithNoGuardsWrapper:
    ...
    def __init__(self) -> None:
        ...
        mode = cfie_config.compilation_config.mode
        self.layerwise_nvtx_tracing_enabled = (
            cfie_config.observability_config.enable_layerwise_nvtx_tracing
        )
        if mode is None:
            raise RuntimeError("Compilation mode cannot be NO_COMPILATION")

        backend = cfie_config.compilation_config.init_backend(cfie_config)
        is_eager_backend = cfie_config.compilation_config.backend == "eager"
        options = {}

        if isinstance(backend, str) and backend == "inductor":
            options = cfie_config.compilation_config.inductor_compile_config

        self.first_compile = True
        self.evaluate_guards = (
            cfie_config.compilation_config.dynamic_shapes_config.evaluate_guards
        )

        ds_type = cfie_config.compilation_config.dynamic_shapes_config.type
        ...
        if is_eager_backend:
            self._compiled_callable = compiled_ptr
    ...
    def bytecode_hook(self, old_code: CodeType, new_code: CodeType) -> None:
        ...
        path = self.cfie_config.compile_debug_dump_path()
        if path:
        ...
        if (
            self.cfie_config.compilation_config.cudagraph_mode != CUDAGraphMode.NONE
...
def reset_compile_wrapper(model: torch.nn.Module) -> None:
    ...
    # Reset the cache_dir so CfieBackend recomputes the hash
    # (data_parallel_size changed, so the config hash differs).
    compilation_config = model.cfie_config.compilation_config
