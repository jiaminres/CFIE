...
from cfie.compilation.wrapper import TorchCompileWithNoGuardsWrapper
...
from cfie.config.compilation import DynamicShapesType
from cfie.forward_context import get_forward_context, is_forward_context_available
...
IGNORE_COMPILE_KEY = "_ignore_compile_cfie"

_T = TypeVar("_T", bound=nn.Module)


def should_torch_compile_mm_encoder(cfie_config: CfieConfig) -> bool:
    """Callable to be passed to `@support_torch_compile`'s `enable_if` argument."""
    return cfie_config.compilation_config.compile_mm_encoder
...
def support_torch_compile(
    *,
    enable_if: Callable[[CfieConfig], bool] | None = None,
...
def support_torch_compile(
    ...
    *,
    ...
    `enable_if` is a function that takes a `CfieConfig` object as input and
...
def _model_hash_key(fn: Callable[..., Any]) -> str:
    ...
    sha256_hash.update(cfie.__version__.encode())
...
def _verify_source_unchanged(
    source_info: "SourceInfo", cfie_config: CfieConfig
) -> None:
    ...
    for source in source_info.inlined_sources:
        ...
        cfie_config.compilation_config.traced_files.add(file)
...
def _try_load_aot_compiled_fn(
    ...
    """
    try:
        with monitor_torch_compile(model.cfie_config):
            with (
                set_current_cfie_config(model.cfie_config),
                open(aot_compilation_path, "rb") as f,
            ):
                loaded_fn = torch.compiler.load_compiled_function(
                    f, f_globals=model.forward.__globals__
                )
            _verify_source_unchanged(loaded_fn.source_info(), model.cfie_config)
            ...
            with maybe_use_cudagraph_partition_wrapper(model.cfie_config):
                loaded_fn._artifacts.compiled_fn.finalize_loading(model.cfie_config)
...
def _support_torch_compile(
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # we may not have cfie_config so we may need to patch
        # it
        sig = inspect.signature(old_init)
        if "cfie_config" in sig.parameters:
            kwargs["cfie_config"] = cfie_config
        ...
        enable_compile = enable_if is None or enable_if(cfie_config)
    ...
    def __call__(self: type[_T], *args: Any, **kwargs: Any) -> Any:
        ...
        if getattr(self, "aot_compiled_fn", None) is not None:
            with maybe_use_cudagraph_partition_wrapper(self.cfie_config):
        ...
        if envs.VLLM_USE_AOT_COMPILE:
            ...
            factors: list[str] = aot_compile_hash_factors(self.cfie_config)
            ...
            rank = self.cfie_config.parallel_config.rank
            dp_rank = self.cfie_config.parallel_config.data_parallel_index
            ...
            if not envs.VLLM_DISABLE_COMPILE_CACHE:
                ...
                if loaded_fn is not None:
                    ...
                    with (
                        ...
                        maybe_use_cudagraph_partition_wrapper(self.cfie_config),
                    ):
        ...
        if self.compiled:
            assert (
                ...
                or self.cfie_config.compilation_config.backend == "eager"
            )
        ...
        # cfie skip guards anyways, setting this flag to False can improve
        # compile time.
        ...
        with (
            ...
            if self.cfie_config.compilation_config.backend == "eager":
            ...
            if use_aot_compile:
                ...
                with monitor_torch_compile(self.cfie_config):
    ...
    # triggers CfieSerializableFunction.serialize()
...
def maybe_use_cudagraph_partition_wrapper(
    ...
    if (
        ...
        from cfie.compilation.cuda_graph import CUDAGraphOptions
