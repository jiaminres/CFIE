...
from cfie.utils.collection_utils import flatten_2d_lists, full_groupby
...
[`modality`][cfie.multimodal.processing.PromptUpdate.modality],
...
class PromptUpdateDetails(Generic[_S]):
    ...
    """
    Given [`full`][cfie.multimodal.processing.PromptUpdateDetails.full],
    ...
    [`SupportsMultiModal.embed_multimodal`][cfie.model_executor.models.interfaces.SupportsMultiModal.embed_multimodal].
    """
...
use [`PromptUpdateDetails`][cfie.multimodal.processing.PromptUpdateDetails] to
specify which part.
"""
...
class PromptInsertion(PromptUpdate):
    ...
    [`target`][cfie.multimodal.processing.PromptUpdate.target].
...
def full_groupby_modality(values: Iterable[_M]) -> ItemsView[str, list[_M]]:
    ...
    [`full_groupby`][cfie.utils.collection_utils.full_groupby]
    based on modality.
    """
...
class ResolvedPromptUpdate:
    """
    A [`PromptUpdate`][cfie.multimodal.processing.PromptUpdate] with its
...
class BaseMultiModalProcessor(ABC, Generic[_I]):
    """
    多模态处理器抽象：把原始多模态输入加工成可送入 vLLM/CFIE 的结构。

    不要与 Hugging Face 的 `transformers.ProcessorMixin` 混淆。
    """
    ...
    def _get_prompt_updates(
        ...
        [`PlaceholderRange`][cfie.multimodal.inputs.PlaceholderRange]
    ...
    def _apply_hf_processor_tokens_only(
        ...
        with the output of
        [`_apply_hf_processor_text_only`][cfie.multimodal.processing.BaseMultiModalProcessor._apply_hf_processor_text_only]
        on the
        corresponding text.
        """
    ...
    def _apply_hf_processor_mm_only(
        ...
        [`DummyInputsBuilder`][cfie.multimodal.processing.BaseDummyInputsBuilder]
