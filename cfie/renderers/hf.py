...
from cfie.transformers_utils.chat_templates import get_chat_template_fallback_path
from cfie.transformers_utils.processor import cached_get_processor
...
if TYPE_CHECKING:
    from cfie.multimodal.inputs import MultiModalDataDict, MultiModalUUIDDict
else:
...
def _log_chat_template_content_format(
    ...
    if given_format != "auto" and given_format != detected_format:
        logger.warning(
            ...
            "https://github.com/cfie-project/cfie/issues/new/choose",
            given_format,
            detected_format,
        )
...
class HfRenderer(BaseRenderer[HfTokenizer]):
    ...
    def from_config(  # type: ignore[override]
        ...
        # 先取出模型配置，判断是否需要真正初始化 tokenizer。
        model_config = config.model_config
        # skip_tokenizer_init=True 时，renderer 不持有 tokenizer 实例。
        if model_config.skip_tokenizer_init:
            tokenizer = None
        else: # 默认
            # 否则复用缓存后的 HF tokenizer 构造逻辑加载 tokenizer。
            tokenizer = cast(
                HfTokenizer,
    ...
    def __init__(
        ...
        # 先执行 BaseRenderer 公共初始化逻辑。
        super().__init__(config, tokenizer)

        # 某些模型会在 HF 配置中声明 unified vision chunk 特性，这里提前缓存。
