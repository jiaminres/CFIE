...
from cfie.inputs.data import build_enc_dec_inputs, embeds_inputs, token_inputs
from cfie.logger import init_logger
from cfie.tokenizers import TokenizerLike
from cfie.utils.async_utils import AsyncMicrobatchTokenizer
from cfie.utils.counter import AtomicCounter
from cfie.utils.torch_utils import set_default_torch_num_threads
from cfie.v1.metrics.stats import MultiModalCacheStats
...
if TYPE_CHECKING:
    ...
    from cfie.entrypoints.chat_utils import (
        ChatCompletionMessageParam,
        ConversationMessage,
    )
    from cfie.multimodal.cache import BaseMultiModalProcessorCache
    from cfie.multimodal.inputs import (
        MultiModalDataDict,
        MultiModalInputs,
        MultiModalUUIDDict,
    )
    from cfie.multimodal.parse import MultiModalDataItems, MultiModalUUIDItems
    from cfie.multimodal.processing import BaseMultiModalProcessor
...
class BaseRenderer(ABC, Generic[_T]):
    ...
    def from_config(
        ...
        # 子类需要根据 CfieConfig 与 tokenizer 参数构造具体 renderer。
        raise NotImplementedError

    def __init__(self, config: "CfieConfig", tokenizer: _T | None) -> None:
        super().__init__()

        # 保存总配置对象，便于后续渲染和多模态处理复用。
        self.config = config

        # 缓存模型配置，减少频繁属性跳转。
        self.model_config = config.model_config

        # 记录当前 API 进程 rank，便于多进程场景下做差异化处理。
        self.api_process_rank = config.parallel_config._api_process_rank

        # 保存 tokenizer；若 skip_tokenizer_init=True 则这里可能为 None。
        self.tokenizer = tokenizer

        # 异步 tokenizer 延迟初始化；离线 LLM 场景往往不会用到它。
        ...
        if config.model_config.is_multimodal_model:
            from cfie.multimodal import MULTIMODAL_REGISTRY as mm_registry
            from cfie.multimodal.registry import MultiModalTimingRegistry

            # 按配置创建多模态处理器缓存。
            mm_processor_cache = mm_registry.processor_cache_from_config(config)

            # 深拷贝一份 tokenizer，避免异步 tokenizer 与 HF processor 并发访问
            # 同一个 Rust tokenizer backend 时触发 “Already borrowed” 错误。
            mm_tokenizer = copy.deepcopy(tokenizer)

            # 在受控线程数环境下创建多模态处理器，避免初始化时线程数膨胀。
            ...
            # 生成多模态内部请求 ID；它和 engine core 的 request_id 无关。
    ...
    def get_tokenizer(self) -> _T:
        # skip_tokenizer_init=True 时 tokenizer 不可用，直接报错提示调用方。
    ...
    def _process_multimodal(
        ...
        from cfie.multimodal.parse import parse_mm_uuids
        from cfie.multimodal.processing import ProcessorInputs as MMProcessorInputs
