...
from cfie.v1.attention.ops.rocm_aiter_mla_sparse import (
...
class rocm_aiter_ops:
    """ROCm AITER 操作封装类，用于在 vLLM 中为 AMD GPU 提供加速能力。

    这个类统一管理 AITER 相关算子的导入、注册和启用检查，
    并提供一套统一接口来判断某个 AITER 功能是否可用。

    只有在以下条件同时满足时，这些操作才可能可用：
    1. 当前平台是 ROCm
    2. 当前设备架构受支持（通常是 gfx9）
    3. 已安装 aiter 库

    这个类通过环境变量细粒度控制不同 AITER 优化是否开启。

    环境变量说明：
        VLLM_ROCM_USE_AITER:
            AITER 总开关，控制所有 AITER 操作是否总体启用。

        VLLM_ROCM_USE_AITER_LINEAR:
            控制线性/GEMM/量化相关操作是否启用。

        VLLM_ROCM_USE_AITER_RMSNORM:
            控制 RMSNorm 操作是否启用。

        VLLM_ROCM_USE_AITER_MOE:
            控制 MoE（Mixture of Experts，专家混合）相关操作是否启用。

        VLLM_ROCM_USE_AITER_MLA:
            控制 MLA（Multi-head Latent Attention）相关操作是否启用。

        VLLM_ROCM_USE_AITER_MHA:
            控制 MHA（Multi-Head Attention）相关操作是否启用，
            包括 flash_attn_varlen。

        VLLM_ROCM_USE_AITER_UNIFIED_ATTENTION:
            控制 Triton unified attention 是否启用。

        VLLM_ROCM_USE_AITER_FP8BMM:
            控制 FP8 batched matrix multiply（批量矩阵乘）是否启用。

        VLLM_ROCM_USE_AITER_FP4_ASM_GEMM:
            控制 FP4 汇编 GEMM 是否启用。

        VLLM_ROCM_USE_AITER_TRITON_ROPE:
            控制 Triton rotary embedding 是否启用。

        VLLM_ROCM_USE_AITER_FUSION_SHARED_EXPERTS:
            控制 shared expert fusion（共享专家融合）是否启用。

        VLLM_ROCM_USE_AITER_TRITON_GEMM:
            控制 Triton 非量化 GEMM 是否启用。

    注意：
        这些环境变量的值会在模块导入时读取并保存，
        导入后再修改环境变量，不会自动反映到这里。

        这样做是为了性能考虑。
        因为频繁读取环境变量有开销，
        尤其是在前向推理这种 hot path 中不希望重复读取。

        如果在单元测试中 monkey patch 了环境变量，
        可以调用 refresh_env_variables() 重新加载环境变量。

    检查函数说明：
        所有检查函数（如 is_*_enabled）都被 @if_aiter_supported 装饰，
        这个装饰器会先检查：
        1. 当前平台是否是 ROCm
        2. 当前设备是否是 gfx9 架构
        3. aiter 库是否已安装

        之后，各自的 is_*_enabled 逻辑还会进一步检查对应环境变量是否开启。

        也就是说：

            is_enabled()
              == current_platform.is_rocm()
                 and current_platform.is_on_gfx9()
                 and IS_AITER_FOUND
                 and cls._AITER_ENABLED

        其中：
        - 前三项由 @if_aiter_supported 检查
        - 最后一项由 is_enabled() 内部逻辑检查

    示例：
        from cfie._aiter_ops import rocm_aiter_ops

        # 在使用 AITER 算子前先判断是否启用
        if rocm_aiter_ops.is_enabled():
            result = rocm_aiter_ops.rms_norm(x, weight, epsilon)

    支持的操作包括：
        - RMS 归一化：
            rms_norm, rms_norm2d_with_add

        - GEMM 操作：
            gemm_a8w8, gemm_a8w8_blockscale

        - Fused MoE：
            fused_moe, asm_moe_tkw1

        - Routing：
            topk_softmax, biased_grouped_topk, grouped_topk

        - MLA decode：
            mla_decode_fwd

        - 量化：
            per_tensor_quant, per_token_quant, group_fp8_quant

        - Triton 操作：
            triton_rotary_embed, triton_fp8_bmm, triton_gemm_a8w8_blockscale
    """

    # -------------------------
    # 以下是在模块导入时一次性读取的环境变量值
    # 不会在运行中自动刷新
    # -------------------------
    ...
    # 是否启用线性层 / GEMM / 量化相关 AITER 优化
    _LINEAR_ENABLED = envs.VLLM_ROCM_USE_AITER_LINEAR

    # 是否启用 RMSNorm 相关 AITER 优化
    _RMSNORM_ENABLED = envs.VLLM_ROCM_USE_AITER_RMSNORM

    # 是否启用 Fused MoE 相关 AITER 优化
    _FMOE_ENABLED = envs.VLLM_ROCM_USE_AITER_MOE

    # 是否启用 MLA 相关 AITER 优化
    _MLA_ENABLED = envs.VLLM_ROCM_USE_AITER_MLA

    # 是否启用 MHA 相关 AITER 优化
    _MHA_ENABLED = envs.VLLM_ROCM_USE_AITER_MHA

    # 是否启用 KV Cache 布局 shuffle
    _SHUFFLE_KV_CACHE_ENABLED = envs.VLLM_ROCM_SHUFFLE_KV_CACHE_LAYOUT

    # 是否启用 Triton Unified Attention
    _TRITON_UNIFIED_ATTN_ENABLED = envs.VLLM_ROCM_USE_AITER_UNIFIED_ATTENTION

    # TODO: 后续考虑并入 _LINEAR_ENABLED
    # 是否启用 FP8 batched matrix multiply
    _FP8BMM_ENABLED = envs.VLLM_ROCM_USE_AITER_FP8BMM

    # 是否启用 FP4 batched matrix multiply
    _FP4BMM_ENABLED = envs.VLLM_ROCM_USE_AITER_FP4BMM

    # TODO: 后续考虑并入 _LINEAR_ENABLED
    # 是否启用 FP4 动态量化汇编 GEMM
    _FP4_GEMM_DYNAMIC_QUANT_ASM = envs.VLLM_ROCM_USE_AITER_FP4_ASM_GEMM

    # TODO: 后续考虑统一到 VLLM_ROCM_USE_AITER_ROPE
    # 是否启用 Triton rotary embedding
    _TRITON_ROTARY_EMBED = envs.VLLM_ROCM_USE_AITER_TRITON_ROPE

    # 是否启用 MoE shared experts 融合优化
    _MOE_SHARED_EXPERTS_ENABLED = envs.VLLM_ROCM_USE_AITER_FUSION_SHARED_EXPERTS

    # TODO: 后续考虑并入 _LINEAR_ENABLED
    # 是否启用 Triton 非量化 GEMM
    ...
    def is_fp4bmm_enabled(cls) -> bool:
        from cfie.platforms.rocm import on_gfx950
    ...
    def get_rmsnorm_fused_add_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_rmsnorm2d_fwd_with_add.default

    @staticmethod
    def get_rmsnorm_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_rms_norm.default

    @staticmethod
    def get_rmsnorm_fused_add_dynamic_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_rmsnorm_fused_add_dynamic_quant.default

    @staticmethod
    def get_rmsnorm_fused_dynamic_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_rmsnorm_fused_dynamic_quant.default

    @staticmethod
    def get_rmsnorm_group_fused_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_rmsnorm_fp8_group_quant.default

    @staticmethod
    def get_rmsnorm_group_add_fused_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_rmsnorm_with_add_fp8_group_quant.default

    @staticmethod
    def get_per_token_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_per_token_quant.default

    @staticmethod
    def get_group_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_group_fp8_quant.default

    @staticmethod
    def get_act_mul_fused_fp8_group_quant_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_act_mul_and_fp8_group_quant.default

    @staticmethod
    def get_triton_add_rmsnorm_pad_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_triton_add_rmsnorm_pad.default

    @staticmethod
    def get_triton_rotary_embedding_op() -> OpOverload:
        return torch.ops.cfie.rocm_aiter_triton_rotary_embedding.default

    @staticmethod
    def rms_norm(
        x: torch.Tensor, weight: torch.Tensor, variance_epsilon: float
    ) -> torch.Tensor:
        return torch.ops.cfie.rocm_aiter_rms_norm(x, weight, variance_epsilon)
    ...
    def rms_norm2d_with_add(
        ...
        return torch.ops.cfie.rocm_aiter_rmsnorm2d_fwd_with_add(
    ...
    def gemm_a8w8(
        ...
        return torch.ops.cfie.rocm_aiter_gemm_a8w8(A, B, As, Bs, bias, output_dtype)
    ...
    def triton_gemm_a8w8_blockscale(
        ...
        return torch.ops.cfie.rocm_aiter_triton_gemm_a8w8_blockscale(
    ...
    def gemm_a8w8_blockscale(
        ...
        return torch.ops.cfie.rocm_aiter_gemm_a8w8_blockscale(
            A, B, As, Bs, output_dtype
        )
    ...
    def fused_moe(
        ...
        return torch.ops.cfie.rocm_aiter_fused_moe(
            hidden_states,
            w1,
            w2,
            topk_weight,
    ...
    def asm_moe_tkw1(
        ...
        return torch.ops.cfie.rocm_aiter_asm_moe_tkw1(
            hidden_states,
            w1,
            w2,
            topk_weights,
    ...
    def topk_softmax(
        ...
        torch.ops.cfie.rocm_aiter_topk_softmax(
            topk_weights, topk_indices, token_expert_indices, gating_output, renormalize
        )
    ...
    def topk_sigmoid(
        ...
        torch.ops.cfie.rocm_aiter_topk_sigmoid(
    ...
    def biased_grouped_topk(
        ...
        torch.ops.cfie.rocm_aiter_biased_grouped_topk(
            gating_output,
            correction_bias,
            topk_weights,
            topk_ids,
    ...
    def grouped_topk(
        ...
        torch.ops.cfie.rocm_aiter_grouped_topk(
            gating_output,
            topk_weights,
            topk_ids,
            num_expert_group,
    ...
    def fused_topk(
        ...
        return torch.ops.cfie.rocm_aiter_fused_topk(x, router_logits, top_k, gate_up)
    ...
    def mla_decode_fwd(
        ...
        torch.ops.cfie.rocm_aiter_mla_decode_fwd(
            q,
            kv_buffer.view(-1, 1, 1, q.shape[-1]),
            o,
            qo_indptr,
    ...
    def per_tensor_quant(
        ...
        return torch.ops.cfie.rocm_aiter_per_tensor_quant(x, quant_dtype, scale)
    ...
    def per_token_quant(
        ...
        return torch.ops.cfie.rocm_aiter_per_token_quant(x, quant_dtype, scale)
    ...
    def gemm_a8wfp4(
        ...
        return torch.ops.cfie.rocm_aiter_gemm_a8wfp4(
    ...
    def group_fp8_quant(
        ...
        return torch.ops.cfie.rocm_aiter_group_fp8_quant(input_2d, group_size)
