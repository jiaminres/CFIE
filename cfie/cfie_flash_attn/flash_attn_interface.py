...
try:
    from . import _cfie_fa2_C  # type: ignore[attr-defined]  # noqa: F401

    FA2_OPS = torch.ops._cfie_fa2_C
    FA2_UNAVAILABLE_REASON = None
    FA2_AVAILABLE = True
except ImportError as e:
    try:
        # Prefer the single native artifact built under cfie.vllm_flash_attn.
        from cfie.vllm_flash_attn import _vllm_fa2_C  # type: ignore[attr-defined]  # noqa: F401

        FA2_OPS = torch.ops._vllm_fa2_C
        FA2_UNAVAILABLE_REASON = None
        FA2_AVAILABLE = True
    except ImportError:
...
try:
    from . import _cfie_fa3_C  # type: ignore[attr-defined]  # noqa: F401

    FA3_OPS = torch.ops._cfie_fa3_C
    FA3_UNAVAILABLE_REASON = None
    FA3_AVAILABLE = True
except ImportError as e:
    try:
        from cfie.vllm_flash_attn import _vllm_fa3_C  # type: ignore[attr-defined]  # noqa: F401

        FA3_OPS = torch.ops._vllm_fa3_C
        FA3_UNAVAILABLE_REASON = None
        FA3_AVAILABLE = True
    except ImportError:
...
try:
    ...
    if not os.path.exists(_cute_interface_path):
        raise ImportError("cfie.cfie_flash_attn.cute.interface not found")
...
def get_scheduler_metadata(
    ...
    if headdim_v is None:
        headdim_v = headdim
    assert FA3_OPS is not None
    scheduler_metadata = FA3_OPS.get_scheduler_metadata(
        batch_size,
        max_seqlen_q,
        max_seqlen_k,
        num_heads_q,
...
def flash_attn_varlen_func(
    ...
    if fa_version == 2:
        ...
        assert FA2_OPS is not None
        out, softmax_lse = FA2_OPS.varlen_fwd(
            q,
            k,
            v,
            out,
        ...
        assert FA3_OPS is not None
        out, softmax_lse, _, _ = FA3_OPS.fwd(
            q,
            k,
            v,
            None,
        ...
        from cfie.cfie_flash_attn.cute.interface import _flash_attn_fwd
...
def sparse_attn_func(
    ...
    *,
    ...
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    assert FA2_OPS is not None
    out, softmax_lse = FA2_OPS.fwd_sparse(
        q,
        k,
        v,
        block_count,
...
def sparse_attn_varlen_func(
    ...
    *,
    ...
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    assert FA2_OPS is not None
    out, softmax_lse = FA2_OPS.varlen_fwd_sparse(
        q,
        k,
        v,
        block_count,
