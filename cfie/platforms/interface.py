...
class Platform:
    ...
    def get_vit_attn_backend(
        ...
        """
        获取某个设备平台上的 Vision Transformer attention backend。

        注意：
        ViT Attention 的 backend 选择应由“平台相关实现”检查并覆盖，
        不应在其他位置（例如 model_executor/models/<model_name>.py）随意重写。

        逻辑分两种情况：
        1. 如果 backend 不为 None，说明用户或配置显式指定了后端：
           - 先检查该后端是否被当前平台支持
           - 支持则直接使用
        2. 如果 backend 为 None：
           - 走平台默认选择逻辑
        ...
        # 如果显式传入了 backend，优先尝试使用它
        if backend is not None:
            # 检查当前平台是否支持这个 backend
            assert backend in cls.get_supported_vit_attn_backends(), (
                f"Backend {backend} is not supported for vit attention"
                f"Supported backends are: {cls.get_supported_vit_attn_backends()}"
            )

            # 打日志：说明使用了显式指定的 backend
            logger.info_once(f"Using backend {backend} for vit attention")

            return backend

        # 如果没有显式指定 backend，则使用默认 backend
        logger.info_once(
    ...
    def has_device_capability(
        ...
        [`DeviceCapability.to_int`][cfie.platforms.interface.DeviceCapability.to_int])
        """
    ...
    def pre_register_and_update(
        ...
        This function is called before global CfieConfig is initialized or cli
    ...
    def apply_config_platform_defaults(cls, cfie_config: "CfieConfig") -> None:
        ...
        This function is called during the initialization of global CfieConfig, after
    ...
    def get_device_communicator_cls(cls) -> str:
        ...
        """
        return "cfie.distributed.device_communicators.base_device_communicator.DeviceCommunicatorBase"  # noqa
    ...
    def stateless_init_device_torch_dist_pg(
        ...
        """
        初始化平台相关的 stateless 设备侧 torch ProcessGroup。

        不同平台会在各自实现中选择合适的设备 backend
        （例如 CUDA/ROCm 上的 NCCL/RCCL）。
        """
        raise NotImplementedError
    ...
    def use_custom_op_collectives(cls) -> bool:
        """
        Whether this platform should use torch.ops.cfie.* custom ops for collectives.
