...
from cfie.v1.attention.backend import is_quantized_kv_cache
...
class CpuPlatform(Platform):
    ...
    def check_and_update_config(cls, cfie_config: CfieConfig) -> None:
        ...
        if cfie_config.kv_transfer_config and (
        ...
        if parallel_config.worker_cls == "auto":
            parallel_config.worker_cls = "cfie.v1.worker.cpu_worker.CPUWorker"
        # Disable DBO
        ...
        from cfie.config import CompilationMode

        cfie_config.compilation_config.cudagraph_capture_sizes = []

        compilation_config = cfie_config.compilation_config
        if cfie_config.compilation_config.mode == CompilationMode.VLLM_COMPILE:
        ...
        cfie_config.profiler_config.torch_profiler_dump_cuda_time_total = False

        assert cfie_config.device_config.device_type == "cpu"
        ...
        if (
            ...
            # See: https://github.com/cfie-project/cfie/issues/27369
            # TODO: Remove once:
            ...
            # Search both torch.libs and torch/lib - See: https://github.com/cfie-project/cfie/issues/30470
            torch_libs_paths = [
        ...
        os.environ["LOCAL_WORLD_SIZE"] = str(
            cfie_config.parallel_config.tensor_parallel_size
        )
        ...
        if model_config is not None and model_config.use_mla:
            ...
            cfie_config.scheduler_config.max_num_batched_tokens = max(
                cfie_config.model_config.max_model_len,
                cfie_config.scheduler_config.DEFAULT_MAX_NUM_BATCHED_TOKENS,
            )
    ...
    def get_punica_wrapper(cls) -> str:
        return "cfie.lora.punica_wrapper.punica_cpu.PunicaWrapperCPU"
    ...
    def get_device_communicator_cls(cls) -> str:
        ...
        """
        return "cfie.distributed.device_communicators.cpu_communicator.CpuCommunicator"  # noqa
        ...
        if Platform.get_cpu_architecture() in (CpuArchEnum.X86,):
            ...
            if torch.cpu._is_avx512_supported():
                if torch.cpu._is_avx512_bf16_supported():
                    ...
                    try:
                        import cfie._C_AVX512  # noqa: F401
                    except ImportError as e:
                        if ignored_msg not in e.msg:
                            logger.warning(
                                "Failed to import from cfie._C_AVX512: %r", e
                            )
            else:
                try:
                    import cfie._C_AVX2  # noqa: F401
                except ImportError as e:
                    if ignored_msg not in e.msg:
                        logger.warning("Failed to import from cfie._C_AVX2: %r", e)
        else:
            try:
                import cfie._C  # noqa: F401
            except ImportError as e:
