...
try:
    import cfie._rocm_C  # noqa: F401
except ImportError as e:
    logger.warning("Failed to import from cfie._rocm_C with %r", e)
...
def _get_backend_priorities(
    ...
    if (
        ...
        and cfie_config.attention_config.use_prefill_decode_attention
    ):
...
class RocmPlatform(Platform):
    ...
    def check_and_update_config(cls, cfie_config: "CfieConfig") -> None:
        ...
        if cache_config and not cache_config.user_specified_block_size:
            if (
                ...
                # TODO: monitor https://github.com/cfie-project/cfie/pull/30396
    ...
    def get_device_communicator_cls(cls) -> str:
        return (
            "cfie.distributed.device_communicators.cuda_communicator.CudaCommunicator"  # noqa
        )
    ...
    def stateless_init_device_torch_dist_pg(
        ...
        # -------------------- 构造 NCCL/RCCL backend 所需选项 --------------------
        backend_options = ProcessGroupNCCL.Options()
        # 把调用方传入的 timeout 写到 backend 选项中。
        backend_options._timeout = timeout

        # -------------------- 创建真正的设备侧 backend 实例 --------------------
        backend_class = ProcessGroupNCCL(
            # backend 复用同一份 PrefixStore 做 rendezvous。
            prefix_store, group_rank, group_size, backend_options
        )
        # 后端类型仍然注册为 NCCL。
        backend_type = ProcessGroup.BackendType.NCCL
        # ROCm 路径在 torch 侧仍使用 cuda 设备类型标识。
        device = torch.device("cuda")
        # 把设备侧 backend 设为这个 ProcessGroup 的默认 backend。
        pg._set_default_backend(backend_type)
        # 初始化 sequence number，保证 collective 次序一致。
        backend_class._set_sequence_number_for_group()

        # 把 backend 挂到 ProcessGroup 外壳上。
        pg._register_backend(device, backend_type, backend_class)
        # 返回已经完成组装的 stateless 设备 ProcessGroup。
        return pg
