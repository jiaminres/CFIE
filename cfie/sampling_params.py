...
from cfie.config import ModelConfig, SpeculativeConfig, StructuredOutputsConfig
...
from cfie.v1.serial_utils import PydanticMsgspecMixin
...
class SamplingParams(
    ...
    # nucleus sampling 的累计概率阈值。
    top_p: float = 1.0
    """Controls the cumulative probability of the top tokens to consider. Must
    be in (0, 1]. Set to 1 to consider all tokens."""
    # top-k sampling 的候选 token 数量。
    top_k: int = 0
    """Controls the number of top tokens to consider. Set to 0 (or -1) to
    consider all tokens."""
    # 相对最高概率 token 的最小保留概率阈值。
    min_p: float = 0.0
    ...
    # 每个 prompt token 返回多少个 logprob。
    prompt_logprobs: int | None = None
    """Number of log probabilities to return per prompt token.
    When set to -1, return all `vocab_size` log probabilities."""
    # 是否把 logprobs 以扁平结构返回。
    ...
    # 是否将生成出的 token detokenize 成文本。
    detokenize: bool = True
    """Whether to detokenize the output."""
    # 输出文本时是否跳过 special tokens。
    skip_special_tokens: bool = True
    """Whether to skip special tokens in the output."""
    # 输出特殊 token 时是否在它们之间补空格。
    spaces_between_special_tokens: bool = True
    """Whether to add spaces between special tokens in the output."""
    # 输出文本中是否保留 stop 字符串。
    include_stop_str_in_output: bool = False
    """Whether to include the stop strings in output text."""
    # 请求输出的组织形式，如累计增量或最终结果。
    output_kind: RequestOutputKind = RequestOutputKind.CUMULATIVE
    # 内部优化开关，允许复用当前 SamplingParams 而不做克隆。
    ...
    # 内部缓冲长度，用于 stop 字符串判定时保留尾部文本。
    output_text_buffer_length: int = 0
    # 内部记录的 eos token id。
    _eos_token_id: int | None = None
    # 内部汇总后的全部停止 token id 集合。
    ...
    # 是否跳过 prefix cache 读取，由引擎按场景自动决定。
    ...
    def _validate_logits_processors(self, model_config: ModelConfig) -> None:
        from cfie.v1.sample.logits_processor import (
    ...
    def _validate_structured_outputs(
        ...
        from cfie.v1.structured_output.backend_guidance import (
            has_guidance_unsupported_json_features,
            validate_guidance_grammar,
        )
        from cfie.v1.structured_output.backend_lm_format_enforcer import (
            validate_structured_output_request_lm_format_enforcer,
        )
        from cfie.v1.structured_output.backend_outlines import (
            validate_structured_output_request_outlines,
        )
        from cfie.v1.structured_output.backend_xgrammar import validate_xgrammar_grammar
