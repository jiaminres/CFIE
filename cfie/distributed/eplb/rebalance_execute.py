...
from cfie.distributed.parallel_state import get_ep_group
...
# move_to_buffer / transfer_layer 都返回“不变行、本地接收行、远端接收元数据”三元组。
...
def get_ep_ranks_with_experts_batch(
    ...
    # 处理旧映射：这些位置表示当前有哪些 rank 手里持有对应 expert。
    ...
    if np.any(old_relevant_mask):
        ...
        # 对每个 expert，按“首次出现顺序”提取唯一发送 rank。
    ...
    # ----------------- 再在新映射里找“哪些 rank 未来需要这些 expert，可作为接收方” -----------------
    ...
    if np.any(new_relevant_mask):
        ...
        # 对每个 expert，提取唯一接收 rank，并排除本来就本地持有副本的 ranks。
    ...
    # 对“只发送不接收”或“只接收不发送”的 expert，把另一边补成空列表。
