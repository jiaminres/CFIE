...
from cfie.config import CfieConfig, get_current_cfie_config, get_layers_from_cfie_config
...
from cfie.v1.outputs import KVConnectorOutput, ModelRunnerOutput
...
def get_kv_connector_cache_layout():
    ...
    kv_config = cfie_config.kv_transfer_config
    if kv_config is not None:
        connector_cls = KVConnectorFactory.get_connector_class(kv_config)
        required_kvcache_layout = connector_cls.get_required_kvcache_layout(cfie_config)
...
def get_current_attn_backends(
    cfie_config: CfieConfig, layer_names: list[str] | None = None
) -> list[type[AttentionBackend]]:
    """Get all distinct attention backends for the given layers.

    Args:
        cfie_config: The current vLLM configuration.
    ...
    layers = get_layers_from_cfie_config(cfie_config, layer_type, layer_names)
    if layers:
    ...
    return [
        get_attn_backend(
            head_size=cfie_config.model_config.get_head_size(),
            dtype=cfie_config.model_config.dtype,
            kv_cache_dtype=cfie_config.cache_config.cache_dtype,
            use_mla=cfie_config.model_config.use_mla,
        )
    ]
...
def get_current_attn_backend(
    ...
    return get_current_attn_backends(cfie_config, layer_names)[0]
