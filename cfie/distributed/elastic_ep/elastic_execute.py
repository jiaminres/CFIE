...
from cfie.compilation.cuda_graph import CUDAGraphWrapper
from cfie.compilation.wrapper import reset_compile_wrapper
...
from cfie.distributed.elastic_ep.standby_state import (
...
from cfie.model_executor.layers.fused_moe.layer import FusedMoEParallelConfig
from cfie.v1.engine import ReconfigureDistributedRequest, ReconfigureRankType
from cfie.v1.worker.gpu_ubatch_wrapper import UBatchWrapper
from cfie.v1.worker.workspace import lock_workspace, unlock_workspace
...
class ElasticEPScalingExecutor:
    ...
    def create_standby_groups(
        ...
        world_size = self.worker.cfie_config.parallel_config.world_size
        new_world_size_across_dp = world_size * new_dp_size
        updated_config = copy.copy(self.worker.cfie_config)
        updated_config.parallel_config = copy.deepcopy(
            self.worker.cfie_config.parallel_config
        )
        updated_config.parallel_config.data_parallel_size = new_dp_size
        with set_current_cfie_config(updated_config):
            create_standby_groups(
    ...
    def transfer_weights(self, old_dp_size: int, new_dp_size: int) -> None:
        ...
        dp_rank = self.worker.cfie_config.parallel_config.data_parallel_rank
    ...
    def switch_and_prepare(self) -> None:
        ...
        parallel_config = self.worker.cfie_config.parallel_config
        ...
        with set_current_cfie_config(self.worker.cfie_config):
        ...
        if (
            self.worker.cfie_config.compilation_config.mode
            ...
            backend = self.worker.cfie_config.compilation_config.init_backend(
                self.worker.cfie_config
            )
    ...
    def receive_expert_mapping(self) -> tuple[torch.Tensor, int, int]:
        ...
        tp_size = self.worker.cfie_config.parallel_config.tensor_parallel_size
        new_ep_size = new_dp_size * tp_size
        expanded_physical_to_logical = torch.full(
