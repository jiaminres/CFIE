...
from cfie.v1.outputs import ECConnectorOutput
...
class ECConnectorBase(ABC):
    def __init__(self, cfie_config: "CfieConfig", role: ECConnectorRole):
        self._connector_metadata: ECConnectorMetadata | None = None
        self._cfie_config = cfie_config
        self._role = role
        if cfie_config.ec_transfer_config is not None:
            self._is_producer = cfie_config.ec_transfer_config.is_ec_producer
            self._is_consumer = cfie_config.ec_transfer_config.is_ec_consumer
        else:
