...
from cfie.logging_utils import ColoredFormatter, NewLineFormatter
...
DEFAULT_LOGGING_CONFIG: dict[str, dict[str, Any] | Any] = {
    "formatters": {
        "cfie": {
            "class": "cfie.logging_utils.NewLineFormatter",
            "datefmt": _DATE_FORMAT,
            "format": _FORMAT,
        },
        "cfie_color": {
            "class": "cfie.logging_utils.ColoredFormatter",
            "datefmt": _DATE_FORMAT,
            "format": _FORMAT,
        },
    },
    "handlers": {
        "cfie": {
            ...
            "formatter": "cfie_color" if _use_color() else "cfie",
...
def _should_log_with_scope(scope: LogScope) -> bool:
    ...
    if scope == "global":
        from cfie.distributed.parallel_state import is_global_first_rank

        return is_global_first_rank()
    if scope == "local":
        from cfie.distributed.parallel_state import is_local_first_rank
...
_METHODS_TO_PATCH = {
    "debug_once": _CfieLogger.debug_once,
    "info_once": _CfieLogger.info_once,
    "warning_once": _CfieLogger.warning_once,
}


def _configure_cfie_root_logger() -> None:
    ...
    if envs.VLLM_CONFIGURE_LOGGING:
        ...
        cfie_handler = logging_config["handlers"]["cfie"]
        # Refresh these values in case env vars have changed.
        cfie_handler["level"] = envs.VLLM_LOGGING_LEVEL
        cfie_handler["stream"] = envs.VLLM_LOGGING_STREAM
        cfie_handler["formatter"] = "cfie_color" if _use_color() else "cfie"

        cfie_loggers = logging_config["loggers"]["cfie"]
        cfie_loggers["level"] = envs.VLLM_LOGGING_LEVEL
    ...
    for formatter in logging_config.get("formatters", {}).values():
        ...
        if formatter.get("class") == "cfie.logging.NewLineFormatter":
            formatter["class"] = "cfie.logging_utils.NewLineFormatter"

    if logging_config:
        dictConfig(logging_config)


def init_logger(name: str) -> _CfieLogger:
    """The main purpose of this function is to ensure that loggers are
    retrieved in such a way that we can be sure the root cfie logger has
    ...
    return cast(_CfieLogger, logger)
...
def current_formatter_type(logger: Logger) -> Literal["color", "newline", None]:
    ...
    while lgr is not None:
        if lgr.handlers and len(lgr.handlers) == 1 and lgr.handlers[0].name == "cfie":
...
def _trace_calls(log_path, root_dir, frame, event, arg=None):
    if event in ["call", "return"]:
        ...
        if not filename.startswith(root_dir):
            # only log the functions in the cfie root_dir
            return
...
def enable_trace_function_call(log_file_path: str, root_dir: str | None = None):
    ...
    if root_dir is None:
        # by default, this is the cfie root directory
