...
def import_pynvml():
    ...
    https://github.com/cfie-project/cfie/issues/12847 for example.
    After all the troubles, we decide to copy the official `pynvml`
    module to our codebase, and use it directly.
    """
    import cfie.third_party.pynvml as pynvml
...
def import_triton_kernels():
    ...
    `site-packages`. Use `cfie.third_party.triton_kernels` as a fall-back.
    """
    ...
    elif _has_module("cfie.third_party.triton_kernels"):
        import cfie.third_party.triton_kernels as triton_kernels
...
@cache
def get_cfie_optional_dependencies():
    metadata = importlib.metadata.metadata("cfie")
...
class _PlaceholderBase:
    ...
    [`__getattr__`][cfie.utils.import_utils._PlaceholderBase.__getattr__]
...
class PlaceholderModule(_PlaceholderBase):
    ...
    def __getattr__(self, key: str) -> Never:
        ...
        try:
            ...
            for extra, names in get_cfie_optional_dependencies().items():
                if name in names:
                    msg = f"Please install cfie[{extra}] for {extra} support"
...
def has_triton_kernels() -> bool:
    ...
    if not (
            _has_module("triton_kernels")
            or _has_module("cfie.third_party.triton_kernels")
    ):
        return False

    # 先挂载顶层包，再检查关键子模块是否真的存在。
    import_triton_kernels()
    return (
            _has_module("triton_kernels.matmul_ogs")
            and _has_module("triton_kernels.swiglu")
    )
